...
- PRESERVES Excel DataValidation arrows (↑→↓) automatically
- PRESERVES emoji symbols in string literals and comparisons
- Detailed reporting with conversion examples
- Summary statistics
- Export report to file

Important:
- Automatically preserves Unicode arrows in Excel DataValidation formula1 strings
- Automatically preserves emojis in string literals (for Excel status matching)
- This prevents Excel import issues and data extraction failures
- All other Unicode is safely converted to ASCII
...
Usage:
    ...
    python3 convert_unicode_to_ascii.py --recursive ./src
...
Date: 2026-01-17
Updated: 2026-01-17 - Added DataValidation arrow + emoji preservation
...
def remove_utf8_declarations(text: str) -> tuple[str, int]:
    """
    Remove UTF-8 encoding declarations from Python source code.
    ...
    # Common UTF-8 encoding declaration patterns
    utf8_patterns = [
        '# -*- coding: utf-8 -*-',
        '# coding: utf-8',
        '# coding=utf-8',
    ...
    for line in lines:
        ...
        # Check if line matches any UTF-8 encoding pattern
        ...
        # Keep line if it's not an encoding declaration
...
def preserve_datavalidation_arrows(text: str) -> tuple[str, dict]:
    """
    Preserve Unicode arrows in DataValidation formula1 strings.
    Returns: (protected_text, restoration_map)
    """
    restoration_map = {}
    
    # Pattern to match DataValidation formula1 with Unicode arrows
    # Handles nested quotes like: formula1='"value"'  or formula1="'value'"
    # Match the full DataValidation(...) block
    pattern = r'(DataValidation\([^)]+\))'
    
    def protect_match(match):
        full_match = match.group(1)
        
        # Check if contains arrow characters we want to preserve
        arrow_chars = {'↑', '→', '↓', '←', '↔', '↕'}
        if any(arrow in full_match for arrow in arrow_chars):
            # Create unique placeholder
            placeholder = f'__DATAVALIDATION_PRESERVE_{len(restoration_map)}__'
            restoration_map[placeholder] = full_match
            return placeholder
        
        return full_match  # No arrows, return unchanged
    
    protected_text = re.sub(pattern, protect_match, text)
    return protected_text, restoration_map


def preserve_emoji_strings(text: str, existing_map: dict) -> tuple[str, dict]:
    """
    Preserve string literals containing emoji symbols (for Excel status matching).
    Uses a simpler character-by-character approach for reliability.
    Returns: (protected_text, updated_restoration_map)
    """
    restoration_map = existing_map.copy()
    
    # Common emoji used in ISMS scripts for status matching
    # Using explicit Unicode values for reliability
    emoji_to_preserve = {
        '✅': '__EMOJI_CHECK_MARK__',
        '❌': '__EMOJI_CROSS_MARK__',
        '⚠️': '__EMOJI_WARNING__',
        '⚠': '__EMOJI_WARNING_PLAIN__',
        '✓': '__EMOJI_CHECK__',
        '💾': '__EMOJI_FLOPPY__',
        '🎯': '__EMOJI_TARGET__',
        '🔷': '__EMOJI_BLUE_DIAMOND__',
        '🔶': '__EMOJI_ORANGE_DIAMOND__',
        '•': '__EMOJI_BULLET__',
        '️': '__EMOJI_VARIANT_SELECTOR__',  # Variant selector U+FE0F
        # Unicode comparison operators (CRITICAL for Excel threshold display values!)
        '≥': '__UNICODE_GTE__',  # Greater than or equal (U+2265) - MUST preserve for Excel!
        '≤': '__UNICODE_LTE__',  # Less than or equal (U+2264) - MUST preserve for Excel!
        '≠': '__UNICODE_NEQ__',  # Not equal (U+2260)
    }
    
    # Replace each emoji with its placeholder
    protected_text = text
    for emoji, placeholder in emoji_to_preserve.items():
        if emoji in protected_text:
            # Store in restoration map
            restoration_map[placeholder] = emoji
            # Replace in text
            protected_text = protected_text.replace(emoji, placeholder)
    
    return protected_text, restoration_map


def restore_preserved_content(text: str, restoration_map: dict) -> str:
...
def convert_file(filepath: Path, dry_run: bool = False, show_samples: bool = False, 
    ...
    try:
        ...
        # Step 1: Preserve DataValidation arrows before conversion
        protected_text, restoration_map = preserve_datavalidation_arrows(original)
        
        # Step 2: Preserve emoji strings
        protected_text, restoration_map = preserve_emoji_strings(protected_text, restoration_map)
        
        # Step 3: Convert using unidecode (on protected text)
        converted = unidecode(protected_text)
        
        # Step 4: Restore preserved content
        converted = restore_preserved_content(converted, restoration_map)
        
        # Step 5: Remove UTF-8 encoding declarations (no longer needed for ASCII files)
        converted, encoding_lines_removed = remove_utf8_declarations(converted)
        ...
        result['changed'] = True
        result['encoding_lines_removed'] = encoding_lines_removed
...
def generate_report(results: List[Dict], report_file: Path, dry_run: bool, backup_enabled: bool):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        total_encoding_lines = sum(r.get('encoding_lines_removed', 0) for r in results)
        ...
        if total_encoding_lines > 0:
            f.write(f"UTF-8 encoding lines removed: {total_encoding_lines}\n")
        if backed_up > 0:
        ...
        if converted > 0:
            ...
            for result in results:
                if result['changed']:
                    ...
                    encoding_lines = result.get('encoding_lines_removed', 0)
                    if encoding_lines > 0:
                        f.write(f"UTF-8 encoding lines removed: {encoding_lines}\n")
...
Common Unicode → ASCII conversions:
  ...
  "smart quotes" → "regular quotes"
        ...
        if args.verbose or (not args.recursive and len(all_files) < 20):
            if result['success']:
                if result['changed']:
                    ...
                    encoding_removed = result.get('encoding_lines_removed', 0)
                    
                    info_parts = [f"{char_count} chars"]
                    if encoding_removed > 0:
                        info_parts.append(f"{encoding_removed} encoding line(s)")
                    
                    print(f"{status} ({', '.join(info_parts)})", end='')
                    if result.get('backup_path'):
    ...
    if total_encoding_lines > 0:
        print(f"UTF-8 encoding declarations removed: {total_encoding_lines}")
