...
def get_project_covered_cg_ids(db: DBSession, project_id: str | None, org_id: str) -> set:
    """Return ControlGroup IDs in scope for the given project (or org-wide)."""
    import uuid as _uuid
    from src.domain.projects import ProjectChecklist
    from src.domain.assessments import Assessment
    if project_id:
        stmt = select(ProjectChecklist.control_group_id).where(
            ProjectChecklist.project_id == _uuid.UUID(project_id)
        )
    else:
        # Org-wide: all control groups that have any assessment
        stmt = (
            select(Assessment.control_group_id)
