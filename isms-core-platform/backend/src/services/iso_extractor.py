...
_FILENAME_TO_STANDARD: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"nist.*csf.*2\.0|csf.*2\.0|nist.*cybersecurity.*framework.*2|CSWP.*32|nist.*cswp", re.I), "NIST CSF 2.0"),
    (re.compile(r"IR.*8374|nist.*ir.*8374",  re.I),   "NIST IR 8374 (CSF 2.0 Privacy)"),
    (re.compile(r"fips.*199|NIST\.FIPS\.199", re.I),  "NIST FIPS 199"),
    (re.compile(r"800-34",            re.I),   "NIST SP 800-34r1"),
    (re.compile(r"800-39",            re.I),   "NIST SP 800-39"),
    (re.compile(r"800-40",            re.I),   "NIST SP 800-40r4"),
    (re.compile(r"800-47",            re.I),   "NIST SP 800-47r1"),
    (re.compile(r"800-207A",           re.I),   "NIST SP 800-207A"),
    (re.compile(r"800-207",           re.I),   "NIST SP 800-207"),
    (re.compile(r"800-53A",           re.I),   "NIST SP 800-53Ar5"),
    ...
    (re.compile(r"800-63B",           re.I),   "NIST SP 800-63B-4"),
    (re.compile(r"800-63",            re.I),   "NIST SP 800-63-4"),
    (re.compile(r"800-82",            re.I),   "NIST SP 800-82r3"),
    (re.compile(r"800-86",            re.I),   "NIST SP 800-86"),
    (re.compile(r"800-88",            re.I),   "NIST SP 800-88r2"),
    (re.compile(r"800-92",            re.I),   "NIST SP 800-92"),
    (re.compile(r"800-115",           re.I),   "NIST SP 800-115"),
    (re.compile(r"800-128",           re.I),   "NIST SP 800-128"),
    (re.compile(r"800-137",           re.I),   "NIST SP 800-137"),
    (re.compile(r"800-145",           re.I),   "NIST SP 800-145"),
    (re.compile(r"800-146",           re.I),   "NIST SP 800-146"),
    (re.compile(r"800-160",           re.I),   "NIST SP 800-160"),
    (re.compile(r"800-161",           re.I),   "NIST SP 800-161r1"),
    (re.compile(r"800-171A",          re.I),   "NIST SP 800-171Ar3"),
    (re.compile(r"800-171",           re.I),   "NIST SP 800-171r3"),
    (re.compile(r"800-190",           re.I),   "NIST SP 800-190"),
    (re.compile(r"800-218|ssdf",      re.I),   "NIST SP 800-218 (SSDF)"),
    (re.compile(r"nist.*ai.*100|ai.*100-1|nist.*ai.*rmf", re.I), "NIST AI RMF 1.0"),
    (re.compile(r"CSWP.*01162020|nist.*privacy.*framework", re.I), "NIST Privacy Framework 1.0"),
    (re.compile(r"stopransomware|ransomware.*guide", re.I), "CISA StopRansomware Guide"),
    (re.compile(r"zero.trust.maturity|ztmm", re.I), "CISA Zero Trust Maturity Model v2.0"),
    (re.compile(r"cloud.*security.*technical.*reference|cloud.*tra", re.I), "CISA Cloud Security TRA v2"),
    (re.compile(r"C5.*2020|bsi.*c5", re.I),   "BSI C5:2020"),
    ...
    (re.compile(r"cisa.*playbook|cybersecurity.*incident.*vulnerability.*response", re.I), "CISA Incident & Vulnerability Response Playbooks"),
    (re.compile(r"PSSI.IA|20250203.*pssi",      re.I),  "CLUSIF PSSI-IA 2025"),
    (re.compile(r"OT.ICS.*2025|cybersecurity.*guide.*OT", re.I), "CLUSIF Cybersecurity Guide OT/ICS 2025"),
    (re.compile(r"DoD.*Control.*Systems.*SRG|control.*systems.*SRG", re.I), "DoD Control Systems SRG"),
    (re.compile(r"ZT.*Operational.*Technology|OperationalTechnology.*Activities", re.I), "DoD Zero Trust OT Activities"),
    (re.compile(r"ICS.*SCADA.*Depend|SCADA.*depend|WP2016.*ICS", re.I), "ENISA ICS/SCADA Dependencies"),
    (re.compile(r"diamond.*model.*intrusion|intrusion.*analysis", re.I), "Diamond Model of Intrusion Analysis"),
    (re.compile(r"cyber.*kill.*chain|gaining.*advantage.*cyber", re.I), "Lockheed Martin Cyber Kill Chain"),
    (re.compile(r"unified.*kill.*chain", re.I), "Unified Kill Chain"),
    (re.compile(r"IATF|information.*assurance.*technical.*framework", re.I), "NSA IATF 3.1"),
...
def discover_documents(root: Path) -> list[Path]:
    ...
    _SKIP_PATTERNS = {
        ...
        "800-37r2",         # NIST SP 800-37r2 — OOM on PDF extraction → use DOCX version
    }
    ...
    for p in sorted(root.rglob("*")):
        ...
        if suffix == ".pdf" and any(skip in name_lower for skip in _PDF_SKIP_PATTERNS):
            continue
        # Format preference: PDF > DOCX > TXT for the same stem in the same dir.
        # PDF extraction via PyMuPDF is highest fidelity for well-formed PDFs.
        # DOCX (python-docx) is preferred over Acrobat plain-text export when the
        # PDF is skipped (OOM/corrupt) but a Word version exists.
        if suffix == ".txt":
            # Skip if a non-skipped PDF sibling exists
            pdf_sibling = p.with_suffix(".pdf")
            if pdf_sibling.exists() and pdf_sibling.stat().st_size >= 10_000:
                pdf_name_lower = pdf_sibling.name.lower()
                if not any(skip in pdf_name_lower for skip in _PDF_SKIP_PATTERNS):
                    continue  # PDF will be used instead
            # Skip if a DOCX sibling exists (DOCX > TXT when PDF is unavailable)
            docx_sibling = p.with_suffix(".docx")
            if docx_sibling.exists() and docx_sibling.stat().st_size >= 10_000:
                continue  # DOCX will be used instead
        if suffix == ".docx":
            # Skip if a non-skipped PDF sibling exists
            pdf_sibling = p.with_suffix(".pdf")
            if pdf_sibling.exists() and pdf_sibling.stat().st_size >= 10_000:
