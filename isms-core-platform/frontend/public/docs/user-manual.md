...
- Run compliance assessments across 29 regulatory frameworks (NIS2, DORA, NIST CSF 2.0, NIST AI RMF 1.0, PCI DSS v4.0.1, BSI C5:2026, CSA CCM, NIST SP 800-53, NCSC CAF v4.0, and more)
...
ISMS CORE includes 29 built-in compliance assessment modules. Each module maps your ISO 27001 control posture to a specific regulatory framework.
...
| BSI C5:2026 | German federal cloud security compliance criteria (2026 edition) — 130+ criteria, 17 domains |
| BSI C3A | BSI Cybersecurity and Cloud Criteria for AI — AI-specific cloud security overlay |
| PCI DSS v4.0.1 | Payment Card Industry Data Security Standard — 12 requirements, 323 sub-requirements, 6 milestones |
...
The **Intelligence** section aggregates live threat and vulnerability data from two dedicated feed containers. All feeds run on a fixed schedule and seed automatically on first boot. No external subscription is required for the core feeds; some OSINT feeds require free API keys.

### Feed containers

The platform runs two separate feed containers:

- **`isms-core-feeds`** — vulnerability and adversary intelligence (always active)
- **`isms-core-threat-intel`** — OSINT IOC feeds (optional; enabled with `VITE_THREAT_INTEL_ENABLED=true`)

When the threat-intel profile is not active, all Intelligence sidebar items remain visible but are greyed out.

### Available feeds — vulnerability & adversary intelligence

These feeds run in the `isms-core-feeds` container and are always active.

| Feed | Schedule (UTC) | Description |
|------|----------------|-------------|
| **MITRE ATT&CK v19** | Weekly, Sun 00:00 | Full adversary TTP matrix — tactics, techniques, sub-techniques, mitigations, groups, software |
| **MITRE ATLAS** | Weekly, Sun 00:30 | AI/ML-targeted attack techniques (separate dataset from ATT&CK) |
| **CISA KEV** | Daily 02:00 | Known Exploited Vulnerabilities — CVEs confirmed exploited in the wild |
| **EPSS** | Daily 02:30 | Exploit Prediction Scoring System — probability of exploitation in the next 30 days |
| **NVD CVE** | Daily delta 03:00 / Full weekly Sun 01:00 | ~250,000 CVEs with CVSS v2/v3/v4 scores, CWEs, and CPE applicability |
| **NVD CPE** | Weekly, Sun 01:30 | Common Platform Enumeration entries for vendor/product resolution |
| **ENISA EUVD** | Daily | EU Vulnerability Database — exploited-flag CVEs and high-severity EU-assigned entries |
| **Exploit-DB** | Daily | ~52,000 public exploits; cross-enriches CVEs with `edb_id` and `edb_verified` flags |
| **MaxMind GeoLite2** | Weekly, Tue 01:00 | GeoIP database used for IP-to-country resolution across all feeds |

### Available feeds — OSINT IOC feeds

These feeds run in the `isms-core-threat-intel` container. All feeds seed automatically on first boot if no previous successful run exists.

| Feed | Schedule (UTC) | API key | Description |
|------|----------------|---------|-------------|
| **CIRCL MISP** | Every 6h: 00:00, 06:00, 12:00, 18:00 | None | Public OSINT MISP feed — IPs, domains, URLs, hashes with ATT&CK TIDs and Malpedia tags |
| **Botvrij MISP** | Every 6h: 01:00, 07:00, 13:00, 19:00 (staggered) | None | Public OSINT MISP feed — same schema, deduplicated against CIRCL by value + source |
| **AbuseIPDB** | Daily 02:00 | `ABUSEIPDB_API_KEY` (required) | Top 10,000 confidence=100 abusive IPs; single-IP on-demand enrichment cached 24h |
| **URLhaus** | Daily 03:00 | None | Malicious URLs hosting malware payloads, from abuse.ch |
| **ThreatFox** | Every 6h: 03:00, 09:00, 15:00, 21:00 | `THREATFOX_API_KEY` (optional) | IOCs (IPs, domains, URLs, hashes) linked to named malware families |
| **SSLBL** | Daily 04:00 | None | SSL certificate blacklist for known C2 infrastructure, from abuse.ch |
| **AlienVault OTX** | Daily 04:30 | `OTX_API_KEY` (required) | Pulses from the Open Threat Exchange — IOCs with TLP labels, ATT&CK TIDs, confidence scores |
| **Feodo Tracker** | Every 6h: 04:30, 10:30, 16:30, 22:30 | None | Botnet C2 IP blocklist for Feodo/Emotet/TrickBot/QakBot families |
| **Red Flag Domains** | Daily 05:00 | None | Domains flagged for phishing, malware, and C2 activity |
| **Stopforumspam** | Daily 05:30 | None | ~140,000 IPs reported for spam, botnet activity, and forum abuse |
| **MalwareBazaar** | Every 6h: 02:00, 08:00, 14:00, 20:00 | `MALWAREBAZAAR_API_KEY` (required) | Recent malware samples — MD5/SHA1/SHA256 hashes with family attribution |
| **Malpedia** | Weekly, Sun 03:00 | None | Malware family knowledge base (aliases, ATT&CK TIDs) and threat actor directory |
| **VirusTotal** (enrichment) | Daily 07:00 | `VT_API_KEY` (optional) | Enriches existing IOCs with VT detection ratios — updates confidence scores only, does not add new IOCs |
| **IPInfo** (enrichment) | Runs after AbuseIPDB | `IPINFO_API_KEY` (optional) | Geo-enriches AbuseIPDB IPs with city, country, and ASN data; 120s time budget per run |

> **First-boot note:** All OSINT feeds run once on startup to seed the database. Large feeds (CIRCL MISP ~133K IOCs, Stopforumspam ~140K IPs, AlienVault OTX ~90K IOCs) take several minutes each. The scheduler will handle subsequent delta updates automatically.
...
- **RUN** button to trigger any feed on demand outside its schedule (admin only)
- **CANCEL** button to abort a running feed (admin only)

### Threat Exposure

Go to **Intelligence → Threat Exposure** to see which of your ISO 27001 controls are exposed to active threat techniques observed in the OSINT feeds.

> Requires the `isms-core-threat-intel` container to be active and populated.

The page works by joining MITRE ATT&CK technique IDs attached to live IOCs in `ti_iocs` against the ATT&CK → ISO 27001 crosswalk. Controls that appear in the crosswalk are then cross-referenced against your framework assessment scores.

**Summary bar:**
- **Active techniques** — distinct ATT&CK technique IDs seen in live IOC feeds
- **Affected controls** — ISO 27001 controls mapped to those techniques
- **Gaps identified** — affected controls with an assessment score below threshold or status "non_compliant" / "not_assessed"

**Technique → Control table:**

| Column | Meaning |
|--------|---------|
| Technique | ATT&CK TID (e.g. T1190 — Exploit Public-Facing Application) |
| IOC count | Number of active IOCs tagged with this technique |
| Sources | Feed sources contributing IOCs for this technique |
| ISO 27001 controls | Controls mapped to this technique, coloured by assessment score (green ≥ 70%, orange 40–69%, red < 40%, grey = not assessed) |
...
Click a CVE row to open the detail panel — shows CVSS v2/v3/v4 scores and vectors, CWEs, CPE applicability statements, EPSS score, KEV status, EUVD badge, Exploit-DB badge, and NVD reference links.
...
Go to **Intelligence → IOC Explorer** to search and filter Indicators of Compromise across all OSINT feeds.
...
| Column | Description |
|--------|-------------|
| Type | IOC type chip (IP / Domain / URL / hash) |
| Value | The indicator value (truncated; expand row for full value) |
| Source | Feed that contributed this IOC |
| Confidence | Detection ratio (0–100%); sourced from feed metadata or VirusTotal enrichment |
| TLP | Traffic Light Protocol label (WHITE / GREEN / AMBER / RED) if provided by the source |
...
> **Malpedia note:** Malpedia does not contribute rows to the IOC Explorer — it populates the Malware Atlas (families, actors, tools) instead. A Malpedia result count of 0 in the IOC Explorer is correct.
...
> Requires the `isms-core-threat-intel` container. No API key needed — data sourced from the Malpedia MISP galaxy open dataset.
...
|----------|-------------|-------------------|
| `NIST_API_KEY` | Faster NVD seeding (raises limit from 5→50 req/30s) | nvd.nist.gov |
...
| `OTX_IMPORT_DAYS` | History depth for OTX first run (default: `90`) | — |
| `THREATFOX_API_KEY` | Higher rate limits for ThreatFox (works without key at lower limits) | threatfox.abuse.ch |
...
> **VirusTotal enrichment detail:** VT does not add new IOCs — it queries IOCs already in the database (NULL confidence first, then oldest check) and updates their `confidence` field only when the VT score is higher than the existing value. IOCs are re-checked every 30 days. The free tier allows ~500 requests/day; the default cap of 450 keeps the feed safely under that limit (15-second sleep between requests, ~4 req/min).

The first NVD full run indexes ~250,000 CVEs into OpenSearch — this takes several hours without a NIST API key.
