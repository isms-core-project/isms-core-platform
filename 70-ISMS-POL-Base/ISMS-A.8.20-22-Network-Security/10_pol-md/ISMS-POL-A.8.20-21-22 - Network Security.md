...
References to specific cloud providers (AWS, Azure, GCP) throughout this policy are illustrative of cloud networking concepts. Applicable cloud environments are documented in the network infrastructure inventory (Workbook 1). As of policy effective date, [Organization]'s in-scope cloud environments are:

- **Primary cloud provider**: [Specify: AWS / Azure / GCP / Other]
- **Secondary cloud provider** (if applicable): [Specify or "None"]
- **SaaS with network integration**: [List any SaaS requiring VPN/private connectivity]
...
### 2.1 Network Infrastructure Security (A.8.20)
...
| **Network Access Controls** | Industry-standard network access control mechanisms; Port security on switches; Authentication-based access control | ISMS-IMP-A.8.20-21-22-S3 |
...
### 2.2 Network Services Security (A.8.21)
...
|-------------|------------------------------|-------------------------|
| **DNS** | Industry-standard DNS security mechanisms; Split DNS architecture; DNS query logging; DNS filtering for malicious domains; DDoS protection for public DNS | ISMS-IMP-A.8.20-21-22-S4 |
| **DHCP** | DHCP security mechanisms on switches; Rogue DHCP detection; Documented scopes with periodic review; High availability for critical DHCP services | ISMS-IMP-A.8.20-21-22-S4 |
| **NTP** | NTP authentication mechanisms; Documented time source hierarchy; Access control restrictions; Time synchronization monitoring | ISMS-IMP-A.8.20-21-22-S4 |
...
**Cloud Applicability**: Cloud-provided network services (Route 53, Azure DNS, Cloud Load Balancing) **MUST** implement equivalent security mechanisms using cloud-native capabilities. See ISMS-IMP-A.8.20-21-22-S4 for cloud-specific guidance.

### 2.3 Network Segmentation (A.8.22)
...
|--------|--------|----------------------|---------------------|
| Network device hardening compliance | ≥95% | Quarterly | <90% triggers remediation plan |
| Network device inventory accuracy | ≥98% | Quarterly | <95% triggers discovery review |
| Firewall rule review completion | 100% within review cycle | Per defined review cycle | Any overdue review escalated to CISO |
| Segmentation effectiveness test pass rate | 100% critical zones | Annually minimum | Any critical zone failure triggers immediate remediation |
| Network services availability (critical) | ≥99.9% | Monthly | <99.5% triggers incident review |
| Mean time to patch critical vulnerabilities (network devices) | ≤14 days | Monthly | >30 days triggers exception request |
| Exception backlog | ≤5 active exceptions | Quarterly | >10 triggers executive review |
...
**Next Review Date**: [Effective Date + 12 months]  
**Document Location**: [Organization] Internal Policy Portal

---

*This policy establishes requirements for network security controls covering network infrastructure (A.8.20), network services (A.8.21), and network segmentation (A.8.22). Implementation procedures (HOW) are documented in ISMS-IMP-A.8.20-21-22 Implementation Guides. Technical standards and configuration guidance are provided in supporting materials (CIS benchmarks, vendor hardening guides, topology diagram templates). Assessment and compliance evidence is generated via automated Python workbooks for systematic network security evaluation.*