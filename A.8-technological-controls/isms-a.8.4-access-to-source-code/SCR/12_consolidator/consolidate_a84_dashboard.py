...
# =============================================================================
"""
ISMS-A.8.4 Source Code Access - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('A.8.4.1', 'Repository_Access', 'Repository Access'),
    ('A.8.4.2', 'Branch_Protection', 'Branch Protection'),
]

DASHBOARD_PATTERNS = ['A.8.4.3', 'Source_Code_Security_Dashboard']
            ...
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'issue']):
                ...
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    if status or gap_desc:
                        ...
                        gaps.append([f'GAP-SRC-{gap_counter:03d}', assessment_area,
                            os.path.basename(filepath), row_values[0] or 'Repository',
...
def generate_risks_and_remediation(gaps):
    ...
    for i, gap in enumerate([g for g in gaps if g[7] == 'High'], 1):
        risks.append([f'RISK-{i:03d}', gap[0], 'Source Code Security', f'Risk: {gap[4][:80]}',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.8.4 Source Code Access - Dashboard Consolidation")
    print("=" * 80)
