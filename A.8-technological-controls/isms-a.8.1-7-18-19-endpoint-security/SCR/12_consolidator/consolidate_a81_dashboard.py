...
# =============================================================================
"""
ISMS-A.8.1-7-18-19 Endpoint Security - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('S1_Endpoint_Inventory', 'A.8.1-7-18-19.S1', 'Endpoint Inventory'),
    ('S2_Protection_Coverage', 'A.8.1-7-18-19.S2', 'Protection Coverage'),
    ('S3_Software_Controls', 'A.8.1-7-18-19.S3', 'Software Controls'),
    ('S4_Privileged_Utilities', 'A.8.1-7-18-19.S4', 'Privileged Utilities'),
    ('S5_Compliance_Matrix', 'A.8.1-7-18-19.S5', 'Compliance Matrix'),
]

DASHBOARD_PATTERNS = ['S6_Compliance_Dashboard', 'A.8.1-7-18-19.S6']
...
def extract_gaps_from_workbook(filepath, assessment_area, gap_counter_start=1):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'issue', 'non-compliant']):
                ws = wb[sheet_name]
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    if status or gap_desc:
                        ...
                        gaps.append([f'GAP-EPT-{gap_counter:03d}', assessment_area,
                            os.path.basename(filepath), row_values[0] or 'Endpoint',
...
def generate_risks_and_remediation(gaps):
    ...
    for i, gap in enumerate([g for g in gaps if g[7] == 'High'], 1):
        risks.append([f'RISK-{i:03d}', gap[0], 'Endpoint Security', f'Risk: {gap[4][:80]}',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.8.1-7-18-19 Endpoint Security - Dashboard Consolidation")
    print("=" * 80)
