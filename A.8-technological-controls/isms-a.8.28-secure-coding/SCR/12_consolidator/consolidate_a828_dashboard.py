...
# =============================================================================
"""
ISMS-A.8.28 Secure Coding - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('A.8.28.1', 'SDLC_Assessment', 'SDLC Assessment'),
    ('A.8.28.2', 'Standards_Tools', 'Standards & Tools'),
    ('A.8.28.3', 'Code_Review', 'Code Review & Testing'),
    ('A.8.28.4', 'Third_Party_OSS', 'Third Party/OSS'),
]

DASHBOARD_PATTERNS = ['A.8.28.5', 'Compliance_Summary_Dashboard']
            ...
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'issue']):
                ...
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    if status or gap_desc:
                        ...
                        gaps.append([f'GAP-COD-{gap_counter:03d}', assessment_area,
                            os.path.basename(filepath), row_values[0] or 'Code Base',
...
def generate_risks_and_remediation(gaps):
    ...
    for i, gap in enumerate([g for g in gaps if g[7] == 'High'], 1):
        risks.append([f'RISK-{i:03d}', gap[0], 'Secure Coding', f'Risk: {gap[4][:80]}',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.8.28 Secure Coding - Dashboard Consolidation")
    print("=" * 80)
