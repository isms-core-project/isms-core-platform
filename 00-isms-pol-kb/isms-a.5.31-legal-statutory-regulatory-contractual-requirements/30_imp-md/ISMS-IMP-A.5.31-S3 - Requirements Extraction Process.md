...
**Document ID**: ISMS-IMP-A.5.31-S3  
**Title**: Requirements Extraction Process  
**Version**: 1.0  
**Related Policy**: ISMS-POL-A.5.31-S3 (Requirements Extraction & Control Mapping Framework)  
**Purpose**: Operational guide for extracting actionable requirements from applicable regulations
...
This implementation guide operationalizes the requirements extraction methodology defined in ISMS-POL-A.5.31-S3 Section 2. It provides step-by-step instructions for transforming regulatory text into specific, actionable requirements that can be mapped to ISO 27001 controls and implemented by [Organization].
...
**Trigger**: Regulation determined applicable (IMP-S1 process complete, regulation added to ISMS-POL-00 as Tier 1 or Tier 2)

**Input**: 
- Regulation text (official legal text)
- ISMS-POL-00 entry for regulation
- Applicability Assessment (from IMP-S1)

**Output**:
- Requirements Register populated with extracted requirements
- Each requirement ready for control mapping (IMP-S3 next step)
...
```
[Applicable Regulation - Tier 1 or Tier 2 from IMP-S1]
              ↓
...
[Requirements Register Updated & Approved]
              ↓
[Proceed to Control Mapping (IMP-S3)]
```
...
- Applicability Assessment (from IMP-S1, provides context)
...
## 2. Step 1 - Parse Regulation Structure
...
Reconfirm what regulation applies to (already assessed in IMP-S1, but re-read for extraction context).
...
## 3. Step 2 - Identify Mandatory vs. Recommendatory Language
...
## 5. Step 4 - Categorize Requirements
...
## 6. Step 5 - Requirements Register Entry
...
- **Initially**: Blank (populated during IMP-S3 Control Mapping process)
...
## 8. Example Requirements Extractions
...
| Mapped Controls | [TBD - during IMP-S3] |
| Gap Status | TBD |
...
### 9.3 Requirement ID Numbering Cheatsheet
