...
| **Related Policy** | ISMS-POL-A.8.24-S2.3 (Data Storage Requirements) |
...
|---------|------|---------|--------|
| 1.0 | [Original Date] | Initial technical specification for Data Storage assessment workbook | ISMS Implementation Team |
...
This assessment implements **ISMS-POL-A.8.24-S2.3 (Data Storage Requirements)** which defines mandatory cryptographic controls for:
...
- [ ] Centralized BitLocker recovery key repository (Active Directory, Azure AD)
...
- [ ] Policy repository (ISMS-POL-A.8.24-S2.3)
...
2. **Sheet 2: Laptops & Workstations**
   ...
   - Verify recovery key escrow (AD, Azure AD, Jamf)
...
### 4.2 Section 2: Laptops & Workstations
...
# Key Protectors: TPM, Recovery Password (escrowed to AD/Azure AD)
```
...
- [ ] **Recovery keys escrowed to Active Directory, Azure AD, or organizational KMS**  
  *Critical:* Without escrow, device is bricked if user forgets password  
  *Verify:* Check AD/Azure AD console for BitLocker recovery keys  
...
### 4.3 Additional Sections Summary (4.4 through 4.7)
...
- Recovery key escrow verification (AD/Azure AD/Jamf)
...
**Problem:** BitLocker enabled on laptops but recovery keys not saved to AD/Azure AD  
**Why Wrong:** If employee forgets password or hardware fails, device is permanently locked (business data loss)  
**Correct Approach:** Enable BitLocker with recovery key backup to AD/Azure AD/Intune  
...
**Policy Requirement:** Mobile devices storing organizational data MUST use full device encryption (Policy Section 2.3.1)
...
**Policy Requirement:** All laptops and workstations containing sensitive data (Confidential or Restricted) MUST implement full disk encryption (Policy Section 2.3.2)
...
| Recovery keys escrowed to Active Directory, Azure AD, or organizational KMS | [ ] Yes [ ] No [ ] N/A | Critical for business continuity |
...
**Policy Requirement:** Servers storing Confidential or Restricted data MUST implement disk/volume encryption (Policy Section 2.3.3)
...
**Policy Requirement:** Databases containing Confidential or Restricted data MUST implement Transparent Data Encryption (TDE) or equivalent (Policy Section 2.3.4)
...
**Policy Requirement:** Cloud storage containing Confidential or Restricted data MUST use encryption with customer-managed keys (CMEK) preferred over cloud-service-managed keys (CSEK) (Policy Section 2.3.5)
...
**Policy Requirement:** Backups containing Confidential or Restricted data MUST be encrypted using AES-256 minimum (Policy Section 2.3.6)
...
**Policy Requirement:** Removable media (USB drives, external disks) containing organizational data MUST be encrypted (Policy Section 2.3.7)
