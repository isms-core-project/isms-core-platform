...
| **Related Policy** | ISMS-POL-A.8.24-S2.5 (Authentication Requirements) |
...
|---------|------|---------|--------|
| 1.0 | [Original Date] | Initial technical specification for Authentication assessment workbook | ISMS Implementation Team |
...
**Required Knowledge:**
- [Organization]'s identity provider(s) (Active Directory, Azure AD, Okta, etc.)
...
**Complexity Factors:**
- **Simple:** Single identity provider (AD or Azure AD), password-only with some MFA - 6 hours
...
This assessment implements **ISMS-POL-A.8.24-S2.5 (Authentication Requirements)** which defines mandatory cryptographic controls for:
...
**Password Security (Section 2.5.1):**
...
**Multi-Factor Authentication (Section 2.5.2):**
...
**Certificate-Based Authentication (Section 2.5.3):**
...
**Service Accounts (Section 2.5.4):**
...
- [ ] Azure Active Directory (Azure AD / Entra ID) admin portal
...
- [ ] Service account inventory (AD, Azure AD, application databases)
...
- [Organization]'s identity architecture (AD, Azure AD, hybrid, multi-IdP)
...
# Check user MFA status (Azure AD)
Get-MsolUser -All | Select-Object DisplayName, UserPrincipalName, @{N="MFA Status"; E={if($_.StrongAuthenticationRequirements.State){"Enabled"}else{"Disabled"}}}
...
**Reporting Tools:**
- **MFA Enrollment Reports:** Export from Duo, Okta, Azure AD, etc.
- **Password Policy Auditing:** GPO export, Azure AD policy export
...
- Export password policies (AD GPO, Azure AD, application configs)
...
1. **Sheet 1: Password Security** ← START HERE (foundation of all authentication)
   ...
   - Export Azure AD password policy
   ...
   - **Evidence:** GPO exports, Azure AD policy screenshots, database password hash samples
...
4. **Sheet 4: Service Accounts**
   - Generate service account inventory (AD, Azure AD, Linux, databases)
...
**Work in Batches:**
- Export all password policies at once (AD, Azure AD, applications)
...
- Azure AD PowerShell for cloud policy extraction
...
### 4.1 Section 1: Password Security
...
- **"No":** Only if [Organization] is 100% certificate-based or biometric (extremely rare - even Azure AD passwordless still has fallback passwords)
- **"Not Applicable":** Not appropriate for this section

**Where to Find This Information:**
- Active Directory Users and Computers
- Azure AD user portal
...
|-------|-----------------|----------|---------------|
| **System/Application** | Authentication system name | "Active Directory", "Azure AD", "PostgreSQL production database", "Linux servers /etc/shadow" | Identity provider, application docs |
...
- **Good Practice:** Enable Azure AD Password Protection for compromised password detection
...
- [ ] **Minimum 12 characters (14 characters preferred)**  
  *Verify:* AD: `Get-ADDefaultDomainPasswordPolicy`, Azure AD: Password policy settings  
...
- [ ] **Password expiration: 90 days (privileged), 180 days (standard), OR no expiry with strong MFA**  
  ...
  *Verify:* AD: `Get-ADDefaultDomainPasswordPolicy`, Azure AD: Password policy  
...
- [ ] **Account lockout: 5 failed attempts, 30-minute lockout**  
  ...
  *Verify:* AD: Account Lockout Policy, Azure AD: Smart Lockout  
...
- Azure AD password policy: `EV-1-AzureAD-Password-Policy-20260115.png`
...
**Solution:** NTLM is acceptable for AD (industry standard), enhance with Azure AD Password Protection for compromised password detection
...
### 4.2 Section 2: Multi-Factor Authentication (MFA)
...
| Field | How to Complete | Examples | Where to Find |
|-------|-----------------|----------|---------------|
| **System/Application** | What system requires MFA? | "VPN access", "Azure AD - all users", "AWS Console", "Privileged accounts" | MFA enrollment reports, policies |
...
- [ ] **MFA enforced for access to Confidential/Restricted data**  
  *Verify:* Conditional access policies (Azure AD), application-specific MFA  
...
**4. Service Accounts:**
- Service account inventory (AD, Azure AD, Linux, databases)
...
# MFA status (requires Azure AD PowerShell)
Get-MsolUser -All | Select DisplayName, UserPrincipalName, @{N="MFA";E={$_.StrongAuthenticationRequirements.State}} | Export-Csv "EV-2-MFA-Status.csv"
...
```bash
# Azure AD password policy
az ad policy list --query "[?contains(type, 'password')]" > EV-1-AzureAD-Password-Policy.json
...
**Policy Requirement:** Passwords MUST be hashed using approved algorithms (bcrypt, Argon2, PBKDF2, scrypt). Minimum 12 characters, complexity requirements enforced (Policy Section 2.5.1)
...
|-------------------|----------------------|-----------|--------------------|-----------------------|------------------------|--------------------| ---------|-------------------|-----------------|-------------------|
| Example: Active Directory, Azure AD, PostgreSQL database, Linux /etc/shadow | | | | | | | [ ] ✅ [ ] ⚠️ [ ] ❌ [ ] N/A | | | [ ] Yes [ ] No |
...
- **Primary identity provider:** [ ] Active Directory [ ] Azure AD [ ] LDAP [ ] Other: _____
- **Password policy enforcement:** [ ] Centralized (AD/Azure AD) [ ] Per-application [ ] Mixed
...
**Policy Requirement:** MFA REQUIRED for all privileged accounts, access to Confidential/Restricted data, and all remote access (Policy Section 2.5.2)
...
|-------------------|----------------------|-----------|--------------------|-----------|-----------------| ---------|-------------------|-----------------|-------------------|
| Example: VPN access, Azure AD, AWS Console, Privileged accounts | | | | | | [ ] ✅ [ ] ⚠️ [ ] ❌ [ ] N/A | | | [ ] Yes [ ] No |
...
**Policy Requirement:** Certificate-based authentication MUST use RSA 2048-bit minimum (RSA 3072+ or ECDSA P-256+ preferred). CRL/OCSP revocation checking REQUIRED (Policy Section 2.5.3)
...
**Policy Requirement:** Service accounts MUST use strong passwords (same complexity as human accounts) OR certificate-based authentication. Password rotation quarterly (90 days) minimum OR managed service identities. Service accounts MUST NOT have interactive login rights (Policy Section 2.5.4)
...
|-------------------|----------------------|-----------|--------------------|--------------|--------------|-----------| ---------|-------------------|-----------------|-------------------|
| Example: SQL Server service account, Linux cron job, Azure AD application, AWS IAM service account | | | | | | | [ ] ✅ [ ] ⚠️ [ ] ❌ [ ] N/A | | | [ ] Yes [ ] No |
...
**Policy Requirement:** SSO/Federation MUST use SAML 2.0, OAuth 2.0, or OpenID Connect (OIDC). IdP signing certificates MUST be valid and monitored for expiration. Federation trust relationships MUST be documented and reviewed annually (Policy Section 2.5.5)
...
|-------------------|----------------------|-----------|--------------------|--------------|----|-------------------| ---------|-------------------|-----------------|-------------------|
| Example: Office 365, Salesforce, AWS SSO, Internal apps via Azure AD SAML | | | | | | | [ ] ✅ [ ] ⚠️ [ ] ❌ [ ] N/A | | | [ ] Yes [ ] No |
...
| S | Identity Provider (IdP) | 20 | Azure AD, Okta, Auth0, Google, Internal, Other |
...
**Additional Details:**
- **SSO solution:** [ ] Azure AD [ ] Okta [ ] Auth0 [ ] Google Workspace [ ] Other: _____
...
## 6. Overall Authentication Summary
...
**Evidence Types:**
- Password policy exports (GPO, Azure AD, application configs)
