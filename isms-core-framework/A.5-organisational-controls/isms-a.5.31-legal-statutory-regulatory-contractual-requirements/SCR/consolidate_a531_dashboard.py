...
# =============================================================================
"""
ISMS-A.5.31 Legal/Regulatory Compliance - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('A.5.31.1', 'Regulatory_Inventory', 'Regulatory Inventory'),
    ('A.5.31.2', 'Applicability_Matrix', 'Applicability Matrix'),
    ('A.5.31.3', 'Requirements_Register', 'Requirements Register'),
    ('A.5.31.4', 'Control_Mapping', 'Control Mapping'),
    ('A.5.31.5', 'Evidence_Register', 'Evidence Register'),
]

DASHBOARD_PATTERNS = ['A.5.31.6', 'Compliance_Dashboard']
...
def extract_gaps_from_workbook(filepath, assessment_area, gap_counter_start=1):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'non-compliance', 'issue']):
                ws = wb[sheet_name]
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    if status or gap_desc:
                        ...
                        gaps.append([f'GAP-REG-{gap_counter:03d}', assessment_area,
                            os.path.basename(filepath), row_values[0] or 'Requirement',
...
def generate_risks_and_remediation(gaps):
    ...
    for i, gap in enumerate([g for g in gaps if g[7] == 'High'], 1):
        risks.append([f'RISK-{i:03d}', gap[0], 'Compliance', f'Risk: {gap[4][:80]}',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.5.31 Legal/Regulatory - Dashboard Consolidation")
    print("=" * 80)
    ...
    print(f"  Total: {len(all_gaps)} gaps | {len(all_risks)} risks | {len(all_remediation)} remediation | {len(all_evidence)} evidence\n")
    return True
...
# =============================================================================
# QA_VERIFIED: 2026-01-30
# QA_STATUS: PASSED (syntax validated, auto-detect pattern implemented)
