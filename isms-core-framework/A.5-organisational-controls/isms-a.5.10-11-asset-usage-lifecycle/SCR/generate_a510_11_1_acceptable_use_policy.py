...
Assessment Domain 1 of 3: Acceptable Use Policy Assessment
...
**Purpose:**
Enables systematic assessment of Acceptable Use Policy Assessment compliance under ISO 27001:2022 Controls A.5.10 and A.5.11. Supports evidence-based documentation of acceptable use adherence, asset lifecycle tracking, and return procedures for audit readiness.
...
Advanced Usage:
    ...
    python3 generate_a510_11_1_acceptable_use_policy.py --output /path/to/dir

    # Generate with specific date suffix
    python3 generate_a510_11_1_acceptable_use_policy.py --date 20250115

Output:
    File: ISMS-IMP-A.5.10-11.S1_Acceptable_Use_Policy_Assessment_YYYYMMDD.xlsx
...
Version 1.0 - [Date to be set]
    - Initial release
    - Implements full assessment framework per ISMS-IMP-A.5.10-11.S1 specification
...
def create_instructions_sheet(ws):
    ...
    for i, line in enumerate([
        '1. Complete Policy Assessment — evaluate acceptable use policy coverage for all asset types.',
        '2. Complete Asset Coverage — confirm AUP scope includes all information asset categories.',
        '3. Complete Awareness Tracking — verify all personnel have acknowledged the AUP.',
        '4. Complete Communication Matrix — confirm AUP is communicated at onboarding and annually.',
        '5. Maintain Evidence Records with policy acknowledgement signatures and training records.',
...
def create_policy_assessment_sheet(ws, styles):
    """Create the Policy Assessment sheet - AUP completeness evaluation."""
    # Row 1: Title banner
    ...
    ws["A2"] = "Assess AUP policy content completeness against ISO 27001:2022 A.5.10 requirements"
    ...
    # Data validations (applied to rows 5-54, after sample row 4)
    ...
    sample_data = [
        "REQ-001",
        "Policy scope defines applicable assets (information, hardware, software, network, cloud)",
        "Scope",
        "Yes",
        "Section 2.1",
        "Clear",
        ...
        "CISO",
        "Compliant",
        "Full coverage across all asset types confirmed",
        "AUP-v2.3.pdf",
    ]
    ...
    # Pre-populate ISO 27001:2022 A.5.10 requirements (rows 5-9) — grey as pre-populated data
    requirements = [
        ("REQ-001", "Policy scope defines applicable assets", "Scope"),
        ("REQ-002", "Policy defines acceptable business use", "Permitted Use"),
        ("REQ-003", "Policy explicitly prohibits unauthorised activities", "Prohibited Use"),
        ("REQ-004", "Personal use guidelines are documented", "Personal Use"),
        ("REQ-005", "Monitoring and logging disclosure included", "Monitoring"),
    ]
...
def create_asset_coverage_sheet(ws, styles):
    """Create the Asset Coverage sheet - coverage by asset category."""
    ...
    ws["A2"] = "Verify acceptable use policy coverage for each information asset category"
    ws["A2"].font = Font(name="Calibri", size=10, italic=True)
    ws["A2"].alignment = Alignment(horizontal="left", vertical="center")
    ...
    sample_data = [
        "Information Assets",
        "Documents, databases, intellectual property, email",
        "Yes",
        "Section 3.1",
        "Yes",
        "Yes",
    ...
    # Pre-populate asset categories (rows 5-9)
    asset_categories = [
    ...
    grey_fill = PatternFill(start_color="F2F2F2", end_color="F2F2F2", fill_type="solid")
    for row_idx, (category, examples) in enumerate(asset_categories, start=5):
        ws.cell(row=row_idx, column=1, value=category).fill = grey_fill
        ws.cell(row=row_idx, column=1).border = THIN_BORDER
        ws.cell(row=row_idx, column=2, value=examples).fill = grey_fill
        ws.cell(row=row_idx, column=2).border = THIN_BORDER
    ...
    # Empty FFFFCC rows (50 empty rows after sample + pre-pop: rows 10-59)
    for r in range(len(asset_categories) + 5, 60):
        for c in range(1, 10):
            cell = ws.cell(row=r, column=c)
            cell.fill = styles["input_cell"]["fill"]
            cell.border = THIN_BORDER
...
def create_awareness_tracking_sheet(ws, styles):
    """Create the Awareness Tracking sheet - user acknowledgment tracking."""
    ...
    ws["A2"] = "Track AUP acknowledgment rates by department and monitor awareness campaign effectiveness"
    ws["A2"].font = Font(name="Calibri", size=10, italic=True)
    ws["A2"].alignment = Alignment(horizontal="left", vertical="center")
    ...
    for col_idx, value in enumerate(sample_data, start=1):
        ...
        cell.border = THIN_BORDER
        cell.alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
    # Ack% formula for sample row
    ws.cell(row=4, column=5).value = '=IF(B4>0,C4/B4,"")'
    ws.cell(row=4, column=5).fill = sample_fill
    ws.cell(row=4, column=5).border = THIN_BORDER
    ws.cell(row=4, column=5).number_format = '0.0%'

    # Pre-populate common departments (rows 5-15)
    departments = [
    ...
    grey_fill = PatternFill(start_color="F2F2F2", end_color="F2F2F2", fill_type="solid")
    for row_idx, dept in enumerate(departments, start=5):
        ws.cell(row=row_idx, column=1, value=dept).fill = grey_fill
        ws.cell(row=row_idx, column=1).border = THIN_BORDER
        ...
        # Acknowledgment % formula (col E = col 5)
        ws.cell(row=row_idx, column=5).value = f'=IF(B{row_idx}>0,C{row_idx}/B{row_idx},"")'
        ws.cell(row=row_idx, column=5).fill = styles["input_cell"]["fill"]
    ...
    # Empty FFFFCC rows (50 empty rows after sample + pre-pop: rows 16-65)
    for r in range(len(departments) + 5, 66):
        for c in range(1, 11):
            cell = ws.cell(row=r, column=c)
            cell.fill = styles["input_cell"]["fill"]
            cell.border = THIN_BORDER
            cell.alignment = styles["input_cell"]["alignment"]
        ws.cell(row=r, column=5).value = f'=IF(B{r}>0,C{r}/B{r},"")'
        ws.cell(row=r, column=5).fill = styles["input_cell"]["fill"]
...
def create_communication_matrix_sheet(ws, styles):
    """Create the Communication Matrix sheet - policy dissemination assessment."""
    ws.merge_cells("A1:I1")
    ...
    # Row 2: Subtitle
    ws.merge_cells("A2:I2")
    ws["A2"] = "Assess effectiveness of AUP communication channels and training delivery"
    ws["A2"].font = Font(name="Calibri", size=10, italic=True)
    ws["A2"].alignment = Alignment(horizontal="left", vertical="center")
    ...
    # Pre-populate communication channels (rows 5-12)
    channels = [
    ...
    grey_fill = PatternFill(start_color="F2F2F2", end_color="F2F2F2", fill_type="solid")
    for row_idx, (channel, audience, frequency) in enumerate(channels, start=5):
        ws.cell(row=row_idx, column=1, value=channel).fill = grey_fill
        ws.cell(row=row_idx, column=1).border = THIN_BORDER
        ws.cell(row=row_idx, column=2, value=audience).fill = grey_fill
        ws.cell(row=row_idx, column=2).border = THIN_BORDER
        ws.cell(row=row_idx, column=3, value=frequency).fill = grey_fill
        ws.cell(row=row_idx, column=3).border = THIN_BORDER
    ...
    # Empty FFFFCC rows (50 empty rows after sample + pre-pop: rows 13-62)
    for r in range(len(channels) + 5, 63):
        for c in range(1, 10):
            cell = ws.cell(row=r, column=c)
            cell.fill = styles["input_cell"]["fill"]
            cell.border = THIN_BORDER
...
def create_summary_dashboard_sheet(ws):
    """Create Gold Standard Summary Dashboard for ISMS-IMP-A.5.10-11.S1."""
    ...
    ws.merge_cells("A1:G1")
    ws["A1"] = "ACCEPTABLE USE POLICY ASSESSMENT \u2014 SUMMARY DASHBOARD"
    ...
    # TABLE 1 column headers (Row 5) — D9D9D9 fill (GS-SD-016 for TABLE 2; TABLE 1 always D9D9D9)
    ...
    # TABLE 1 data rows (rows 6-7): 2 assessment areas for S1
    # Data rows post-fix: sample at row 4, data rows 5-54
    area_configs = [
        # (Area Name, Total formula, Compliant formula, Partial formula, NonCompliant formula, NA formula)
        # Policy Assessment: sample row 4, 5 pre-pop rows 5-9, 50 empty FFFFCC rows 10-59
        (
            "AUP Policy Requirements",
            "=COUNTA('Policy Assessment'!A5:A59)",
            "=COUNTIF('Policy Assessment'!D5:D59,\"Yes\")",
            "=COUNTIF('Policy Assessment'!D5:D59,\"Partial\")",
            "=COUNTIF('Policy Assessment'!D5:D59,\"No\")",
            "=COUNTIF('Policy Assessment'!D5:D59,\"N/A\")",
        ),
        # Asset Coverage: sample row 4, 5 pre-pop rows 5-9, 50 empty FFFFCC rows 10-59
        (
            "Asset Category Coverage",
            "=COUNTA('Asset Coverage'!A5:A59)",
            "=COUNTIF('Asset Coverage'!C5:C59,\"Yes\")",
            "=COUNTIF('Asset Coverage'!C5:C59,\"Partial\")",
            "=COUNTIF('Asset Coverage'!C5:C59,\"No\")",
            "=0",
        ),
    ]
    ...
    # TABLE 2 header row (GS-SD-016: must be D9D9D9, not 4472C4)
    ...
    # TABLE 2 metrics (white fill, 000000 font — GS standard)
    # Ranges cover full data extent: sample row 4, pre-pop rows 5-N, empty FFFFCC rows N+1 to end
    # Policy Assessment: rows 5-59 | Asset Coverage: rows 5-59
    # Awareness Tracking: rows 5-65 | Communication Matrix: rows 5-62
    metrics = [
        # AUP Policy Requirements
        ("Total AUP Requirements Assessed", "=COUNTA('Policy Assessment'!A5:A59)"),
        ("Requirements Fully Addressed", "=COUNTIF('Policy Assessment'!D5:D59,\"Yes\")"),
        ("Requirements Partially Addressed", "=COUNTIF('Policy Assessment'!D5:D59,\"Partial\")"),
        ("Requirements with Gaps (No)", "=COUNTIF('Policy Assessment'!D5:D59,\"No\")"),
        ("Policy Sections with Clarity Issues", "=COUNTIF('Policy Assessment'!F5:F59,\"Unclear\")"),
        # Asset Coverage
        ("Asset Categories Assessed", "=COUNTA('Asset Coverage'!A5:A59)"),
        ("Asset Categories Fully Covered", "=COUNTIF('Asset Coverage'!C5:C59,\"Yes\")"),
        ("Asset Categories Partially Covered", "=COUNTIF('Asset Coverage'!C5:C59,\"Partial\")"),
        ("Asset Categories NOT Covered", "=COUNTIF('Asset Coverage'!C5:C59,\"No\")"),
        # User Awareness
        ("Departments / Groups Tracked", "=COUNTA('Awareness Tracking'!A5:A65)"),
        ("Departments with HR Escalation", "=COUNTIF('Awareness Tracking'!I5:I65,\"HR Escalation\")"),
        ("Departments with Manager Notification", "=COUNTIF('Awareness Tracking'!I5:I65,\"Manager Notified\")"),
        # Communication
        ("Communication Channels Assessed", "=COUNTA('Communication Matrix'!A5:A62)"),
        ("Channels Rated Highly Effective", "=COUNTIF('Communication Matrix'!F5:F62,\"Highly Effective\")"),
        ("Channels Needing Improvement", "=COUNTIF('Communication Matrix'!F5:F62,\"Needs Improvement\")+COUNTIF('Communication Matrix'!F5:F62,\"Ineffective\")"),
    ]
    ...
    # TABLE 3 critical findings — ranges match extended sheet data rows
    findings = [
        ("AUP Policy Completeness", "Policy requirements with unresolved gaps", "=COUNTIF('Policy Assessment'!D5:D59,\"No\")", "Critical", "Immediate"),
        ("AUP Policy Completeness", "Requirements in remediation (gap closure in progress)", "=COUNTIF('Policy Assessment'!D5:D59,\"Partial\")", "High", "Monitor"),
        ("AUP Policy Completeness", "Policy sections with unclear language", "=COUNTIF('Policy Assessment'!F5:F59,\"Unclear\")", "High", "Rewrite"),
        ("Asset Coverage", "Asset categories not covered in AUP", "=COUNTIF('Asset Coverage'!C5:C59,\"No\")", "Critical", "Extend AUP"),
        ("Asset Coverage", "Asset categories with only partial AUP coverage", "=COUNTIF('Asset Coverage'!C5:C59,\"Partial\")", "High", "Complete coverage"),
        ("User Awareness", "Departments requiring HR escalation", "=COUNTIF('Awareness Tracking'!I5:I65,\"HR Escalation\")", "High", "Enforce"),
        ("Communication", "Communication channels rated ineffective", "=COUNTIF('Communication Matrix'!F5:F62,\"Ineffective\")", "Medium", "Redesign"),
    ]
...
def create_evidence_register(ws):
    """Create the Evidence Register sheet for audit traceability."""
    ...
    sample_data = [
        "EV-001",
        "Policy Document",
        "Acceptable Use Policy v2.3 — signed and approved",
        "REQ-001",
        "2026-01-15",
...
def main() -> int:
    ...
    try:
        ...
        logger.info("[2/8] Creating Policy Assessment sheet...")
        create_policy_assessment_sheet(wb["Policy Assessment"], styles)

        logger.info("[3/8] Creating Asset Coverage sheet...")
        create_asset_coverage_sheet(wb["Asset Coverage"], styles)

        logger.info("[4/8] Creating Awareness Tracking sheet...")
        create_awareness_tracking_sheet(wb["Awareness Tracking"], styles)

        logger.info("[5/8] Creating Communication Matrix sheet...")
        create_communication_matrix_sheet(wb["Communication Matrix"], styles)
