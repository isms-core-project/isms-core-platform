...
# =============================================================================
"""
ISMS-A.5.9 Asset Inventory - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('A.5.9.1', 'A_5_9_1', 'Asset Discovery'),
    ('A.5.9.2', 'A_5_9_2', 'Inventory Maintenance'),
    ('A.5.9.3', 'A_5_9_3', 'Quality Compliance'),
    ('A.5.9.4', 'A_5_9_4', 'Owner Accountability'),
]

DASHBOARD_PATTERNS = ['A.5.9.5', 'A_5_9_5', 'Dashboard']
...
def extract_gaps_from_workbook(filepath, assessment_area, gap_counter_start=1):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'issue', 'compliance']):
                ws = wb[sheet_name]
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    if status or gap_desc:
                        ...
                        gaps.append([f'GAP-{assessment_area[:3].upper()}-{gap_counter:03d}',
                            assessment_area, os.path.basename(filepath),
                            row_values[0] if row_values[0] else 'Asset',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.5.9 Asset Inventory - Dashboard Consolidation")
    print("=" * 80)
