...
Assessment Domain 4 of 4: Ongoing Governance & Risk Management
...
**Purpose:**
Enables systematic assessment of Ongoing Governance & Risk Management under ISO 27001:2022 Controls A.5.19-A.5.23. Supports evidence-based evaluation of cloud service security posture, vendor compliance, and governance effectiveness.
...
Advanced Usage:
    ...
    python3 generate_a523_4_governance.py --output /path/to/dir

    # Generate with specific date suffix
    python3 generate_a523_4_governance.py --date 20250115

Output:
    File: ISMS-IMP-A.5.23.S4_Ongoing_Governance_&_Risk_Management_YYYYMMDD.xlsx
...
Version 1.0 - [Date to be set]
    - Initial release
    - Implements full assessment framework per ISMS-IMP-A.5.23.S4 specification
...
def create_workbook() -> Workbook:
    ...
    sheets = [
        ...
        "Evidence Register",             # Renumbered from 8
        "Summary Dashboard",            # Renumbered from 9
        "Approval Sign-Off",            # Renumbered from 10
    ]
    for name in sheets:
...
def get_checklists() -> dict:
    ...
    return {
        "access_review": [
            ...
            "Shared account usage minimised and justified",
        ...
        "incident_mgmt": [
            ...
            "DORA-INC-03: ICT incident root causes analysed and documented",
...
def create_instructions_sheet(ws):
    ...
    _instructions = ['1. Complete each assessment sheet for all cloud services in scope.', '2. Use dropdown menus for standardised entries.', '3. Fill in yellow-highlighted cells with your service-specific information.', '4. Complete regulatory columns for DORA/NIS2/AI Act/CLOUD Act as applicable.', '5. Provide evidence location for each assessment entry.', '6. Complete Jurisdictional Risk sheet for all US-nexus providers.', '7. Document DORA ICT risk monitoring frequency if in scope.', '8. Classify NIS2 incidents according to severity thresholds.', '9. Monitor AI systems according to AI Act risk classification.', '10. Assign regulatory risk owners for all identified risks.', '11. Summary Dashboard auto-calculates compliance statistics.', '12. Maintain Evidence Register for audit traceability.', '13. Obtain sign-offs in sequence: IT Ops, Compliance, DPO, CRO, CISO.']
    for _i, _line in enumerate(_instructions):
        ws[f"A{13 + _i}"] = _line

    _leg_row = 27

    _REG_LINES = ['This workbook includes regulatory compliance tracking for:', '• DORA (Digital Operational Resilience Act) — ICT risk management', '• NIS2 (Network and Information Security Directive 2) — Incident management', '• EU AI Act — AI system risk monitoring', '• US CLOUD Act — Jurisdictional risk assessment', '', 'Regulatory columns are included in each assessment sheet.', 'Complete only the regulatory sections applicable to your organisation.']
    _EVIDENCE = ['✓ Access review reports with sign-offs', '✓ Change management tickets and approvals', '✓ Incident reports and root cause analyses', '✓ BC/DR test results and RTO/RPO measurements', '✓ Vendor risk assessments and scorecards', '✓ Quarterly ICT risk register reviews (DORA)', '✓ Vendor concentration risk assessment', '✓ Business continuity test reports (annual minimum)', '✓ Incident notification records (NIS2 ≤24h threshold)', '✓ AI system monitoring logs (for high-risk AI)', '✓ Risk ownership assignments (signed by risk owners)', '✓ Jurisdictional risk assessment (for US-nexus providers)', '✓ Risk acceptance forms (signed by CISO/DPO/CRO)']
...
def create_assessment_sheet(ws, styles, sheet_name, config, base_cols, ext_cols, 
    ...
    for col_idx, (header, width) in enumerate(all_cols, start=1):
        ...
        cell = ws.cell(row=3, column=col_idx, value=header.replace("_", " "))
...
def create_7_exit_strategy_review(ws, styles):
    ...
    ws['A73'].fill = PatternFill(start_color="C6EFCE", end_color="C6EFCE", fill_type="solid")
...
def create_summary_dashboard_sheet(ws, styles):
    ...
    ws.merge_cells(f"A{metrics_row}:G{metrics_row}")
    ...
    ws.merge_cells(f"A{health_row}:G{health_row}")
...
def create_approval_sheet(ws):
    ...
    summary_fields = [
        ...
        ("Assessment Period:", ""),
        ("Overall Compliance Rating:", "=IFERROR(AVERAGE(\'Summary Dashboard\'!G5:G9),\"\")")  ,
        ("Assessment Status:", ""),
        ("Assessed By:", ""),
    ]
    row = 4
...
def main():
    ...
    logger.info('"Risk management is what you do when you realise \'YOLO\' isn\'t a valid')
