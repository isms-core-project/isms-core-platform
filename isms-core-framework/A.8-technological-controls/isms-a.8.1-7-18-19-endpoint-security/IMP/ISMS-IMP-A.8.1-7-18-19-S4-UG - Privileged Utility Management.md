...
**ISMS-IMP-A.8.1-7-18-19-S4-UG - Privileged Utility Management**
...
| 2 | Utility Inventory | Catalogue all privileged utility tools |
| 3 | Access Controls | Document access controls for privileged utilities |
| 4 | Approval Workflow | Track approval processes for privileged utility usage |
| 5 | Usage Audit | Record privileged utility usage and audit trail |
| 6 | MFA Compliance | Assess multi-factor authentication compliance for utilities |
| 7 | Quarterly Reviews | Document quarterly access reviews for privileged utilities |
| 8 | Capability Requirements | Define required privileged utility control capabilities |
| 9 | Gap Analysis | Identify gaps in privileged utility management |
...
**Assessment Name:** ISMS-IMP-A.8.1-7-18-19-S4 - Privileged Utility Management Assessment

#### What This Assessment Covers

This assessment evaluates [Organisation]'s controls over privileged utility programs - tools that can bypass or disable security controls. This answers:
...
- Network tools (Wireshark, port scanners, protocol analysers)
...
- Anomaly detection
- Incident response for unauthorised usage
...
7. DOCUMENT GAPS (Gap Analysis sheet)
...
5. Review incident response for unauthorised usage
...
- Note: Distinguish authorised (corporate-approved) vs. unauthorised remote access
...
- Forensic tools (FTK, EnCase - authorised security use only)
...
HIGH: Remote access (unauthorised), debugging tools, password tools
MEDIUM: System admin tools (Registry, Services), network analysers
...
- Alerting configured? (anomaly detection, unauthorised usage)
...
- Kernel debuggers (if unauthorised)
...
- Authorised use? (Security team only, or prohibited)
...
MEDIUM: Can be used maliciously but has legitimate admin uses
  ...
  - Examples: BCDEdit, kernel debuggers on authorised admin workstations

```

**Step 3: Document Authorised Use Cases**
...
**Requirements for Authorised Use:**

- Explicitly approved by CISO
- Used only on authorised endpoints (security lab, isolated networks)
...
- Authorised use not restricted (security tools on production)
...
- [ ] Authorised use cases documented
...
#### Purpose
Document all gaps in privileged utility controls.
...
Document gaps from assessment sheets, assign risk, and track remediation.
...
6. **Security Bypass** - Detection configurations, authorised use approvals
