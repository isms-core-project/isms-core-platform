...
- Network traffic optimisation and capacity planning (covered under network management policies)
...
| **A.8.8** | Vulnerability Management | Monitoring identifies exploitation attempts; prioritises patching based on observed attacks |
...
| **A.8.17** | Clock Synchronisation | Accurate timestamps essential for event correlation and timeline reconstruction |
...
- Virtualisation platforms (VM creation/deletion, resource allocation, migration events)
...
**System Utilisation Baselines**:

- CPU utilisation (mean, median, standard deviation, 95th percentile) during normal and peak times
...
- Compress archived data to optimise storage costs
...
- **Anonymised/Pseudonymised**: Where feasible for privacy protection (while maintaining forensic value)
...
- Monitoring scope determined through risk assessment (critical assets prioritised for monitoring)
...
- Tuning Playbook (procedures for alert tuning and optimisation)
...
| **ISMS-POL-A.8.8** | Vulnerability Management - monitoring identifies exploitation, prioritises patching |
...
Centralised platform for collecting, correlating, analyzing, and storing security-relevant logs and events from multiple sources. SIEM provides real-time alerting, forensic investigation, and compliance reporting.
...
|---------------------|------------------|------------------|-------------------|
| **SIEM** | Centralised log management, correlation, alerting | Multi-source ingestion, real-time correlation, compliance reporting, forensics | Enterprise-wide, centralised |
...
- Playbook execution (standardised response procedures)
...
- Basic logging enabled, limited centralisation
...
- Centralised log collection (SIEM or equivalent)
...
- Metrics-driven optimisation (MTTD, MTTR, detection rates tracked)
...
**Purpose**: Standardised template for documenting system, user, and application baselines per policy Section 2.2.
...
- [ ] System Utilisation (CPU, memory, disk, network)
