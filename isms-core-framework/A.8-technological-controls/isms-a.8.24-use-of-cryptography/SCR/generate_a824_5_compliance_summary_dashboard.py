...
"""
================================================================================
ISMS-IMP-A.8.24.5 - Compliance Dashboard Consolidation Generator
================================================================================

ISO/IEC 27001:2022 Control A.8.24: Use of Cryptography
Consolidated Compliance Dashboard: Executive Summary & Audit Readiness
...
your organization's specific cryptographic infrastructure assessment structure
and reporting requirements.

CRITICAL: This consolidation script MUST be customized based on the ACTUAL
...
3. Scoring algorithms - ADAPT TO YOUR RISK PROFILE
4. Compliance thresholds - BASED ON YOUR REQUIREMENTS
5. Executive summary formatting - ALIGN WITH YOUR GOVERNANCE

MANDATORY STEPS BEFORE USING THIS SCRIPT:
1. Generate all four A.8.24 assessment workbooks (Domains 1-4)
2. Document the actual sheet structure of each workbook
3. Update WORKBOOK_SCHEMAS dictionary with real structure
4. Validate schema against actual files before consolidation
...
This script consolidates data from all four cryptographic assessment domains
into a unified compliance dashboard for executive oversight and audit readiness.

**Purpose:**
Provides centralized visibility of cryptographic control implementation status
across all domains, enabling evidence-based decision-making and demonstrating
ISO 27001:2022 Control A.8.24 compliance to auditors and management.

**Input Assessment Domains:**
1. Domain 1: Data Transmission Cryptography (A.8.24.1)
   - TLS/SSL, VPN, file transfer, email, database connections
   
2. Domain 2: Data Storage Cryptography (A.8.24.2)
   - Database encryption, file system, object storage, backups, endpoints
   
3. Domain 3: Authentication Cryptography (A.8.24.3)
   - Password hashing, digital signatures, certificates, tokens, MFA
   
4. Domain 4: Key Management (A.8.24.4)
   - Key lifecycle, rotation, storage, access control, destruction

**Generated Dashboard Structure:**
1. Executive Summary - High-level compliance status and key metrics
2. Compliance Overview - Domain-by-domain compliance percentages
3. Data Transmission Status - Consolidated domain 1 findings
4. Data Storage Status - Consolidated domain 2 findings
5. Authentication Status - Consolidated domain 3 findings
6. Key Management Status - Consolidated domain 4 findings
7. Gap Analysis Summary - Critical gaps requiring immediate attention
8. Remediation Tracker - Action items with owners and deadlines
9. Trend Analysis - Historical compliance tracking (if available)
10. Audit Evidence Index - Cross-reference to supporting documentation
11. Risk Summary - Residual cryptographic risks
12. Approval & Sign-Off - Executive and CISO approval workflow

**Key Features:**
- Automated data extraction from domain assessment workbooks
- Intelligent scoring and weighting by criticality
- Visual compliance status indicators (traffic light system)
- Gap prioritization based on risk and regulatory requirements
- Executive-friendly summary with actionable insights
- Audit trail linking dashboard to source assessments
- Trend tracking for compliance improvement monitoring
- Export-ready formats for board reporting
...
Output: ISMS_A_8_24_5_Compliance_Dashboard_YYYYMMDD.xlsx
...
Dependencies:
    ...
    - os (standard library - file path handling)

Input Files Required:
    All four domain assessment workbooks must exist in the same directory
    or in paths specified via command-line arguments.

--------------------------------------------------------------------------------
USAGE
--------------------------------------------------------------------------------

Basic Usage (all workbooks in current directory):
...
Advanced Usage:
    ...
    python3 generate_a824_5_compliance_summary_dashboard.py --dry-run

Output:
    File: ISMS_A_8_24_5_Compliance_Dashboard_YYYYMMDD.xlsx
    Location: Current directory (or specified output path)

Validation:
    Script performs pre-consolidation validation:
    - Checks existence of all four input workbooks
    - Validates workbook structure against schema
    - Verifies required sheets and columns are present
    - Reports missing or malformed data
    
    Use --dry-run to validate without generating output.

Post-Generation Steps:
    1. Review consolidated compliance percentages for accuracy
    2. Validate gap analysis prioritization with security team
    3. Assign remediation action owners and deadlines
    4. Present executive summary to CISO and management
    5. Use for Stage 1/Stage 2 audit evidence
    6. Update quarterly or after remediation completion
    7. Track compliance trends over time
...
Assessment Domain:    5 (Consolidation) - Dashboard of Domains 1-4
...
Version 1.0 - [Date to be set]
    - Initial release
    - Implements full consolidation framework per ISMS-IMP-A.8.24.5 specification
    - Supports comprehensive cross-domain cryptography compliance reporting
    - Provides executive summary and audit-ready evidence index
...
This script MUST be customized to match your actual assessment workbook
structures. DO NOT run this script without first:
1. Generating all four domain assessment workbooks
2. Documenting their actual sheet names and column structures
3. Updating the WORKBOOK_SCHEMAS dictionary accordingly
4. Running in --dry-run mode to validate before production use

Failure to validate schemas will result in incorrect consolidation or
script failure. This is NOT optional.

**Audit Considerations:**
This dashboard is primary evidence for ISO 27001:2022 Control A.8.24 compliance.
Ensure all source data is accurate and evidence links are valid. Auditors will
expect to trace dashboard findings back to source assessments and underlying
technical evidence.

**Data Protection:**
The consolidated dashboard contains highly sensitive summary information about
your organization's cryptographic security posture, including:
- Overall compliance gaps and weaknesses
- Unencrypted systems and vulnerable configurations
- Key management deficiencies
- Remediation priorities

Handle with MAXIMUM SECURITY in accordance with your organization's highest
data classification level. Consider encrypting the dashboard workbook.

**Maintenance:**
Regenerate dashboard:
- Quarterly: Regular compliance tracking and trend analysis
- After major remediation: Demonstrate compliance improvement
- Before audits: Prepare current evidence for auditor review
- Ad-hoc: When significant cryptographic changes occur

**Quality Assurance:**
Have cryptography SMEs and ISMS manager review consolidated dashboard before
presenting to executives or auditors. Incorrect compliance reporting can have
serious audit and risk management consequences.
...
- Government: Jurisdiction-specific cryptographic audit requirements

Customize dashboard sections to highlight regulatory-specific compliance.

**Executive Communication:**
Dashboard is designed for non-technical audiences. Use executive summary and
visual indicators to communicate security posture. Provide technical details
in supporting sheets for security team reference.

**Trend Tracking:**
Maintain historical dashboard versions to track compliance improvement over
time. Demonstrate continuous improvement to auditors and management. Archive
previous versions with appropriate version control.

**Integration Opportunities:**
Consider integrating dashboard data with:
- Security Information and Event Management (SIEM) systems
- Governance, Risk, and Compliance (GRC) platforms
- IT Service Management (ITSM) ticketing for remediation tracking
...
def create_executive_dashboard(ws, styles):
    ...
    ws["A1"] = (
        "ISMS-IMP-A.8.24.5 – Cryptography Compliance Summary Dashboard\n"
        "ISO/IEC 27001:2022 - Control A.8.24: Use of Cryptography - Executive Overview"
    )
    ...
    scorecard_metrics = [
        ("Overall Compliance Rate", '=SUMPRODUCT(VALUE(SUBSTITUTE(C26:C29,"%","")))/COUNTA(C26:C29)&"%"', "≥95%", ""),  # Handles text percentages from external links
        ("Critical Gaps", "=COUNTIFS('Gap Analysis'!I8:I207,\"Critical\",'Gap Analysis'!R8:R207,\"<>Closed\")", "0", ""),
        ("High-Risk Items", "=COUNTIFS('Risk Register'!N21:N120,\">=15\",'Risk Register'!Q21:Q120,\"<>Closed\")", "≤5", ""),  # FIX: Changed from N20:N119 to N21:N120 (avoid circular ref)
    ...
    for area_name, source_doc, total_row in assessment_areas:
        ...
        ws.cell(row=row, column=3, value=f"='[{source_doc}]Summary Dashboard'!G{total_row}")  # Overall compliance %
        ws.cell(row=row, column=4, value=f"='[{source_doc}]Summary Dashboard'!B{total_row}")  # Total items
        ws.cell(row=row, column=5, value=f"='[{source_doc}]Summary Dashboard'!C{total_row}")  # Compliant
        ws.cell(row=row, column=6, value=f"='[{source_doc}]Summary Dashboard'!D{total_row}")  # Partial
        ws.cell(row=row, column=7, value=f"='[{source_doc}]Summary Dashboard'!E{total_row}")  # Non-Compliant
    ...
    # Use SUMPRODUCT to handle text percentages from external links
    ...
    summary_fields = [
        ("Assessment Period:", ""),
        ("Overall Compliance Status:", '=IF(VALUE(SUBSTITUTE(C21,"%",""))/100>=0.95,"Excellent",IF(VALUE(SUBSTITUTE(C21,"%",""))/100>=0.90,"Good",IF(VALUE(SUBSTITUTE(C21,"%",""))/100>=0.80,"Adequate",IF(VALUE(SUBSTITUTE(C21,"%",""))/100>=0.70,"Needs Improvement","Critical"))))'),
        ("Security Posture:", '=IF(VALUE(SUBSTITUTE(C21,"%",""))/100>=0.90,"Strong",IF(VALUE(SUBSTITUTE(C21,"%",""))/100>=0.75,"Adequate","Weak"))'),
    ]
    ...
    ws[f"A{row}"].font = Font(bold=True)
    row += 1
    ws[f"A{row}"] = "Total Estimated Remediation Budget:"
    ws[f"A{row}"].font = Font(bold=True)
    ws[f"B{row}"] = "=SUM('Remediation Roadmap'!R37:R236)"
    ws[f"B{row}"].font = Font(color="0000FF")
    ...
    ws[f"B{row}"].number_format = '"CHF"#,##0.00'
    ws[f"B{row}"].border = styles["border"]
    approved_budget_row = row
    
    row += 1
    ws[f"A{row}"] = "Budget Gap:"
    ws[f"A{row}"].font = Font(bold=True)
    ws[f"B{row}"] = f"=B{approved_budget_row}-B{total_budget_row}"  # Approved - Estimated
    ws[f"B{row}"].font = Font(color="0000FF")
    ws[f"B{row}"].number_format = '"CHF"#,##0.00'
    ws[f"B{row}"].border = styles["border"]
    budget_gap_row = row
...
def create_gap_analysis(ws, styles):
    ...
    dv_source = DataValidation(
        type="list",
        formula1='"ISMS-IMP-A.8.24.1,ISMS-IMP-A.8.24.2,ISMS-IMP-A.8.24.3,ISMS-IMP-A.8.24.4"',
        allow_blank=False
    )
...
def create_remediation_roadmap(ws, styles):
    ...
    for metric, formula, target, status in summary_metrics:
        ...
        if "=" in str(formula):
            ...
            if "SUM" in str(formula):
                ws.cell(row=row, column=2).number_format = '"CHF"#,##0.00'
        else:
            ws.cell(row=row, column=2).fill = styles["input_cell"]["fill"]
    ...
    for r in range(row, row + 200):
        ...
        ws.cell(row=r, column=18).number_format = '"CHF"#,##0.00'  # R: Budget Required
        ws.cell(row=r, column=20).number_format = '"CHF"#,##0.00'  # T: Actual Cost
...
def create_kpis_metrics(ws, styles):
    ...
    ws["A1"] = (
        ...
        "Cryptography program effectiveness measurement"
    )
    ws["A1"].font = styles["header"]["font"]
    ws["A1"].fill = styles["header"]["fill"]
    ws["A1"].alignment = styles["header"]["alignment"]
    ...
    compliance_kpis = [
        ("Overall Cryptography Compliance Rate", "='Executive Dashboard'!C29", "≥95%", "90%"),
...
def create_action_items(ws, styles):
    ...
    ws["A1"] = (
        ...
        "Centralised tracking of all outstanding actions"
    )
    ws["A1"].font = styles["header"]["font"]
    ws["A1"].fill = styles["header"]["fill"]
    ws["A1"].alignment = styles["header"]["alignment"]
...
def create_approval_signoff(ws, styles):
    ...
    summary_fields = [
        ("Document:", "ISMS-IMP-A.8.24.5 - Compliance Summary Dashboard"),
        ("Assessment Period:", ""),
        ("Portfolio Compliance:", "='Executive Dashboard'!C29"),
        ("Dashboard Status:", ""),
    ]
...
def main() -> int:
    ...
    try:
        logger.info("=" * 80)
        logger.info("ISMS-IMP-A.8.24.5 - Compliance Summary Dashboard Generator")
        ...
        logger.info("[1/9] Creating Executive Dashboard sheet...")
        ...
        filename = f"ISMS-IMP-A.8.24.5_Compliance_Summary_Dashboard_{datetime.now().strftime('%Y%m%d')}.xlsx"
        wb.save(filename)
        ...
        logger.info("=" * 80)
        logger.info("Workbook Structure (9 Sheets):")
        ...
        logger.info("External Links Configuration:")
        ...
        logger.info("IMPORTANT SETUP STEPS:")
        logger.info("  1. Run normalization script FIRST:")
        logger.info("     python3 normalize_assessment_files.py")
        logger.info("")
        ...
        logger.info("")
        logger.info("=" * 80)
        logger.info("ALL 5 CRYPTOGRAPHY ASSESSMENT TOOLS COMPLETE!")
        logger.info("You now have:")
        logger.info("  - 4 Assessment workbooks (Data Transmission, Storage, Auth, Key Mgmt)")
