...
"""
================================================================================
ISMS-IMP-A.8.2-3-5.S3 - Privileged Account Inventory Assessment Excel Generator
================================================================================
...
your organization's specific privileged access management approach, admin
tiering model, and PAM deployment.

Key customization areas:
1. Admin tier definitions (Tier 0/1/2 per your tiering model)
2. PAM solution integration (CyberArk, HashiCorp per your deployment)
3. Account types (named, shared, service per your governance)
4. Rotation requirements (based on your security policy)
5. PAW deployment criteria (aligned with your zero trust strategy)
...
Output:
    ISMS-IMP-A.8.2-3-5.S3_Privileged_Accounts_Assessment_YYYYMMDD.xlsx
"""
# =============================================================================
...
def populate_instructions(wb, styles):
    ...
    # Title
    ws['A1'] = f"{DOCUMENT_ID}  -  Privileged Accounts Inventory & Admin Tiering Assessment\n{CONTROL_REF}"
    ...
    tier0_info = [
        f"{BULLET} Systems: Domain Controllers, Microsoft Entra ID (formerly Azure AD) tenant, AWS Organisation root, PKI CA, SIEM, PAM vault, Backup infrastructure",
        f"{BULLET} Accounts: Domain Admins, Enterprise Admins, Global Administrator, AWS root, Security admins, PAM admins",
        f"{BULLET} Access Level: Can access EVERYTHING - highest privilege in organisation",
    ...
    ws[f'A{row}'].fill = PatternFill(patternType='solid', fgColor='C6EFCE')
    tier2_info = [
...
def populate_tiering_matrix(wb, styles):
    ...
    tier0_systems = [
        ...
        ("AWS Organisation Root", "Tier 0", "Critical", "Root user MFA, breakglass only", ""),
...
def populate_remaining_sheets(wb, styles):
    ...
    for col_idx, header in enumerate(roster_headers, start=1):
        cell = ws_roster.cell(row=header_row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(pam_headers, start=1):
        cell = ws_pam.cell(row=header_row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(token_headers, start=1):
        cell = ws_tokens.cell(row=header_row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(rotation_headers, start=1):
        cell = ws_rotation.cell(row=header_row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
...
def generate_workbook():
    ...
    logger.info("║  Workbook 3: Privileged Accounts & Admin Tiering Assessment    ║")
    logger.info("║                                                                ║")
    logger.info("║  'Privileged access = keys to the kingdom'                     ║")
    logger.info("╚════════════════════════════════════════════════════════════════╝")
    logger.info("Creating workbook structure...")
    wb = create_workbook()
    styles = setup_styles()
    logger.info("Populating Instructions & Legend (Admin Tiering Model)...")
    populate_instructions(wb, styles)
    logger.info("Populating Privileged Account Inventory (150 accounts)...")
    populate_account_inventory(wb, styles)
    logger.info("Populating Admin Tiering Classification Matrix...")
    populate_tiering_matrix(wb, styles)
    logger.info("Populating remaining sheets...")
    populate_remaining_sheets(wb, styles)
    # Save workbook
    filename = f"ISMS-IMP-A.8.2-3-5.S3_Privileged_Accounts_{GENERATED_TIMESTAMP}.xlsx"
    wb.save(filename)
    logger.info("=" * 70)
    logger.info("{CHECK} Workbook generated successfully: {filename}")
    logger.info("Next Steps:")
    logger.info("  1. Discover all privileged accounts (AD groups, cloud admins, DBAs)")
    logger.info("  2. Classify accounts into Tier 0/1/2 in Admin_Tiering_Matrix")
    logger.info("  3. Complete Privileged_Account_Inventory with tier assignments")
    logger.info("  4. Verify PAM_Vault_Coverage (target: 100% shared accounts)")
    logger.info("  5. Track MFA_Hardware_Tokens for Tier 0 admins")
    logger.info("  6. Monitor Credential_Rotation_Status")
    logger.info("  7. Conduct quarterly Access_Review_Results")
    logger.info("  8. Investigate any Tier_Isolation_Compliance violations")
    ...
    logger.info("  • Tier 0: Hardware MFA tokens MANDATORY, PAWs required")
    logger.info("  • All Privileged: 100% MFA coverage (no exceptions)")
    logger.info("  • Shared Accounts: 100% in PAM vault")
    logger.info("  • Tier Isolation: ZERO cross-tier violations")
    logger.info("  • Access Reviews: Quarterly MANDATORY")
    return filename
