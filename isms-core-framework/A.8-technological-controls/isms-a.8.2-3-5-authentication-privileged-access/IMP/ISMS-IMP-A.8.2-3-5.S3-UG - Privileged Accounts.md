<!-- ISMS-CORE:IMP:ISMS-IMP-A.8.2-3-5.S3-UG:framework:UG:a.8.2-3-5 -->
**ISMS-IMP-A.8.2-3-5.S3-UG - Privileged Account Inventory**
...
| **Document ID** | ISMS-IMP-A.8.2-3-5.S3-UG |
...
This is the **User Completion Guide**. The companion Technical Specification is documented in ISMS-IMP-A.8.2-3-5.S3-TG.
...
| 2 | Privileged Account Inventory | Catalogue all privileged accounts across all systems |
| 3 | Admin Tiering Matrix | Document the admin tiering model and tier assignments |
| 4 | Privileged User Roster | Maintain the roster of privileged users and their roles |
| 5 | PAM Vault Coverage | Track privileged access management vault coverage |
| 6 | MFA Hardware Tokens | Document hardware token deployment for privileged accounts |
| 7 | Credential Rotation Status | Track credential rotation compliance |
| 8 | Access Review Results | Record results of periodic privileged access reviews |
| 9 | Tier Isolation Compliance | Assess compliance with tier isolation requirements |
...
- [ ] Entra ID privileged roles (Global Administrator, Privileged Role Administrator, etc.)
- [ ] AWS IAM privileged users (AdministratorAccess policy, root account users)
- [ ] GCP IAM privileged users (Owner, Editor at organisation/project level)
...
- [ ] Entra ID Global Reader (read privileged role assignments)
...
# Understanding Admin Tiering Model (CRITICAL SECTION)
...
**Scope:** Controls that can access or modify EVERYTHING in the organisation

**Examples:**

- **Active Directory**: Domain Admins, Enterprise Admins, Schema Admins, BUILTIN\Administrators on domain controllers
- **Entra ID**: Global Administrator, Privileged Role Administrator, Cloud Application Administrator (can reset Global Admin passwords)
- **AWS**: Root account users, users with AdministratorAccess at organisation level
- **GCP**: Organisation Owners, Folder/Project Owners at organisation level
...
- Can compromise ANY system in the organisation
...
5. COLLECT EVIDENCE
   ...
   → Entra ID privileged role assignments
...
**Entra ID Privileged Roles (Microsoft Graph PowerShell):**
```powershell
...
Get-MgDirectoryRole -Filter "displayName eq 'Global Administrator'" | ForEach-Object {
    Get-MgDirectoryRoleMember -DirectoryRoleId $_.Id | Select-Object @{N='DisplayName';E={$_.AdditionalProperties.displayName}}, @{N='UserPrincipalName';E={$_.AdditionalProperties.userPrincipalName}}
}

# Get all privileged role assignments
Get-MgDirectoryRole | ForEach-Object {
    $role = $_
    Get-MgDirectoryRoleMember -DirectoryRoleId $role.Id | Select-Object @{N='Role';E={$role.DisplayName}}, @{N='DisplayName';E={$_.AdditionalProperties.displayName}}, @{N='UserPrincipalName';E={$_.AdditionalProperties.userPrincipalName}}
}
```
...
For privileged users requiring hardware token distribution:

1. **Token Assignment** (Columns A-E):
   - User Name: Privileged account owner
   - Admin Tier: Tier 0, Tier 1, Tier 2
   - Hardware Token Type: YubiKey 5, YubiKey Security Key, Titan Key, etc.
   - Token Serial Number: Unique identifier
   - Issuance Date: When token was provided

2. **Token Status** (Columns F-H):
   - Token Status: Active, Inactive, Compromised, Replaced, Lost
   - Last Authentication Date: Last successful MFA with token
   - Replacement Schedule: When token replacement due (3-5 year lifecycle)
...
1. **Account Rotation** (Columns A-E):
   - Account ID: From Sheet 2
   - Account Type: Named, Shared, Service, Break-Glass
   - Account Tier: Tier 0, Tier 1, Tier 2
   - Rotation Frequency Policy: Daily, Weekly, Monthly, Quarterly, Manual
   - Rotation Method: Automated (PAM), Manual, Hybrid

2. **Rotation Tracking** (Columns F-H):
   - Last Rotation Date: Last password/credential change
   - Next Scheduled Rotation: Due date
   - Days Since Rotation: Auto-calculated (should not exceed policy)
...
For quarterly privileged access reviews:

1. **Review Details** (Columns A-D):
   - Review Period: Q1 2026, Q2 2026, etc.
   - Account Reviewed: Yes, No (account included in review?)
   - Review Date: When review was completed
   - Reviewer Name: Who performed the review

2. **Access Justification** (Columns E-H):
   - Business Justification: Current? (Is access still justified?)
   - Manager Approval: Valid? (Does manager still approve?)
   - Access Appropriate: Yes, No (privilege level appropriate?)
   - Findings: Compliant, Minor Issue, Major Issue

3. **Remediation** (Columns I-K):
   - Remediation Required: Yes, No
   - Remediation Type: Access Removal, Privilege Reduction, MFA Enforcement
...
- **Entra ID Privileged Role Assignments**: Screenshot or CSV export
...
**Step 11: Complete Evidence Register (Sheet 10)**

Document all collected evidence with links.

**Step 12: Approval & Sign-Off (Sheet 11)**
...
3. **Cloud IAM Evidence**:
   ...
   - GCP IAM bindings (organisation/project Owners)
...
- Azure Global Administrator = Tier 0 (can do anything in Entra ID tenant)
- AWS root account users = Tier 0
- GCP Organisation Owner = Tier 0
...
- [ ] All privileged accounts inventoried (AD, Entra ID, cloud IAM, applications, databases)
...
- [ ] Entra ID role assignments documented
...
## Understanding Privileged Access Scores
