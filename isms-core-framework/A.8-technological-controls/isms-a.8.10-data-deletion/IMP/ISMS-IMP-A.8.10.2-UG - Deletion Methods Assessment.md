<!-- ISMS-CORE:IMP:ISMS-IMP-A.8.10.2-UG:framework:UG:a.8.10.2 -->
...
| 2 | 2. Physical Storage Media | Assess deletion methods for physical storage media |
| 3 | 3. Database Systems | Evaluate database deletion and purging procedures |
| 4 | 4. Cloud Storage | Assess cloud storage deletion capabilities and verification |
| 5 | 5. File Systems & Backup Media | Review file system and backup media deletion procedures |
| 6 | 6. Deletion Verification Test | Document and track deletion verification testing results |
...
**Assessment Output:** Excel workbook with ~100-200 data points documenting deletion methods in use, NIST SP 800-88 Rev. 2 compliance status, verification test results, and remediation plans for ineffective deletion methods.
...
**NIST SP 800-88 Rev. 2 Context:**

NIST Special Publication 800-88 Rev. 2 "Guidelines for Media Sanitization" defines three sanitization methods:
...
- **NIST SP 800-88 Rev. 2 Compliance Assessment (2-3 hours):** Map methods to NIST categories, identify gaps
...
**Pro Tip:** If [Organisation] has never conducted deletion verification testing (forensic recovery attempts), allow additional time (8-12 hours) to plan and execute initial testing before completing this assessment.
...
- **Use Case:** Internal reuse of media within same organisation at same classification level
- **Examples:** 
...
- [ ] **Inventory all storage media types** in use at [Organisation] (HDD, SSD, cloud, tapes, etc.)
- [ ] **Review NIST SP 800-88 Rev. 2** documentation (at least executive summary and decision flowcharts)
...
4. Assign NIST SP 800-88 Rev. 2 Category in Column T (Clear, Purge, Destroy)
...
- **Clear:** Overwrite (for reuse within organisation)
- **Purge:** Degaussing
...
- **Best Practice:** DELETE + Log truncation + Database reorganisation + Backup purge
...
- **Reuse Within Organisation:** Overwrite (Clear category)
- **External Disposal:** Degaussing (Purge) or Physical Destruction (Destroy)
- **Retirement:** Most organisations prefer physical destruction for decommissioned tapes
...
- NIST SP 800-88 Rev. 2 compliance mapping
...
A: Standard OS delete (Windows Delete, Linux `rm`) only removes the directory entry. The actual data remains on the disk in unallocated space and is easily recoverable with free forensic tools like PhotoRec or Recuva. For Confidential/Restricted data, NIST SP 800-88 Rev. 2 requires Purge (degaussing, multi-pass overwrite) or Destroy (physical destruction).
...
- **NIST SP 800-88 Rev. 2:** 1 pass sufficient for modern HDDs (>15 GB), but acknowledges 3+ passes common
...
**For true deletion:** DELETE + log truncation + file reorganisation + backup purge.
...
- ❌ **"Confidential data using Clear methods only"** → Upgrade to Purge or Destroy per NIST SP 800-88 Rev. 2
...
- [ ] NIST SP 800-88 Rev. 2 categories correctly assigned (Clear/Purge/Destroy)
- [ ] SSD deletion methods appropriate (NOT standard overwrite)
- [ ] Database deletion includes transaction log management
...
2. **NIST SP 800-88 Rev. 2 Check:** Verify categories assigned correctly
