<!-- ISMS-CORE:IMP:ISMS-IMP-A.8.10.4-UG:framework:UG:a.8.10.4 -->
...
| Attribute | Value |
|-------|-------|
| **Document Title** | Verification & Evidence Assessment |
...
| 2 | Deletion Logging Assessment | Assess deletion audit trail infrastructure and completeness |
| 3 | Verification Testing Program | Document and track deletion verification testing |
| 4 | Evidence Repository Assessment | Assess the deletion evidence repository |
| 5 | Certificate Management | Manage deletion certificates and documentation |
| 6 | Audit Trail Completeness | Verify completeness of the deletion audit trail |
...
This assessment evaluates [Organisation]'s **capability to prove information deletion occurred** through systematic verification procedures and evidence management, addressing the fundamental challenge: *How do you prove you deleted something without keeping the thing you deleted?*
...
Organisations must prove deletion occurred **without retaining the deleted data itself**. This creates a fundamental challenge:

- ❌ **Wrong Approach:** Keep copies of deleted data to prove deletion → Violates data minimisation
...
- **EU GDPR (Article 5.1.f):** "Integrity and confidentiality" principle - organisations must demonstrate data processing security including deletion
- **EU GDPR (Article 17):** "Right to erasure" - organisations must provide evidence of deletion to data subjects on request
...
- [Organisation]'s deletion logging infrastructure (SIEM, audit logs, application logs)
- Forensic testing methodologies (NIST SP 800-88 Rev. 2, data remanence analysis)
...
**Pro Tip:** If [Organisation] has no formal verification testing program, the assessment will reveal this gap quickly (~2 hours). The remaining time should be spent documenting current ad-hoc verification practices and planning remediation.
...
- How [Organisation] currently logs deletion events (if at all)
...
- NIST SP 800-88 Rev. 2: Guidelines for Media Sanitization (Appendix A - Verification)
...
2. **Sheet 3 - Verification Testing Program**
   ...
   - Assess testing coverage against NIST SP 800-88 Rev. 2 requirements
...
**Parallel (For Mature Organisations):**
...
**Purpose:** Evaluate whether [Organisation] has comprehensive, tamper-evident deletion logs that can support audit trail reconstruction.
...
- This is a common finding for organisations starting A.8.10 implementation
...
**Purpose:** Evaluate whether [Organisation] performs forensic testing to verify deletion methods actually work.
...
- This is a **critical gap** that many organisations have
- Mark Gap Severity (Column D) as "High" - forensic testing is mandatory per NIST SP 800-88 Rev. 2
...
- A: Per NIST SP 800-88 Rev. 2 and ISMS-POL-A.8.10, Section 2.4 (Verification & Evidence Requirements):
...
1. ❌ **Confusing vendor certification with forensic testing**
   ...
   - Forensic testing requires [Organisation] or third-party lab to verify
...
3. ❌ **One-time testing years ago**
   ...
   - Annual re-testing is minimum per NIST SP 800-88 Rev. 2

## Sheet 4: Evidence Repository Assessment (13 Assessment Rows)

**Purpose:** Evaluate whether [Organisation] securely stores deletion evidence with proper access controls and retention.
...
- A: Certificate quality scoring rubric (in reference table):
  - **Excellent (5 points):** Certificate includes: vendor name, date, data type, deletion method, NIST category, authorised signature, unique certificate ID
...
**Pitfall:** Organisation freezes assessment because verification testing doesn't exist.
...
- Evidence must be organised, indexed, and accessible
...
| Level 3 | CISO / Audit Committee | Final approval, commit resources for remediation, authorise dashboard publication | Week 4 |
