<!-- ISMS-CORE:IMP:ISMS-IMP-A.8.10.3-UG:framework:UG:a.8.10.3 -->
...
| 2 | 2. Cloud Provider Deletion | Assess cloud provider deletion capabilities and policies |
| 3 | 3. SaaS Application Deletion | Evaluate SaaS application data deletion procedures |
| 4 | 4. Vendor Contract Assessment | Review vendor contract deletion obligations |
| 5 | 5. Subprocessor Mapping | Map subprocessor data deletion responsibilities |
| 6 | 6. Shadow IT Assessment | Identify and assess shadow IT deletion risks |
...
4. **Subprocessor Mapping** - Identify all subprocessors handling organisational data
5. **Shadow IT Assessment** - Unapproved cloud services with organisational data
...
**Extension to Third Parties:** When [Organisation] stores data with cloud providers or third-party vendors, deletion responsibility doesn't transfer entirely to the vendor. [Organisation] remains accountable for ensuring deletion occurs.
...
- **Shadow IT Risk:** Unapproved cloud services with organisational data = uncontrolled deletion
...
- [Organisation]'s cloud service inventory (AWS, Azure, GCP, SaaS applications)
...
- **Shadow IT Prohibition:** Unapproved cloud services with organisational data not permitted
...
Under GDPR, [Organisation] is typically the "controller" (determines purposes and means of processing). Cloud providers and SaaS vendors are typically "processors" (process data on behalf of controller).
...
> "Taking into account the nature of the processing, the processor shall assist the controller by appropriate technical and organisational measures, insofar as this is possible, for the fulfillment of the controller's obligation to respond to requests for exercising the data subject's rights..."
...
- [Organisation] cannot outsource GDPR compliance responsibility to vendors
- If vendor fails to delete data, [Organisation] is liable under GDPR
...
**Critical:** If [Organisation] lacks a complete cloud service inventory, allow additional time (20-30 hours) to conduct discovery before assessment.
...
**Objective:** Identify all subprocessors handling organisational data
...
A subprocessor is a third party engaged by the processor (your vendor) to process personal data on behalf of the controller (your organisation).
...
2. **Authorisation:** Controller must authorise subprocessor engagement (general or specific)
...
**Objective:** Identify unapproved cloud services containing organisational data
...
- **Acceptable:** Email confirmation from authorised vendor representative (saved to evidence register)
- **Insufficient:** Automated system confirmation with no human verification

**Audit Readiness:** Auditors prefer signed certificates, but email from authorised person (CFO, CTO, Legal) is typically acceptable.
...
A: GDPR allows "general authorisation" with notification:

- **General Authorisation:** Controller authorises vendor to use subprocessors, subject to notification
...
- Organisation deletes from AWS S3 immediately
...
**Pitfall:** Vendor engages new subprocessor, organisation unaware

**Scenario:**

- SaaS vendor sends email: "We're adding Acme Analytics as subprocessor"
- Email goes to procurement inbox, ignored
- Acme Analytics processes customer data without organisation's knowledge
- Acme Analytics has no deletion clause, data retained indefinitely
- **Result:** GDPR Article 28 violation (subprocessor not authorised, deletion unverified)
...
- Organisation contracts with niche SaaS vendor
- Contract clause: "Data deletion only permitted after contract termination"
- SaaS vendor performance degrades, organisation wants to switch vendors
...
- Organisation requests deletion, SaaS vendor deletes from application database
...
- [ ] Subprocessors authorised and disclosed
