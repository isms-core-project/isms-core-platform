<!-- ISMS-CORE:IMP:ISMS-IMP-A.8.12.3-UG:framework:UG:a.8.12.3 -->
...
| 2 | Channel Overview | Summary of all data egress channels and coverage status |
| 3 | Email Channel | DLP policy coverage for email data exfiltration |
| 4 | Web Cloud Channel | DLP coverage for web and cloud data exfiltration |
| 5 | Endpoint Channel | DLP coverage for endpoint data exfiltration |
| 6 | Network Channel | DLP coverage for network data exfiltration |
| 7 | Application Channel | DLP coverage for application data exfiltration |
| 8 | Mobile Channel | DLP coverage for mobile device data exfiltration |
| 9 | Coverage Metrics | Aggregate DLP channel coverage metrics |
| 10 | Gap Analysis | Identify channel coverage gaps and bypass paths |
...
This assessment evaluates [Organisation]'s **DLP channel coverage completeness** to ensure compliance with ISO/IEC 27001:2022 Control A.8.12 and eliminate data exfiltration bypass paths.
...
- **Swiss nDSG (Art. 8):** Requires appropriate organisational and technical measures against data loss
- **EU GDPR (Art. 32):** Technical measures must be appropriate to the risk (gaps = inadequate measures)
- **Industry Standards:** PCI DSS v4.0.1 (Req. 4), HIPAA (§164.312), SOC 2 all require comprehensive data egress protection
...
- [Organisation]'s complete data egress channel inventory (all ways data can leave organisation)
...
**Total Assessment Time:** 6-8 hours (depending on channel complexity and organisation size)
...
**Pro Tip:** For organisations with >500 employees or complex multi-site deployments, consider splitting by channel:
...
Organisations often deploy DLP on "obvious" channels (email, web) but miss "alternative" channels that attackers/insiders readily exploit.
...
- Organisation deployed email DLP (99% of sensitive data sent via email)
...
- [ ] Data flow diagrams (how data moves between systems and out of organisation)
...
- Complete inventory of data egress channels in [Organisation]
...
- List ALL data egress channels in organisation
...
**Assessment Question:** *"What are all the data egress channels in your organisation?"*

**How to Answer:**

- **Comprehensive inventory:** List EVERY way data can leave organisation (email, web, USB, print, mobile, cloud, etc.)
