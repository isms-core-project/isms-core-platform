...
def create_joiner_events_sheet(wb, styles):
    ...
    for i, user in enumerate(users):
        ...
        ws.cell(row=row, column=7).value = hire_date.strftime("%d.%m.%Y")
        ws.cell(row=row, column=8).value = provision_date.strftime("%d.%m.%Y")
...
def create_mover_events_sheet(wb, styles):
    ...
    for i, user in enumerate(users):
        ...
        ws.cell(row=row, column=8).value = effective_date.strftime("%d.%m.%Y")
        ws.cell(row=row, column=9).value = update_date.strftime("%d.%m.%Y")
...
def create_leaver_events_sheet(wb, styles):
    ...
    for i, user in enumerate(users):
        ...
        ws.cell(row=row, column=4).value = user["full_name"]
        ws.cell(row=row, column=5).value = user["department"]
        ws.cell(row=row, column=6).value = term_date.strftime("%d.%m.%Y")
        ws.cell(row=row, column=7).value = disable_date.strftime("%d.%m.%Y %H:%M")
...
def create_contractor_lifecycle_sheet(wb, styles):
    ...
    for contractor in contractors:
        ...
        ws.cell(row=row, column=7).value = end_date.strftime("%d.%m.%Y")
...
def create_orphaned_remediation_sheet(wb, styles):
    ...
    for i, user in enumerate(users):
        ...
        ws.cell(row=row, column=4).value = (datetime.now() - timedelta(days=random.randint(90, 365))).strftime("%d.%m.%Y")
        ws.cell(row=row, column=5).value = detection_date.strftime("%d.%m.%Y")
        ...
        ws.cell(row=row, column=9).value = completion_date.strftime("%d.%m.%Y") if completion_date else ""
...
def create_hr_integration_sheet(wb, styles):
    ...
    components = [
        ("Employee Data Feed", "Active", "API", "Real-time", (datetime.now() - timedelta(hours=1)).strftime("%d.%m.%Y %H:%M"), "None", "Automated employee sync"),
        ("Termination Feed", "Partial", "Manual", "Daily", (datetime.now() - timedelta(hours=24)).strftime("%d.%m.%Y %H:%M"), "Manual delays", "Automation pending"),
        ("Department Transfer Feed", "Not Implemented", "N/A", "N/A", "N/A", "No automated feed", "Requires manual process"),
        ("Contractor Data Feed", "Active", "CSV Import", "Weekly", (datetime.now() - timedelta(days=3)).strftime("%d.%m.%Y %H:%M"), "None", "Weekly contractor sync"),
        ("Authoritative Source Validation", "Active", "API", "Hourly", (datetime.now() - timedelta(minutes=30)).strftime("%d.%m.%Y %H:%M"), "None", "HR is authoritative source"),
    ]
...
def create_gap_analysis_sheet(wb, styles):
    ...
    gaps = [
        ("GAP-001", "Provisioning", "6 late provisioning events", "Medium", "6 events",
         ...
         (datetime.now() + timedelta(days=60)).strftime("%d.%m.%Y"), "Planned"),
        ("GAP-002", "Deprovisioning", "3 late deprovisioning events", "High", "3 events",
         ...
         (datetime.now() + timedelta(days=30)).strftime("%d.%m.%Y"), "In Progress"),
...
def main():
    ...
    logger.info("  2/10 Creating Joiner Events...")
    create_joiner_events_sheet(wb, styles)
    
    logger.info("  3/10 Creating Mover Events...")
    create_mover_events_sheet(wb, styles)
    
    logger.info("  4/10 Creating Leaver Events...")
    create_leaver_events_sheet(wb, styles)
    
    logger.info("  5/10 Creating Contractor Lifecycle...")
    create_contractor_lifecycle_sheet(wb, styles)
    
    logger.info("  6/10 Creating Orphaned Remediation...")
    create_orphaned_remediation_sheet(wb, styles)
    
    logger.info("  7/10 Creating Timeliness Metrics...")
    create_metrics_sheet(wb, styles)
    
    logger.info("  8/10 Creating HR Integration...")
    create_hr_integration_sheet(wb, styles)
    
    logger.info("  9/10 Creating Gap Analysis...")
    create_gap_analysis_sheet(wb, styles)
    
    logger.info("  10/10 Creating Evidence Register...")
    create_evidence_register_sheet(wb, styles)
    
    # Save workbook
    filename = f"ISMS-IMP-A.5.15-16-18.S5_Lifecycle_Compliance_Detailed_{GENERATED_TIMESTAMP}.xlsx"
    ...
    logger.info(f"📋 Lifecycle events: {JOINER_EVENTS + MOVER_EVENTS + LEAVER_EVENTS}")
    logger.info("="*80)
    logger.info("\nWorkbook Contents:")
    logger.info(f"  • Joiner Events ({JOINER_EVENTS} provisioning events)")
    logger.info(f"  • Mover Events ({MOVER_EVENTS} role change events)")
    logger.info(f"  • Leaver Events ({LEAVER_EVENTS} termination events)")
    logger.info(f"  • Contractor Lifecycle ({CONTRACTOR_COUNT} contractors)")
    logger.info(f"  • Orphaned Account Remediation ({ORPHANED_REMEDIATION} accounts)")
    logger.info("  • Timeliness Metrics & SLA Compliance")
    logger.info("  • HR System Integration Status")
