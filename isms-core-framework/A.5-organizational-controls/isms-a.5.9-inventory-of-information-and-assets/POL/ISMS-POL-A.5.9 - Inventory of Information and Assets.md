<!-- ISMS-CORE:POLICY:ISMS-POL-A.5.9:framework:GOV-POL:a.5.9 -->
...
| **Owner** | Accountable individual (for information assets, this is the "Data Owner" in GDPR terminology - the business accountable party) | Responsibility assignment | Owner acknowledgment |
| **Custodian** | Day-to-day manager (may differ from owner) - technical party managing infrastructure/systems | Operational responsibility | Custodian acknowledgment |
...
1. Escalate to appropriate management level within 5 business days of asset discovery
2. Document temporary custodian assignment (operational responsibility during determination period)
3. Set deadline for permanent owner determination:
   - **Initial determination deadline**: 30 calendar days
   - **Extension permitted**: Up to 90 calendar days with CISO approval if ownership requires cross-functional resolution (document justification and compensating controls: assigned custodian monitors asset, security incidents escalated immediately)
4. Assets unowned beyond 90 days require Executive Management approval as formal exception (Section 3.4)

Unowned asset escalations tracked in exception register. Target: ≥95% assets assigned permanent owners within 30 days, 100% within 90 days.
...
- Information Assets: 85% accuracy
- IT Infrastructure: 90% accuracy
- Physical Assets: 80% accuracy
- Personnel Assets: 95% accuracy

**Mature State Accuracy Targets** (within 12 months post-certification):

- Information Assets: 95% accuracy
- IT Infrastructure: 98% accuracy
- Physical Assets: 90% accuracy
- Personnel Assets: 100% accuracy

Accuracy improvement tracked quarterly. Sampling methodology defined in ISMS-IMP-A.5.9-3.
...
**Clarification**: Maximum staleness refers to **update triggers** (how quickly inventory is updated after a change event). Verification Schedule below refers to **proactive owner review** (periodic attestation). Critical assets require prompt updates when changes occur (daily) AND scheduled owner review to catch unnoticed drift.
...
**Integration Maturity Approach**: Integration requirements are **phased** based on organisational system maturity:

**Phase 1 - Initial Certification** (minimum viable integration):

- CMDB: If CMDB exists, quarterly manual export/reconciliation. If no CMDB, IT infrastructure tracked in dedicated asset database with quarterly validation against procurement records and network discovery scans.
- HRIS: Quarterly manual reconciliation of personnel assets (critical roles/competencies) against HR records.
- Procurement: Annual review of procurement records for new asset additions; new assets added to inventory within 30 days of procurement approval.
- Document Management: Information assets identified via document repository metadata exports (quarterly).

**Phase 2 - Mature State** (within 18 months post-certification):

- Bi-directional automated synchronisation where technically feasible (CMDB ↔ inventory, HRIS → inventory).
...
- **Primary Location**: All Control A.5.9 evidence maintained in [Organisation's GRC Platform / Central Evidence Repository]. Asset inventory database hosted in [System Name]. Assessment workbooks archived in [ISMS Evidence Library].
- **Retention Period**: Minimum 3 years per ISO 27001 Clause 7.5.3, aligned with [Organisation] general record retention policy. Assessment workbooks, owner acknowledgments, gap registers, and exception register retained for duration of certification plus 3 years.
- **Access Control**: Evidence repository access restricted to Information Security Manager, Internal Audit, and authorised personnel per A.5.15 Access Control policy. External auditors granted read-only access during audit period.
- **Backup**: Evidence backed up per A.8.13 Information Backup requirements; tested quarterly.
...
Before diving into responsibilities, be aware of these frequent errors:

1. **Confusing 'Owner' with 'Custodian'**: You're accountable even if IT manages the system. Owner = business accountability, Custodian = operational management.
2. **Classifying all assets as 'Critical'**: Use the business impact framework (Annex A). Not everything is critical - this dilutes the meaning.
3. **Listing individual people as personnel assets**: Document roles/competencies, not names. "Database Administrator competency (3 qualified staff)" NOT "John Smith - DBA".
4. **Forgetting to update inventory when assets change**: Set calendar reminders for owned assets. Changes trigger updates within staleness thresholds.
5. **Assuming someone else will update**: As Owner, you are accountable for record accuracy even if Custodian performs updates.
