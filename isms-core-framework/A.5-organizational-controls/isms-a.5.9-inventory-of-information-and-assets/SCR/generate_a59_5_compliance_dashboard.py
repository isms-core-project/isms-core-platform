...
Output:
    File: ISMS_A_5_9_5_Compliance_Dashboard_YYYYMMDD.xlsx
...
def main():
    ...
    logger.info("="*80)
    logger.info("ISMS Control A.5.9 - Compliance Dashboard Generator")
    logger.info("="*80)
    logger.info("")
    ...
    logger.info(f"Date suffix: {date_suffix}")
    logger.info("")
    
    # Validate input files exist
    logger.info("Validating input CSV files...")
    csv_paths = {}
    all_exist = True
    ...
    if not all_exist:
        logger.info("")
        logger.error("ERROR: Not all required CSV files found.")
        logger.info("Please ensure all four assessments have exported their metrics to CSV.")
        logger.info("")
        logger.info("Required files:")
        for key, pattern in CONFIG['csv_files'].items():
            logger.info(f"  - {pattern.replace('{date}', date_suffix)}")
        logger.info("")
        return 1
    
    logger.info("")
    logger.info("All CSV files validated ✓")
    logger.info("")
    
    if dry_run:
        logger.info("DRY RUN mode - validation complete, not generating output.")
        return 0
    
    # Create workbook
    logger.info("Generating dashboard workbook...")
    logger.info("")
    ...
    logger.info("  1. Review Executive Summary (Sheet 1) for overall status")
    logger.info("  2. Validate Compliance Scorecard weighted calculations")
    logger.info("  3. Review imported metrics from all 4 assessments")
    logger.info("  4. Analyze Trending Analysis for improvement trajectory")
    logger.info("  5. Review Action Register and assign priorities/owners")
    logger.info("  6. Present to CISO and senior management")
    logger.info("  7. Use for audit evidence (Stage 1/Stage 2)")
    logger.info("  8. Update quarterly or after remediation completion")
    logger.info("")
    logger.info(f"Output location: {os.path.abspath(output_path)}")
    logger.info("")
...
def read_approval_status():
    """Read approval status from each assessment workbook"""
    import os
    from openpyxl import load_workbook

    approval_data = []
    assessments = [
        ('Asset Discovery', 'ISMS-IMP-A.5.9.1_Asset_Discovery'),
        ('Inventory Maintenance', 'ISMS-IMP-A.5.9.2_Inventory_Maintenance'),
        ('Quality & Compliance', 'ISMS-IMP-A.5.9.3_Quality_Compliance'),
        ('Owner Accountability', 'ISMS-IMP-A.5.9.4_Owner_Accountability'),
    ]
    
    for assessment_name, file_prefix in assessments:
        status = {
        ...
        # Find the most recent workbook for this assessment
        try:
            ...
            if files:
                ...
                # Check if Approval & Sign-Off sheet exists
                if 'Approval & Sign-Off' in wb.sheetnames:
                    ws = wb['Approval & Sign-Off']
                    
                    # Look for CISO approval (usually last approver)
                    # Scan for "CISO" row and check if signature/date is filled
                    for row in range(1, 100):
                        cell_value = ws.cell(row=row, column=1).value
                        if cell_value and 'CISO' in str(cell_value):
                            # Check if name and date are filled (columns 2 and 4)
                            name = ws.cell(row=row, column=2).value
                            date = ws.cell(row=row, column=4).value
                            if name and date:
                                status['approved'] = True
                                status['approver'] = str(name)
                                status['date'] = str(date)
                            break
                
                wb.close()
        except Exception as e:
            pass  # Skip if file not found or error reading
        
        approval_data.append(status)
...
def create_executive_summary_sheet(ws):
    ...
    ws['A3'] = f"Organisation: {CONFIG['organisation']}"
    ...
    # Assessment Approval Status section
    ws['A18'] = "ASSESSMENT APPROVAL STATUS"
    ...
    ws['A19'] = "Assessment"
    ws['B19'] = "Status"
    ws['C19'] = "Approval Date"
    ws['D19'] = "Approved By"
    for col in ['A', 'B', 'C', 'D']:
        ws[f'{col}19'].font = Font(bold=True)
    
    # Read approval status from assessment workbooks
    approval_data = read_approval_status()
    
    row = 20
    approved_count = 0
    for assessment in approval_data:
        ws[f'A{row}'] = assessment['name']
        if assessment['approved']:
            ws[f'B{row}'] = "✅ Approved"
            ws[f'C{row}'] = assessment['date']
            ws[f'D{row}'] = assessment['approver']
            approved_count += 1
        else:
            ws[f'B{row}'] = "⏳ Pending"
            ws[f'C{row}'] = "-"
    ...
    ws[f'A{row}'] = "Overall Approval Progress:"
    ws[f'A{row}'].font = Font(bold=True)
    ws[f'C{row}'] = f"{approved_count}/4 Complete ({approved_count*25}%)"
    ws[f'C{row}'].font = Font(bold=True)
    if approved_count == 4:
        ws[f'C{row}'].fill = PatternFill(start_color=CONFIG['colors']['green_light'], fill_type='solid')
    elif approved_count >= 2:
        ws[f'C{row}'].fill = PatternFill(start_color=CONFIG['colors']['yellow_light'], fill_type='solid')
    else:
        ws[f'C{row}'].fill = PatternFill(start_color=CONFIG['colors']['red_light'], fill_type='solid')
    
    # Key findings
    ws['A26'] = "KEY FINDINGS & PRIORITIES"
    ws['A18'].font = Font(size=12, bold=True, color=CONFIG['colors']['header_text'])
    ws['A18'].fill = PatternFill(start_color=CONFIG['colors']['section_bg'], fill_type='solid')
    ws.merge_cells('A26:H26')
    
    ws.merge_cells('A27:H29')
    ws['A27'] = "[Auto-populated from Action Register - Top 3 Priorities]"
    ...
    ws['A32'] = "[Summary recommendation based on overall score - approve/remediate/escalate]"
...
def create_action_register_sheet(ws):
    ...
    ws['A1'] = "Action Register - Prioritised Remediation Actions"
    ws['A1'].font = Font(size=14, bold=True, color=CONFIG['colors']['header_text'])
    ws['A1'].fill = PatternFill(start_color=CONFIG['colors']['header_bg'], fill_type='solid')
