<!-- ISMS-CORE:IMP:ISMS-IMP-A.7.6-7-14-S1-UG:framework:UG:a.7.6-7-14-s1 -->
**ISMS-IMP-A.7.6-7-14-S1-UG - Secure Areas Working Assessment**
**User Completion Guide**
### ISO/IEC 27001:2022 Control A.7.6: Working in Secure Areas

---

**Document Control**

| Attribute | Value |
|-----------|-------|
| **Document ID** | ISMS-IMP-A.7.6-7-14-S1-UG |
| **Version** | 1.0 |
| **Assessment Area** | Secure Areas - Working Procedures, Access Controls, Personnel Conduct |
| **Related Policy** | ISMS-POL-A.7.6-7-14, Section 2.1 (Working in Secure Areas) |
| **Purpose** | Document secure area definitions, assess working procedures against policy requirements, track personnel compliance and third-party access |
| **Target Audience** | Facilities Management, Security Operations, IT Operations, Compliance Officers, Auditors |
| **Assessment Type** | Operational & Procedural |
| **Review Cycle** | Quarterly or After Major Facility Changes |
| **Date** | [Date] |

### Version History

| Version | Date | Changes | Author |
|---------|------|---------|--------|
| 1.0 | [Date] | Initial technical specification for Secure Areas Working assessment workbook | ISMS Implementation Team |

### Document Structure

This is the **User Completion Guide**. The companion Technical Specification is documented in ISMS-IMP-A.7.6-7-14-S1-TG.

---

## Assessment Overview

### Purpose & Scope

**Assessment Name:** ISMS-IMP-A.7.6-7-14-S1 - Secure Areas Working Assessment

#### What This Assessment Covers

This assessment documents the SECURE AREAS defined within your organisation and evaluates the working procedures in place for personnel operating within these controlled environments. It answers:

- What areas are classified as secure areas? (server rooms, datacentres, security operations centres, executive offices)
- What access controls are in place for each secure area?
- What working procedures are defined and enforced?
- How is third-party access managed? (visitors, contractors, maintenance personnel)
- What security incidents have occurred in secure areas?
- What compliance gaps exist between current practices and policy requirements?

#### ISO 27001:2022 Control Reference

> *Security measures for working in secure areas should be designed and implemented.*
>
> — ISO/IEC 27001:2022 Annex A, Control A.7.6

**Control Objective:** Protect information and assets in secure areas by ensuring personnel follow appropriate security procedures.

#### Key Principle

This assessment is **completely vendor-agnostic and technology-independent**. You document YOUR specific secure areas and verify working procedures against generic policy requirements from ISMS-POL-A.7.6-7-14, Section 2.1.

#### What You'll Document

**Secure Area Inventory:**

- Every defined secure area in your organisation
- Classification level of each area (Tier 1 Critical, Tier 2 Standard)
- Access control mechanisms (badge readers, biometrics, mantraps)
- Authorised personnel lists and access review dates

**Working Procedures:**

- Need-to-know basis communication requirements
- Unsupervised working restrictions
- Recording equipment controls (cameras, phones)
- Emergency procedure accessibility
- Clean desk/screen enforcement in secure areas

**Third-Party Access:**

- Visitor escort requirements and compliance
- Contractor access logging and time limits
- Third-party confidentiality agreement status
- Equipment inspection procedures

**Incidents and Compliance:**

- Secure area security incidents (last 12 months)
- Procedure violations and remediation actions
- Periodic inspection results

#### How This Relates to Other A.7.6-7-14 Assessments

| Assessment | Focus | Relationship to S1 |
|------------|-------|-------------------|
| **ISMS-IMP-A.7.6-7-14-S1** | **Secure Areas Working** | **WHAT secure areas exist, HOW do personnel work within them** |
| ISMS-IMP-A.7.6-7-14-S2 | Clear Desk/Screen Compliance | Workspace information protection practices |
| ISMS-IMP-A.7.6-7-14-S3 | Equipment Disposal Assessment | Secure equipment disposal and re-use |
| ISMS-IMP-A.7.6-7-14-S4 | Compliance Dashboard | Consolidated view across all secure areas and media handling |

### Who Should Complete This Assessment

#### Primary Stakeholders

1. **Facilities Manager** - Secure area physical infrastructure ownership
2. **Security Operations Manager** - Access control and monitoring oversight
3. **IT Operations** - Server room and datacentre management
4. **Compliance Officers** - Audit requirements and evidence collection

#### Required Skills

- Understanding of physical security zones and classifications
- Familiarity with access control systems and procedures
- Knowledge of visitor management processes
- Understanding of information classification levels
- Access to facility floor plans and security documentation

#### Time Commitment

- **Initial assessment:** 8-12 hours (comprehensive review of all secure areas)
- **Quarterly updates:** 2-4 hours (update incidents, access reviews, minor changes)

### Expected Outputs

Upon completion, you will have:

1. **Complete secure area inventory** - Every secure area documented with classification
2. **Access control assessment** - Who has access, how access is controlled
3. **Procedure compliance analysis** - Working procedures against policy requirements
4. **Third-party access tracking** - Visitor and contractor access management
5. **Incident tracking** - Security incidents and procedure violations documented
6. **Gap analysis** - Identified gaps between current practices and requirements
7. **Evidence register** - Supporting documentation for audit
8. **Approved assessment** - Four-level approval workflow completed

---

## Prerequisites

### Information You'll Need

Before starting this assessment, gather:

#### 1. Facility Documentation

- Facility floor plans with secure area boundaries marked
- Secure area classification documentation
- Access control system documentation (badge reader locations, access levels)
- Emergency evacuation procedures for each secure area

#### 2. Access Management Records

- Current authorised personnel lists per secure area
- Access review records (quarterly reviews)
- Third-party/visitor access logs (last 90 days)
- Contractor access agreements and confidentiality agreements

#### 3. Procedure Documentation

- Working in secure areas procedures
- Visitor escort procedures
- Recording equipment policy
- Emergency procedure locations in each secure area

#### 4. Historical Data

- Security incident reports involving secure areas (last 12 months)
- Procedure violation reports
- Periodic inspection results
- Access control audit logs

#### 5. Policy Requirements

- ISMS-POL-A.7.6-7-14, Section 2.1 (Working in Secure Areas Requirements)
  - Access and Conduct requirements
  - Third-Party Access requirements
  - Information Protection requirements

### Required Tools

- Microsoft Excel (2016 or later) for workbook completion
- Access to facility floor plans
- Access to access control system admin console
- Access to visitor management system
- Screen capture tools (for evidence screenshots)

### Dependencies

This assessment has NO dependencies on other assessments - it can be completed independently.

However, outputs from this assessment are INPUT to:

- ISMS-IMP-A.7.6-7-14-S4 (Compliance Dashboard) - Consolidates secure area compliance with other controls

---

## Workflow

### High-Level Process

```
1. PREPARE
   |
2. INVENTORY SECURE AREAS (Sheet 2: Secure Area Register)
   |
3. ASSESS WORKING PROCEDURES (Sheet 3: Working Procedures)
   |
4. DOCUMENT THIRD-PARTY ACCESS (Sheet 4: Third-Party Access)
   |
5. REVIEW INCIDENTS (Sheet 5: Incidents - last 12 months)
   |
6. COLLECT EVIDENCE (Sheet 7: Evidence Register)
   |
7. REVIEW SUMMARY (Sheet 6: Summary Dashboard - automated scoring)
   |
8. QUALITY CHECK (Self-assessment against checklist)
   |
9. OBTAIN APPROVALS (Sheet 8: Approval Sign-Off)
   |
10. SUBMIT FOR AUDIT
```

### Detailed Step-by-Step

**Step 1: Preparation (Day 1 - 2 hours)**

- Read this Part I User Guide completely
- Review ISMS-POL-A.7.6-7-14, Section 2.1 requirements
- Gather all prerequisites (facility plans, access records, procedures)
- Schedule time with Facilities Manager and Security Operations Manager
- Download or generate assessment workbook (Excel file)

**Step 2: Secure Area Inventory (Day 1-2 - 3-4 hours)**

- Open assessment workbook
- Complete Sheet 1 (Instructions & Legend) - organisation info, assessment date
- Complete Sheet 2 (Secure Area Register) - inventory all defined secure areas
- Document classification levels, access mechanisms, authorised personnel counts
- Verify access review dates are current

**Step 3: Working Procedures Assessment (Day 2 - 2-3 hours)**

- Complete Sheet 3 (Working Procedures)
- Assess each procedure requirement against policy
- Document compliance status (Compliant, Partial, Non-Compliant)
- Note gaps and remediation plans

**Step 4: Third-Party Access Review (Day 2-3 - 2 hours)**

- Complete Sheet 4 (Third-Party Access)
- Review visitor and contractor access records
- Verify escort compliance and time limits
- Check confidentiality agreement status

**Step 5: Incident Review (Day 3 - 1-2 hours)**

- Query incident management system for secure area incidents (last 12 months)
- Complete Sheet 5 (Incidents) for each incident
- Classify incidents and document response actions

**Step 6: Evidence Collection (Day 3-4 - 2-3 hours)**

- Take screenshots of access control configurations
- Collect sample access logs
- Document procedure locations
- Complete Sheet 7 (Evidence Register)

**Step 7: Summary Review (Day 4 - 1 hour)**

- Review Sheet 6 (Summary Dashboard) - formulas automatically calculate compliance scores
- Verify compliance scores are accurate
- Identify areas requiring remediation

**Step 8: Quality Check (Day 4 - 1 hour)**

- Complete self-assessment using Quality Checklist (see section below)
- Verify all required fields completed
- Verify evidence register complete

**Step 9: Obtain Approvals (Day 5-10 - asynchronous)**

- Complete Sheet 8 (Approval Sign-Off) - Level 1: Assessor (you)
- Submit to Level 2: Security Operations Manager for review
- After Level 2 approval, submit to Level 3: CISO
- After Level 3 approval, submit to Level 4: Compliance Officer

**Step 10: Submit for Audit (Post-Approval)**

- Assessment workbook is now audit-ready
- Provide to Internal Audit or External Auditors
- Evidence register provides traceability

---

## Completing Each Sheet

### Sheet 1: Instructions & Legend

**Purpose:** Assessment metadata and legend for status indicators

**What to Complete:**

- **Document Information** (Rows 4-11):
  - Assessment Date: Date you started this assessment
  - Completed By: Your name and role
  - Organisation: [Organisation] name
  - Leave other fields as-is (Document ID, Version, etc.)

- **Status Legend** (Rows 14-17):
  - Read-only reference - understand colour coding:
    - Green: Compliant - Meets all policy requirements
    - Amber: Partial - Meets some requirements, gaps identified
    - Red: Non-Compliant - Does not meet requirements, immediate action needed

**Time Required:** 5 minutes

### Sheet 2: Secure Area Register

**Purpose:** Document all defined secure areas and their characteristics

**What to Document (Per Secure Area):**

**Column A - Secure Area ID:**

- Unique identifier: "SA-001", "SA-002", etc.

**Column B - Secure Area Name:**

- Descriptive name: "Server Room - Building A", "Datacentre 1", "Security Operations Centre"

**Column C - Location:**

- Physical location: "Building A, Floor 2", "Basement Level 1", etc.

**Column D - Classification:**

- Dropdown: "Tier 1 - Critical", "Tier 2 - Standard"
- Tier 1: Datacentres, server rooms, security operations centres
- Tier 2: Executive offices, R&D labs, archive rooms

**Column E - Access Control Type:**

- Type of access control: "Badge + PIN", "Biometric", "Badge Only", "Mantrap"

**Column F - Authorised Personnel Count:**

- Number of personnel with access: 5, 15, 30, etc.

**Column G - Last Access Review:**

- Date of last quarterly access review: "15.12.2025"

**Column H - Emergency Procedures Posted:**

- Dropdown: "Yes", "No"
- Verify emergency procedures are visibly posted in the area

**Column I - Recording Equipment Controls:**

- Dropdown: "Prohibited", "Authorised Only", "No Restriction"

**Column J - Unsupervised Working:**

- Dropdown: "Prohibited", "Restricted", "Allowed"

**Column K - Compliance Status:**

- Formula auto-calculates based on requirements

**Column L - Notes:**

- Any additional context: "Access review scheduled for Q1 2026"

**Time Required:** 2-3 hours for comprehensive secure area inventory

### Sheet 3: Working Procedures

**Purpose:** Assess compliance with each working procedure requirement

**What to Document (Per Requirement):**

**Column A - Requirement ID:**

- Pre-populated requirement identifiers

**Column B - Requirement Description:**

- Pre-populated from policy requirements

**Column C - Implementation Status:**

- Dropdown: "Implemented", "Partial", "Not Implemented", "N/A"

**Column D - Evidence:**

- Description of evidence demonstrating compliance

**Column E - Compliance Status:**

- Formula auto-calculates

**Column F - Notes:**

- Gap details, remediation plans

**Time Required:** 1-2 hours

### Sheet 4: Third-Party Access

**Purpose:** Document visitor and contractor access to secure areas

**What to Document (Per Access Event or Summary):**

**Column A - Access Period:**

- Period covered: "Q4 2025", "January 2026"

**Column B - Secure Area:**

- Which secure area: "Server Room - Building A"

**Column C - Visitor/Contractor Count:**

- Number of third parties accessing: 5, 12, etc.

**Column D - Escort Compliance:**

- Percentage escorted as required: "100%", "90%", etc.

**Column E - NDA Status:**

- All signed NDAs: "Yes", "Partial", "No"

**Column F - Time Limit Compliance:**

- Access within authorised timeframes: "Yes", "Partial", "No"

**Column G - Equipment Inspected:**

- Third-party equipment inspected: "Yes", "N/A", "No"

**Column H - Compliance Status:**

- Formula auto-calculates

**Column I - Notes:**

- Issues identified, remediation actions

**Time Required:** 1-2 hours

### Sheet 5: Incidents

**Purpose:** Document secure area security incidents from last 12 months

**What to Document (Per Incident):**

**Column A - Incident ID:**

- Unique incident identifier from incident management system

**Column B - Incident Date:**

- Date incident occurred: "15.12.2025"

**Column C - Secure Area:**

- Where incident occurred: "Server Room - Building A"

**Column D - Incident Type:**

- Dropdown: "Unauthorised Access", "Procedure Violation", "Tailgating", "Equipment Policy Violation", "Other"

**Column E - Severity:**

- Dropdown: "Critical", "High", "Medium", "Low"

**Column F - Description:**

- Brief incident description

**Column G - Response Time (Min):**

- Minutes from detection to response

**Column H - Resolution Status:**

- Dropdown: "Resolved", "In Progress", "Escalated"

**Column I - Corrective Action:**

- Actions taken to prevent recurrence

**Column J - Notes:**

- Additional context

**Time Required:** 1-2 hours depending on incident volume

### Sheet 6: Summary Dashboard

**Purpose:** Automated compliance scoring and overall secure area health

**What to Review (Auto-Calculated - Read-Only):**

- Overall Compliance Score
- Secure Area Coverage
- Working Procedure Compliance
- Third-Party Access Compliance
- Incident Metrics
- Gap Summary

**Time Required:** 15-30 minutes for review

### Sheet 7: Evidence Register

**Purpose:** Document all supporting evidence for audit traceability

**What to Document (Per Evidence Item):**

- Evidence ID, Type, Description
- Related Sheet/Item
- File Name, Location
- Collection Date, Collected By
- Retention Period

**Time Required:** 2-3 hours for comprehensive evidence collection

### Sheet 8: Approval Sign-Off

**Purpose:** Four-level approval workflow for assessment completion

**Approval Levels:**

- Level 1: Assessor
- Level 2: Security Operations Manager
- Level 3: CISO
- Level 4: Compliance Officer

**Time Required:** 5 minutes for Level 1, then asynchronous for Levels 2-4

---

## Evidence Collection

### What Evidence to Collect

**Evidence Categories:**

**1. Secure Area Documentation**

- Floor plans with secure area boundaries marked
- Secure area classification matrix
- Access control system configuration screenshots
- Authorised personnel lists

**2. Working Procedure Evidence**

- Procedure documents (working in secure areas)
- Emergency procedure postings (photos)
- Recording equipment policy and signage
- Clean desk audit results for secure areas

**3. Third-Party Access Evidence**

- Visitor logs (sample period)
- Contractor access agreements
- NDA/confidentiality agreements
- Equipment inspection records

**4. Compliance Evidence**

- Access review records
- Periodic inspection reports
- Training records (secure area procedures)
- Incident reports and resolution documentation

### Evidence Storage

**Storage Location:**

- SharePoint > ISMS > Assessments > A.7.6-7-14 > S1_Secure_Areas > Evidence

**Retention:**

- Minimum: 3 years (typical audit requirement)

---

## Common Pitfalls

### Pitfall 1: Incomplete Secure Area Inventory

**Problem:** Only documenting primary facilities, missing branch offices or temporary secure areas

**Impact:** Audit finding - incomplete assessment scope

**How to Avoid:**

- Create master secure area list BEFORE starting assessment
- Include ALL locations where [Organisation] has defined secure areas
- For temporary secure areas (event venues, project rooms), document if applicable

### Pitfall 2: Outdated Access Reviews

**Problem:** Access reviews not conducted quarterly as required

**Impact:** Non-compliance with access management requirements

**How to Avoid:**

- Verify access review dates for each secure area
- Flag overdue reviews as gaps requiring immediate remediation
- Document scheduled review dates for overdue areas

### Pitfall 3: Missing Emergency Procedure Evidence

**Problem:** Claiming emergency procedures are posted without photo evidence

**Impact:** Cannot demonstrate compliance during audit

**How to Avoid:**

- Take photographs of emergency procedure postings in each secure area
- Include date stamp or reference point in photos
- Document in Evidence Register with location details

### Pitfall 4: Third-Party Access Gaps

**Problem:** Visitor access not properly logged or escorts not provided

**Impact:** Policy violation and potential security risk

**How to Avoid:**

- Review visitor logs for escort compliance
- Verify NDA status for all contractors with secure area access
- Document any gaps and remediation actions

### Pitfall 5: Incident Under-Reporting

**Problem:** Only documenting major incidents, missing procedure violations

**Impact:** Incomplete trend analysis, systemic issues unidentified

**How to Avoid:**

- Include ALL secure area incidents (not just Critical/High)
- Include procedure violations (recording equipment, unsupervised working)
- Document "near misses" where appropriate

### Pitfall 6: Recording Equipment Policy Not Enforced

**Problem:** Policy exists but no enforcement mechanism documented

**Impact:** Partial compliance status

**How to Avoid:**

- Document signage and awareness mechanisms
- Include device storage/lockbox availability if applicable
- Note any authorisation process for permitted recording

### Pitfall 7: Unsupervised Working Controls Missing

**Problem:** Policy prohibits unsupervised working but no monitoring in place

**Impact:** Policy exists but cannot demonstrate enforcement

**How to Avoid:**

- Document how unsupervised working is prevented (buddy system, monitoring)
- Include access control logs showing dual-access requirements if applicable
- Note exceptions and authorisation process

### Pitfall 8: Copy-Paste Errors

**Problem:** Copying rows in Excel, forgetting to update secure area names

**Impact:** Inaccurate assessment data

**How to Avoid:**

- Enter data methodically, one secure area at a time
- Cross-check secure area names against inventory
- Have second person spot-check

---

## Quality Checklist

Before submitting for Level 2 approval, complete this self-assessment:

### Sheet 1: Instructions & Legend

- [ ] Assessment Date completed
- [ ] Completed By (your name and role)
- [ ] Organisation name filled in

### Sheet 2: Secure Area Register

- [ ] ALL defined secure areas documented
- [ ] Classification levels assigned (Tier 1/Tier 2)
- [ ] Access control types documented
- [ ] Authorised personnel counts accurate
- [ ] Access review dates current (within 90 days) or flagged as overdue
- [ ] Emergency procedures posted status verified
- [ ] Recording equipment controls documented
- [ ] Unsupervised working policy documented
- [ ] At least one secure area documented (or explicitly state "No defined secure areas")

### Sheet 3: Working Procedures

- [ ] ALL procedure requirements assessed
- [ ] Implementation status accurate
- [ ] Evidence documented for each requirement
- [ ] Gaps identified with remediation plans

### Sheet 4: Third-Party Access

- [ ] Access periods documented
- [ ] Escort compliance verified
- [ ] NDA status confirmed
- [ ] Time limit compliance assessed
- [ ] Equipment inspection status documented

### Sheet 5: Incidents

- [ ] ALL secure area incidents from last 12 months documented
- [ ] Incident types classified correctly
- [ ] Severities assigned per policy
- [ ] Response times documented
- [ ] Corrective actions documented
- [ ] If zero incidents: explicitly documented

### Sheet 6: Summary Dashboard

- [ ] Overall Compliance Score displays
- [ ] Scores appear reasonable
- [ ] Investigate any unexpected red status

### Sheet 7: Evidence Register

- [ ] At least 10 evidence items documented
- [ ] Evidence types appropriate
- [ ] File locations accurate
- [ ] Evidence actually exists in documented location

### Sheet 8: Approval Sign-Off

- [ ] Level 1 (Assessor) completed

---

## Review & Approval

### Four-Level Approval Workflow

**Level 1: Assessor (You)** - Complete assessment and self-review

**Level 2: Security Operations Manager** - Technical review and validation

**Level 3: CISO** - Executive review and resource allocation

**Level 4: Compliance Officer** - Final audit readiness certification

---

**END OF USER GUIDE**

---

*"The measure of intelligence is the ability to change."*
— Albert Einstein

<!-- QA_VERIFIED: 2026-02-06 -->
