...
# =============================================================================
"""
ISMS-A.7.4-5-11 Physical Infrastructure - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('A.7.4.1', 'Access_Monitoring', 'Access Monitoring'),
    ('A.7.4.2', 'Environmental_Protection', 'Environmental Protection'),
    ('A.7.4.3', 'Utility_Resilience', 'Utility Resilience'),
]

DASHBOARD_PATTERNS = ['A.7.4.4', 'Compliance_Dashboard']
            ...
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'issue']):
                ...
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    if status or gap_desc:
                        ...
                        gaps.append([f'GAP-PHY-{gap_counter:03d}', assessment_area,
                            os.path.basename(filepath), row_values[0] or 'Facility',
...
def generate_risks_and_remediation(gaps):
    ...
    for i, gap in enumerate([g for g in gaps if g[7] == 'High'], 1):
        risks.append([f'RISK-{i:03d}', gap[0], 'Physical Security', f'Risk: {gap[4][:80]}',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.7.4-5-11 Physical Infrastructure - Dashboard Consolidation")
    print("=" * 80)
