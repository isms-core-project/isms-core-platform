...
- **ISO/IEC 27001:2022 Control A.8.15**: Event logs recording user activities, exceptions, faults and information security events shall be produced, kept and regularly reviewed
...
**Phase 4: Logging Configuration Verification (Sheets 4-10)**

For each system, verify actual logging configuration:
- **Authentication Events** (Sheet 4): Login success/failure, logout, account lockouts, password changes, MFA events
- **Authorization Events** (Sheet 5): Access to sensitive data, privilege escalation, access control changes
- **Administrative Actions** (Sheet 6): Configuration changes, account management, privilege grants, policy changes
- **Security Events** (Sheet 7): Malware detection, intrusion attempts, firewall blocks, DLP alerts
- **System Events** (Sheet 8): Startup/shutdown, service status, errors, resource warnings
- **Network Events** (Sheet 9): Firewall rule matches, VPN connections, network segmentation traversals
- **Application Events** (Sheet 10): Errors, exceptions, database transactions, API authentication
...
**Phase 5: Gap Identification (Sheet 11)**
...
**Phase 6: Evidence Collection (Sheet 12)**
...
### 4.2 Sheet 3: Event Logging Requirements Summary
...
### 4.3 Sheets 4-10: Event Category Verification
...
**Column N**: Evidence Reference (pointer to Sheet 12 Evidence Register)
**Column O**: Notes

#### Sheet 4: Authentication Events
...
#### Sheet 5: Authorization & Access Control Events
...
#### Sheet 6: Administrative Actions
...
**Specific Events Checklist (Column G)**:
Per ISMS-POL-A.8.15, Section 2.1 (Application Events):
- [ ] Application errors/exceptions (application name, error type, error details, timestamp)
- [ ] Database transactions involving sensitive data (user, table/record accessed, operation, timestamp)
- [ ] API authentication/authorization (API endpoint, user/service account, method, outcome, timestamp)
- [ ] Data export operations (user, data scope, export format, destination, timestamp)
- [ ] Application-level access control decisions (user, requested resource, decision, timestamp)

**Common Gaps**:
- Only critical errors logged, not warnings or info-level events
- Database transactions logged but not specific enough (missing table/record accessed)
- API calls logged but not authentication outcomes
- Data exports not logged (compliance risk for GDPR/data protection)
...
### 5.2 Evidence Register (Sheet 12)

All evidence cataloged in Sheet 12 with:
...
### 6.1 Inventory Completeness Issues
...
### 6.3 "Logging is Enabled" But Insufficient Detail
...
### 6.4 Local Logs Only (Not Centralized)
...
### 6.5 Default Logging Levels (Insufficient)
...
### 6.6 Cloud Services Assumptions
...
### 6.7 Encrypted Traffic Blindness
...
### 6.8 Decommissioned Systems Still in Inventory
...
### 6.9 Inconsistent Assessment Across Similar Systems
...
### 6.10 Ignoring Legacy/End-of-Life Systems
...
- [ ] **Evidence attached**: Every non-compliant finding has evidence reference (Sheet 12)
...
- [ ] **All gaps identified**: Every "Non-Compliant" or "Partial Compliance" has corresponding gap in Sheet 11
...
- [ ] **Evidence linkage**: Evidence Register (Sheet 12) correctly references evidence files
...
### 8.1 Three-Level Approval Process
...
3. Verify logging for each event category (Sheets 4-10)
4. Identify gaps in Sheet 11 (Gap Analysis)
5. Collect evidence in Sheet 12 (Evidence Register)
6. Obtain approvals in Sheet 13 (Approval & Sign-Off)
...
## Sheets 4-10: Event Category Verification
...
| N | Evidence Reference | 15 | Text | Link to Sheet 12 (e.g., "EVD-045") |
...
### Sheet 8: System_Events - Specific Details

**Column G Sample Content**:
```
Per ISMS-POL-A.8.15, Section 2.1:
[X] System startup/shutdown (system ID, event type, timestamp)
[X] Service starts/stops/restarts (service name, action, initiated by)
[X] System errors/crashes (error type, affected system/component, details)
[ ] Resource exhaustion warnings (resource type, utilization level, system)
[X] Time synchronization events (time source, sync status, drift detected)
```

---

### Sheet 9: Network_Events - Specific Details
...
### Sheet 10: Application_Events - Specific Details
...
| M | Evidence Reference | 15 | Text | Link to Sheet 12 (e.g., "EVD-087") |
...
- Column C: [Name]
- Column D: [Date]
- Column E: "I approve this assessment and authorize the remediation plan per Sheet 11 (Gap Analysis)"
...
- v2.0: Updated policy references to consolidated ISMS-POL-A.8.15 (v2.0), updated event categories per revised Section 2.1

**Backward Compatibility**:
- v2.0 workbooks can be opened in Excel 2016+
- v1.0 workbooks should be migrated to v2.0 to reflect consolidated policy structure
- Migration: Manual update of policy references + formula validation
...
Event Requirements (Sheet 3) - Determines what to log based on system attributes
  â†"
Event Category Verification (Sheets 4-10) - Verifies actual configuration
  â†"
Gap Analysis (Sheet 11) - Consolidates non-compliance findings
  â†"
Evidence Register (Sheet 12) - Links supporting evidence
  â†"
Approval (Sheet 13) - Three-level sign-off
  â†"
...
**Quality Review**: All policy references updated to consolidated ISMS-POL-A.8.15-Logging.md (v2.0)

---

*This implementation assessment enables systematic verification of logging requirements per ISMS-POL-A.8.15. Assessment workbook provides objective evidence for ISO 27001 audit validation of Control A.8.15 implementation.*
