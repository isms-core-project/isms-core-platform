...
### 1.6 Critical: Coverage Gap = Bypass Risk
...
### 6.1 "We protect email and web, so we're covered"
...
### 6.2 "DLP is deployed but set to monitor-only everywhere"
...
### 6.3 "USB is blocked, but users can still use Bluetooth/AirDrop"
...
### 6.4 "We have CASB, so cloud is covered"
...
### 6.5 "Mobile devices are MDM-managed, so we're safe"
...
### 6.6 "Application-level DLP is too complex, we skip it"
...
### 8.3 Reviewed By (Security Team Lead)
...
| 2 | Channel_Inventory | 30 | Complete channel catalog | Yes (all egress channels) |
| 3 | Email_Channel | 20 | Email DLP assessment | Yes (email DLP config) |
| 4 | Web_Channel | 25 | Web/proxy/CASB DLP assessment | Yes (web DLP config) |
...
| 7 | Cloud_Channel | 25 | Cloud apps, CASB, SaaS DLP | Yes (cloud DLP) |
| 8 | Mobile_Channel | 25 | Mobile devices, MDM, MAM | Yes (mobile DLP) |
| 9 | Application_Channel | 20 | Database, API, app-level controls | Yes (app controls) |
| 10 | Bypass_Testing | 35 | Bypass test results | Yes (test outcomes) |
| 11 | Gap_Analysis | 45 | Gaps and remediation | Yes (gap details) |
| 12 | Evidence_Register | 110 | Evidence tracking | Yes (evidence entries) |
| 13 | Summary_Dashboard | 40 | KPIs, coverage heatmap | No (formulas) |
