...
"""
================================================================================
ISMS CORE Operational — Compliance Checklist Engine
================================================================================

Shared engine for all Operational compliance checklist generators.
Each control group has a thin generator that defines REQUIREMENTS and calls
this engine.  All workbook structure, styling, formulas, and sheet creation
logic lives here — written once, used by 53 generators.
...
Design principles:
  - Every "shall" statement in an OP-POL = one checklist row
...
def create_executive_summary(ws, styles, domain_config, document_id, control_name):
    ...
    Args:
        ...
        document_id: e.g. "ISMS-OP-CHK-A.5.3"
        control_name: e.g. "Segregation of Duties"
    """
    ...
    doc_info = [
        ...
        ("Source Policy", document_id.replace("OP-CHK", "OP-POL")),
    ...
    for role in ["Assessed By", "Reviewed By (ISO Manager)", "Approved By (CISO)"]:
        ws.merge_cells(f"A{row}:B{row}")
        ws[f"A{row}"] = role
...
def generate_checklist(document_id, control_id, control_name, source_policy,
    ...
    """Generate a complete Operational Compliance Checklist workbook.

    Args:
        document_id:    e.g. "ISMS-OP-CHK-A.5.3"
        control_id:     e.g. "A.5.3"
        control_name:   e.g. "Segregation of Duties"
        source_policy:  e.g. "ISMS-OP-POL-A.5.3"
    ...
    ctrl_word = "Controls" if '-' in control_id else "Control"
    control_ref = f"ISO/IEC 27001:2022 \u2014 {ctrl_word} {control_id}: {control_name}"
