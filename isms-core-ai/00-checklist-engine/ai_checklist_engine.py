...
"""
================================================================================
ISMS CORE AI — Compliance Checklist Engine
================================================================================

Shared engine for all AI Management System (ISO/IEC 42001:2023) compliance
checklist generators.  Each control group has a thin generator that defines
REQUIREMENTS and calls this engine.  All workbook structure, styling, formulas,
and sheet creation logic lives here — written once, used by 10 generators.
...
Design principles:
  - Every "shall" statement in an AI-POL = one checklist row
...
Modelled on priv_checklist_engine.py (51-isms-core-privacy/00-checklist-engine).
...
def create_executive_summary(ws, styles, domain_config, document_id, control_name, source_policy):
    ...
    total_row_dash = 4 + len(domain_config)
    ...
    for role in ["Assessed By (AI System Owner)", "Reviewed By (AI Governance Officer)", "Approved By (CEO / CISO)"]:
        ws.merge_cells(f"A{row}:B{row}")
        ws[f"A{row}"] = role
...
def generate_checklist(document_id, control_id, control_name, source_policy,
    ...
    """Generate a complete AI Compliance Checklist workbook.

    Args:
        document_id:     e.g. "AI-CHK-A.2.2-4"
        control_id:      e.g. "A.2.2-4"
        control_name:    e.g. "AI Policy Framework"
        source_policy:   e.g. "AI-POL-A.2.2-4"
        requirements:    OrderedDict of domain_name -> [(req_id, section, text), ...]
        output_filename: override output path (default: auto-generated)
        iso_standard:    ISO standard string (default: "ISO/IEC 42001:2023")
