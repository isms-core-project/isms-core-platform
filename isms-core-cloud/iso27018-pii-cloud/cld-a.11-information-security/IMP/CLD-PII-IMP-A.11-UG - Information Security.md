<!-- ISMS-CORE:IMP:CLD-PII-IMP-A.11-UG:cloud:UG:a.11 -->
**CLD-PII-IMP-A.11-UG — Information Security — User Guide**
...
| **Document ID** | CLD-PII-IMP-A.11-UG |
| **Related Policy** | CLD-PII-POL-A.11 (Information Security) |
...
- CLD-PII-POL-A.11 (Information Security — the governing policy)
- CLD-PII-IMP-A.11-TG (Information Security — Technical Guide)
- CLD-PII-POL-A.10 (Accountability — breach notification triggered by security incidents)
- CLD-PII-POL-A.5 (Data Minimisation — temporary file erasure alignment)
- CLD-PII-POL-A.8 (Openness, Transparency — sub-processor disclosure)
...
Loss or theft of a portable device potentially containing PII: report to the CISO and DPO immediately. The event will be treated as a PII security incident under CLD-PII-POL-A.10.1.
...
Cloud Engineering reviews and documents cipher suite configurations annually. Weak ciphers (RC4, DES, 3DES, SSL 3.0, TLS 1.0, TLS 1.1) must be disabled. See CLD-PII-IMP-A.11-TG for the encryption specification and current approved cipher suites.
...
2. Log the restoration in the backup restoration log (see CLD-PII-IMP-A.11-TG) before starting
...
- Security obligations (GDPR Article 32 alignment, reference to CLD-PII-POL-A.11 controls)
- Breach notification (24-hour controller notification per CLD-PII-POL-A.10.1)
- Data subject rights assistance (CLD-PII-POL-A.2.1 and CLD-PII-POL-A.9)
- Audit rights for the controller or appointed auditor
- Sub-processor approval and notification obligations
- PII return or deletion on termination (CLD-PII-POL-A.10.3)
...
Sub-processors must be bound by a written contract imposing obligations equivalent to the full CLD-PII-POL-A.X suite. Key requirements that must appear in every sub-processor agreement:
