...
- CLD-PII-POL-A.5 (Data Minimisation — temporary file erasure)
- CLD-PII-POL-A.8 (Openness, Transparency — sub-processor disclosure)
- CLD-PII-POL-A.10 (Accountability — breach notification)
...
**Loss or theft** of any portable device potentially containing PII SHALL be reported to the CISO and DPO immediately and treated as a PII security incident under CLD-PII-POL-A.10.1 and ISMS-POL-A.5.24-28.
...
- Breach notification requirements (controller notification within 24 hours per CLD-PII-POL-A.10.1)
- Data subject rights assistance obligations (per CLD-PII-POL-A.2.1 and CLD-PII-POL-A.9)
...
- Sub-processor approval requirements (per CLD-PII-POL-A.8.1)
- PII return or deletion upon termination (per CLD-PII-POL-A.10.3)
...
[Organisation] SHALL impose on all sub-processors, via binding contract, **obligations equivalent** to those in this policy and the full CLD-PII-POL-A.X suite. Sub-processor contracts SHALL:
...
- Require breach notification to [Organisation] within 12 hours of detection (enabling [Organisation]'s 24-hour controller notification obligation under CLD-PII-POL-A.10.1) — this 12-hour requirement is a **mandatory clause** in all sub-processor agreements; Legal/Compliance maintains the standard sub-processor agreement template
...
Auditors verifying compliance with CLD-PII-POL-A.11 should expect to find:
