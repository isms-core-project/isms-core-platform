...
**CLD-PII-IMP-A.6-UG — Use, Retention and Disclosure Limitation — User Guide**
...
| **Document ID** | CLD-PII-IMP-A.6-UG |
| **Related Policy** | CLD-PII-POL-A.6 (Use, Retention and Disclosure Limitation) |
...
- CLD-PII-POL-A.6 (Use, Retention and Disclosure Limitation — the governing policy)
- CLD-PII-IMP-A.6-TG (Use, Retention and Disclosure Limitation — Technical Guide)
- CLD-PII-POL-A.3 (Purpose Legitimacy — authorised processing purposes per service agreement)
- CLD-PII-POL-A.10 (Accountability — return and disposal of PII on contract termination)
- CLD-PII-POL-A.5 (Data Minimisation — retention limits for temporary files)
...
Automated retention configuration settings per controller are recorded in the Retention Configuration Record (see CLD-PII-IMP-A.6-TG). Cloud Service Delivery confirms configuration to the controller in writing following onboarding.
...
The DPO completes the PII Disclosure Register entry (see CLD-PII-IMP-A.6-TG for schema), recording all mandatory fields. The register entry is closed as complete once all post-disclosure notifications to the controller are confirmed.
...
The warrant canary is a published statement affirming that [Organisation] has not received any orders of a type that would be prohibited from disclosure. Removal or non-renewal of the canary serves as an indirect signal. The warrant canary process and template are maintained in CLD-PII-IMP-A.6-TG.
