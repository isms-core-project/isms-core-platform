...
**CLD-PII-IMP-A.9-UG — Individual Participation and Access — User Guide**
...
| **Document ID** | CLD-PII-IMP-A.9-UG |
| **Related Policy** | CLD-PII-POL-A.9 (Individual Participation and Access) |
...
- CLD-PII-POL-A.9 (Individual Participation and Access — the governing policy)
- CLD-PII-IMP-A.9-TG (Individual Participation and Access — Technical Guide)
- CLD-PII-POL-A.2 (Consent and Choice — co-operation on data subject rights)
- CLD-PII-POL-A.7 (Accuracy and Quality — rectification capability)
- CLD-PII-POL-A.10 (Accountability — breach notification and end-of-contract obligations)
...
All incoming requests SHALL be logged in the **Data Subject Rights Request Log** (see CLD-PII-IMP-A.9-TG for the log schema) within **1 business hour** of receipt.
...
2. State the expected completion timeframe (per CLD-PII-POL-A.9 response timeframes)
...
4. Deliver the export to the controller in the agreed format (JSON or CSV; see CLD-PII-IMP-A.9-TG for the access response package format)
...
3. Log the gap as a remediation item for the annual capability testing programme (per CLD-PII-POL-A.9)
...
- [ ] Annual capability test results on file for all rights types (per CLD-PII-POL-A.9)
