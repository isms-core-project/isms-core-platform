<!-- ISMS-CORE:IMP:CLD-PII-IMP-A.1-UG:cloud:UG:a.1 -->
**CLD-PII-IMP-A.1-UG — General — User Guide**
...
| **Document ID** | CLD-PII-IMP-A.1-UG |
| **Related Policy** | CLD-PII-POL-A.1 (General) |
...
- CLD-PII-POL-A.1 (General — the governing policy)
- CLD-PII-IMP-A.1-TG (General — Technical Guide)
- CLD-PII-POL-A.10 (Accountability)
- CLD-PII-POL-A.11 (Information Security)
- CLD-PII-POL-A.12 (Privacy Compliance)
...
2. **Use the standard processor agreement template.** Legal/Compliance uses the template maintained in the Processor Agreement Register (see CLD-PII-IMP-A.1-TG, Section 1). Custom terms requested by the controller must be reviewed by Legal/Compliance and approved by the DPO before acceptance.
3. **Complete the processing description schedule.** The agreement must include a completed schedule specifying: categories of PII, processing purposes, processing operations, processing duration, and sub-processor arrangements. See CLD-PII-IMP-A.1-TG, Section 2 for the schedule template.
4. **Obtain DPO sign-off.** The DPO reviews the completed processing description schedule for compliance with CLD-PII-POL-A.3 (purpose restriction) and CLD-PII-POL-A.4 (collection limitation) before the agreement is executed.
...
- When an agreement expires or terminates, Legal/Compliance records the termination date and confirms with Cloud Service Delivery that PII return or deletion has been completed per CLD-PII-POL-A.11.11.
...
CLD-PII-POL-A.1 requires [Organisation] to document how each applicable Annex A control is addressed within its services. This documentation (the Control Implementation Documentation) is the artefact that demonstrates ISO/IEC 27018:2025 compliance to controllers and auditors.

**Procedure — producing and maintaining the Control Implementation Documentation:**

1. The CISO maintains a control mapping document that links each CLD-PII-POL-A.X policy to the service feature, process, or technical control that implements it.
...
4. **Record the instruction.** All instruction changes are recorded in the Instruction Change Log (see CLD-PII-IMP-A.1-TG, Section 3) with the date received, the change description, the authorising contact at the controller, and the DPO assessment outcome.
...
5. The event is recorded in the Legally Compelled Processing Record (see CLD-PII-IMP-A.1-TG, Section 4).
...
The CISO reviews sub-processor compliance at least annually. If a sub-processor suffers a security incident or fails to meet its obligations, the CISO notifies the DPO and Legal/Compliance, who assess the impact on controller obligations and initiate appropriate remedial action. Controllers are notified in accordance with CLD-PII-POL-A.11 (Information Security) incident notification obligations.
