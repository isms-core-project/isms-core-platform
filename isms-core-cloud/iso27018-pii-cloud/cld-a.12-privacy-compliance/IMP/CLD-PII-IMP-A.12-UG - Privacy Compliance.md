...
| **Document ID** | CLD-PII-IMP-A.12-UG |
...
- CLD-PII-POL-A.8 (Openness, Transparency — sub-processor disclosure cross-reference)
- CLD-PII-POL-A.11.12 (Sub-contracted PII Processing — sub-processor location obligations)
- CLD-PII-POL-A.1 (General — foundational processor obligations)
...
The review SHALL be completed within the first quarter of each calendar year (or within 12 months of the prior review). The output is a **Compliance Self-Assessment Report** (structure in CLD-PII-IMP-A.12-TG).
...
4. The Transfer Destination Records (CLD-PII-IMP-A.12-TG) are updated with the new mechanism within **5 business days** of implementation
...
Use the checklist in CLD-PII-IMP-A.12-TG (Audit Preparation Checklist) before each scheduled or unscheduled controller audit. At a minimum, verify that the following are current and available for inspection:
...
4. CLD-PII-POL-A.X policy set (all 12 policies, current versions)
5. ISO 27018:2025 Annex A Compliance Self-Assessment Report (current year)
6. Processor RoPA entry for the requesting controller (per CLD-PII-IMP-A.10-TG)
