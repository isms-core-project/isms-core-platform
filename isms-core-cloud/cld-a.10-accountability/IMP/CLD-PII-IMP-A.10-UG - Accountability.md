<!-- ISMS-CORE:IMP:CLD-PII-IMP-A.10-UG:cloud:UG:a.10 -->
**CLD-PII-IMP-A.10-UG — Accountability — User Guide**
...
| **Document ID** | CLD-PII-IMP-A.10-UG |
| **Related Policy** | CLD-PII-POL-A.10 (Accountability) |
...
- CLD-PII-POL-A.10 (Accountability — the governing policy)
- CLD-PII-IMP-A.10-TG (Accountability — Technical Guide)
- CLD-PII-POL-A.11 (Information Security — security controls underpinning accountability)
- CLD-PII-POL-A.6 (Use, Retention and Disclosure Limitation — disclosure log cross-reference)
- CLD-PII-POL-A.9 (Individual Participation and Access — data subject rights support)
...
[Organisation] SHALL maintain a **Processor Records of Processing Activities (RoPA)** capturing all processing performed on behalf of each PII controller. The RoPA covers the mandatory Article 30(2) GDPR fields and additional ISO 27018 fields. See CLD-PII-IMP-A.10-TG for the full schema.
...
- A change to the geographic location of processing (which also triggers CLD-PII-POL-A.12.1 notification)
...
3. Document retention compliance — verification that all CLD-PII-POL-A.X policy versions and processing records are retained per the schedule in CLD-PII-POL-A.10.2
...
The annual review output is a short **Accountability Review Report** (template in CLD-PII-IMP-A.10-TG). The report is retained for 5 years.
...
**Step 3 — Draft notification** (within 12 hours). Use the breach notification template in CLD-PII-IMP-A.10-TG. Include all available information; mark fields as "pending investigation" where facts are not yet confirmed.
...
4. Issue a **written certificate of destruction** to the controller within **30 calendar days** of termination (see template in CLD-PII-IMP-A.10-TG)
...
- [ ] Document retention schedule is enforced per CLD-PII-POL-A.10.2 (5-year minimum for policies and processing records)
