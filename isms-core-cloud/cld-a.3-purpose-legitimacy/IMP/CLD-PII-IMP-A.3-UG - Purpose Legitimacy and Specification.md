<!-- ISMS-CORE:IMP:CLD-PII-IMP-A.3-UG:cloud:UG:a.3 -->
**CLD-PII-IMP-A.3-UG — Purpose Legitimacy and Specification — User Guide**
...
| **Document ID** | CLD-PII-IMP-A.3-UG |
| **Related Policy** | CLD-PII-POL-A.3 (Purpose Legitimacy and Specification) |
...
- CLD-PII-POL-A.3 (Purpose Legitimacy and Specification — the governing policy)
- CLD-PII-IMP-A.3-TG (Purpose Legitimacy and Specification — Technical Guide)
- CLD-PII-POL-A.2 (Consent and Choice)
- CLD-PII-POL-A.4 (Collection Limitation)
- CLD-PII-POL-A.1 (General)
...
The pre-contract purpose review is completed using the Purpose Scope Review Checklist (see CLD-PII-IMP-A.3-TG, Section 1). Legal/Compliance initiates the review; the DPO makes the final determination.
...
This confirmation is recorded in the processing description schedule (see CLD-PII-IMP-A.1-TG, Section 2).
...
3. **Technical correction.** If the drift is due to a misconfiguration or unintended routing, Cloud Engineering implements a technical correction. The event is recorded in the Processing Instruction Change Log (see CLD-PII-IMP-A.3-TG, Section 2).
4. **New purpose authorisation.** If the proposed processing genuinely requires a new purpose, Legal/Compliance obtains written controller authorisation before processing continues. See CLD-PII-POL-A.3 for the new purpose authorisation requirements.
...
- Instructions require [Organisation] to transfer PII to a jurisdiction without adequate data protection safeguards, in breach of GDPR Chapter V or CH FADP transfer requirements, and without the safeguards required by CLD-PII-POL-A.12
...
5. **Record the escalation.** All unlawful instruction escalation events are recorded in the Unlawful Instruction Escalation Record (see CLD-PII-IMP-A.3-TG, Section 3) regardless of outcome.
