...
- Technical anonymisation and erasure methods for temporary files — addressed in CLD-PII-POL-A.5
...
[Organisation] SHOULD use anonymised or synthetic data in non-production environments wherever technically feasible, and SHALL treat production PII use as a last resort. PII from production environments SHALL NOT be used in development, testing, or staging environments without explicit written authorisation from the PII controller. Where controller authorisation is obtained, the minimum necessary PII SHALL be used, the same security controls applicable in production SHALL apply (in accordance with CLD-PII-POL-A.11), and access SHALL be restricted to the minimum personnel required for the specific purpose.
...
Auditors verifying compliance with CLD-PII-POL-A.4 should expect to find:
