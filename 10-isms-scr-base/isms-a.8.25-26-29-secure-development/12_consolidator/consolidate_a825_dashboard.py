...
# =============================================================================
"""
ISMS-A.8.25-26-29 Secure Development - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('A.8.25-26-29.1', 'Security_Requirements', 'Security Requirements'),
    ('A.8.25-26-29.2', 'SDLC_Security', 'SDLC Security'),
    ('A.8.25-26-29.3', 'Security_Testing', 'Security Testing'),
    ('A.8.25-26-29.4', 'Vulnerability_Remediation', 'Vulnerability Remediation'),
]

DASHBOARD_PATTERNS = ['A.8.25-26-29.5', 'Secure_Development_Dashboard']
...
def extract_gaps_from_workbook(filepath, assessment_area, gap_counter_start=1):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'issue', 'vulnerability']):
                ws = wb[sheet_name]
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    for val in row_values:
                        if val:
                            val_str = str(val)
                            if any(x in val_str for x in ['⚠️', '❌', 'Non-Compliant', 'Partial', 'Critical', 'High']):
                                status = val_str
                            elif len(val_str) > 30:
                                gap_desc = val_str
                    if status or gap_desc:
                        severity = 'High' if status and ('❌' in str(status) or 'Critical' in str(status)) else 'Medium'
                        gaps.append([f'GAP-DEV-{gap_counter:03d}', assessment_area,
                            os.path.basename(filepath), row_values[0] or 'Application',
...
def generate_risks_and_remediation(gaps):
    ...
    for i, gap in enumerate([g for g in gaps if g[7] == 'High'], 1):
        risks.append([f'RISK-{i:03d}', gap[0], 'Secure Development', f'Risk: {gap[4][:80]}',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.8.25-26-29 Secure Development - Dashboard Consolidation")
    print("=" * 80)
