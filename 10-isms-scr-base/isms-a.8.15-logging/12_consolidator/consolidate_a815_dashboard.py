...
# =============================================================================
"""
ISMS-A.8.15 Logging - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('A.8.15.1', 'Log_Source_Inventory', 'Log Source Inventory'),
    ('A.8.15.2', 'Log_Collection', 'Log Collection'),
    ('A.8.15.3', 'Log_Protection', 'Log Protection'),
    ('A.8.15.4', 'Log_Analysis', 'Log Analysis'),
]

DASHBOARD_PATTERNS = ['A.8.15.5', 'Compliance_Dashboard']
            ...
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'issue']):
                ...
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    if status or gap_desc:
                        ...
                        gaps.append([f'GAP-LOG-{gap_counter:03d}', assessment_area,
                            os.path.basename(filepath), row_values[0] or 'Log Source',
...
def generate_risks_and_remediation(gaps):
    ...
    for i, gap in enumerate([g for g in gaps if g[7] == 'High'], 1):
        risks.append([f'RISK-{i:03d}', gap[0], 'Logging', f'Risk: {gap[4][:80]}',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.8.15 Logging - Dashboard Consolidation")
    print("=" * 80)
