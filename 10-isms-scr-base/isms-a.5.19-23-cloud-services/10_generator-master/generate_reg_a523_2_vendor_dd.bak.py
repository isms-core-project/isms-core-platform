...
# ============================================================================
# REGULATORY DROPDOWN CONSTANTS (DORA, NIS2, AI Act, CLOUD Act) - NEW
# ============================================================================
...
def create_workbook() -> Workbook:
    ...
    # Sheet structure - v2.0 includes Jurisdictional Risk
    sheets = [
        "Instructions & Legend",
...
def get_extended_columns_contracts() -> dict:
    """Extended columns R-X for contract terms (UPDATED with DORA/NIS2)."""
    return {
...
def create_extended_validations_contracts(ws) -> dict:
    """Validations for contract terms extended columns (UPDATED with DORA/NIS2)."""
    v = {}
...
# --- END BLOCK 1 ---
print("Block 1 loaded: Foundation (Sections 1-3)")

# ============================================================================
# SECTION 4: CHECKLIST DEFINITIONS (UPDATED WITH DORA/NIS2/AI Act)
# ============================================================================
...
# ============================================================================
# SECTION 5: INSTRUCTIONS & LEGEND SHEET (UPDATED WITH REGULATORY INFO)
# ============================================================================
...
def create_instructions_sheet(ws, styles):
    ...
    ws["A1"] = "ISMS-IMP-A.5.23.S2 — Vendor Due Diligence & Contracts"
    ...
    doc_info = [
        ...
        ("Version:", "2.0 (with DORA/NIS2/CLOUD Act)"),
        ("Assessment Date:", ""),
        ("Completed By:", ""),
        ("Organization:", ""),
    ...
    legend = [
        (f"{CHECK} Compliant", "Vendor meets all security and contractual requirements", "C6EFCE"),
        (f"{WARNING} Partial", "Some requirements met, gaps exist (e.g., cert pending)", "FFEB9C"),
        (f"{XMARK} Non-Compliant", "Vendor does not meet minimum requirements", "FFC7CE"),
        ("N/A", "Requirement does not apply to this vendor/service", "D9D9D9"),
    ]
    ...
    evidence_types = [
        ...
        "✓ Jurisdictional risk assessment (for US-nexus providers)",  # NEW
    ]
    row += 1
...
def create_vendor_assessment_sheet(ws, styles, section_title, policy_req, 
    ...
    if sheet_type == "certifications":
        ...
        ext_vals['dora_compliance'].add("Y5:Y24")  # NEW
        ext_vals['nis2_clause'].add("Z5:Z24")      # NEW
    ...
    if checklist_items:
        ...
        status_dv = DataValidation(type="list", formula1='f"{CHECK},f"{WARNING}",f"{XMARK}",N/A"', allow_blank=True)
...
# --- END BLOCK 2 ---
print("Block 2 loaded: Checklists, Instructions, Generic Engine (Sections 4-6)")
...
def create_3_contract_terms(ws, styles):
    """Sheet 3: Contract Terms Analysis (UPDATED with DORA/NIS2)."""
    create_vendor_assessment_sheet(
        ws=ws, styles=styles,
...
def create_5_data_sovereignty(ws, styles):
    ...
    create_vendor_assessment_sheet(
        ...
        policy_req="Policy Requirement: Vendor data processing locations must comply with organizational data residency requirements (Policy S2 Section 3.4)",
...
# ============================================================================
# SECTION 8: NEW - JURISDICTIONAL RISK ASSESSMENT SHEET (CLOUD Act)
# ============================================================================
...
# ============================================================================
# SECTION 8 (continued): SUMMARY DASHBOARD (UPDATED with Jurisdictional Metrics)
# ============================================================================

def create_8_summary_dashboard(ws, styles):
    """Sheet 8: Summary Dashboard with compliance metrics (UPDATED with Jurisdictional Risk)."""
    ...
    # Assessment areas with formula references (UPDATED sheet references)
    areas = [
        ...
        ("Jurisdictional Risk", "'7. Jurisdictional Risk'", "N"),  # NEW - uses Risk Level column
    ]
    ...
    for label, sheet, status_col in areas:
        ...
        if label == "Jurisdictional Risk":
            ...
            ws.cell(row=row, column=3, value=f'=COUNTIF({sheet}!{status_col}5:{status_col}24,f"{CHECK}*")')
            ws.cell(row=row, column=4, value=f'=COUNTIF({sheet}!{status_col}5:{status_col}24,f"{WARNING}*")')
            ws.cell(row=row, column=5, value=f'=COUNTIF({sheet}!{status_col}5:{status_col}24,f"{XMARK}*")')
    ...
    ws[f"A{row}"] = "TABLE 2: JURISDICTIONAL & CLOUD ACT RISK SUMMARY (NEW)"
    ...
    for metric, formula in jur_metrics:
        ...
        ws.cell(row=row, column=3, value=f'=IF(B{row}>0,f"{WARNING} Review Required",f"{CHECK} OK")')
        row += 1
...
# --- END BLOCK 3 ---
print("Block 3 loaded: Individual Sheet Creators + Jurisdictional Risk + Dashboard (Sections 7-8)")
...
def create_9_evidence_register(ws, styles):
    ...
    # Evidence Type dropdown (UPDATED to include Jurisdictional)
    ...
    for i in range(1, 101):
        ...
        ws.cell(row=data_row, column=1).font = Font(color="666666")
    ...
    checklist = [
        ...
        "☐ Jurisdictional risk assessments documented for US-nexus providers",  # NEW
        "☐ Risk acceptance forms signed for high-risk vendors",  # NEW
    ]
...
def create_10_approval_signoff(ws, styles):
    ...
    completion_fields = [
        ...
        ("Jurisdictional Risks Identified:", "='8. Summary Dashboard'!B20", "formula"),  # NEW
    ...
    # DATA PROTECTION OFFICER REVIEW (NEW)
    row += 2
...
def main():
    ...
    print("=" * 70)
    print("ISMS-IMP-A.5.23.S2 - Vendor Due Diligence & Contracts Generator")
    print("Version 2.0 - WITH REGULATORY UPDATES (DORA/NIS2/AI Act/CLOUD Act)")
    ...
    print("[1/10] Creating Instructions & Legend sheet...")
    create_instructions_sheet(wb["Instructions & Legend"], styles)
    
    print("[2/10] Creating 2. Security Certifications sheet...")
    create_2_security_certifications(wb["2. Security Certifications"], styles)
    
    print("[3/10] Creating 3. Contract Terms sheet (with DORA/NIS2)...")
    create_3_contract_terms(wb["3. Contract Terms"], styles)
    
    print("[4/10] Creating 4. SLA Performance sheet...")
    create_4_sla_performance(wb["4. SLA Performance"], styles)
    
    print("[5/10] Creating 5. Data Sovereignty sheet...")
    create_5_data_sovereignty(wb["5. Data Sovereignty"], styles)
    
    print("[6/10] Creating 6. Forensics & Audit sheet...")
    ...
    print("[8/10] Creating 8. Summary Dashboard sheet...")
    create_8_summary_dashboard(wb["8. Summary Dashboard"], styles)
    
    print("[9/10] Creating 9. Evidence Register sheet...")
    create_9_evidence_register(wb["9. Evidence Register"], styles)
    
    print("[10/10] Creating 10. Approval Sign-Off sheet...")
    ...
    print("  1.  Instructions & Legend (with regulatory guidance)")
    print("  2.  Security Certifications (ISO 27001, SOC 2, FedRAMP)")
    print("  3.  Contract Terms (DPA, liability, DORA Art.30, NIS2)")
    print("  4.  SLA Performance (availability, support, incidents)")
    print("  5.  Data Sovereignty (GDPR, nFADP, SCCs, regions)")
    print("  6.  Forensics & Audit (audit rights, IR, breach notification)")
    print("  7.  Jurisdictional Risk (NEW - CLOUD Act assessment)")
    print("  8.  Summary Dashboard (compliance metrics + jurisdictional KPIs)")
    print("  9.  Evidence Register (contract & cert tracking)")
    print("  10. Approval Sign-Off (Legal, Procurement, DPO, CISO)")
    print("\n🔍 NEW in v2.0:")
    print("  • DORA Article 30 contract clause checklist")
    print("  • NIS2 supply chain security requirements")
    print("  • CLOUD Act jurisdictional risk assessment")
    print("  • EU AI Act evaluation criteria")
    print("  • DPO sign-off section")
    print("\nNext steps:")
    print(f"  1. Run validation: python3 excel_sanity_check_a523.py {filename}")
    print("  2. Open in Excel and verify dropdowns work")
    print("  3. Distribute to stakeholders for completion")
    print("  4. Focus on Sheet 7 for US-nexus cloud providers")
...
# --- END BLOCK 4 ---
print("Block 4 loaded: Evidence Register, Approval Sign-Off, Main Execution (Sections 9-11)")
print("\n" + "=" * 70)
print("ALL BLOCKS LOADED - Script ready to execute!")
print("=" * 70)