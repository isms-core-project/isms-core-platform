...
def create_workbook() -> Workbook:
    ...
    # Sheet structure matches ISMS-IMP-A.8.4.1 specification
    sheets = [
        "Instructions & Legend",
...
def create_instructions_sheet(wb, styles):
    ...
    row += 1
    legend_data = [
        (f"{CHECK}", "Appropriate", "Access is justified and appropriate", "Green"),
        (f"{WARNING}", "Excessive", "User has higher access than needed", "Yellow"),
        (f"{XMARK}", "Orphaned", "Former employee still has access", "Red"),
...
def create_repository_inventory_sheet(wb, styles, validations):
    ...
    ws[f'I{row}'] = datetime.now().strftime("%Y-%m-%d")
    ws[f'J{row}'] = "Active"
    ...
    ws[f'S{row}'] = datetime.now().strftime("%Y-%m-%d")
...
def create_user_access_matrix_sheet(wb, styles, validations):
    ...
    ws[f'N{row}'] = (datetime.now() - timedelta(days=15)).strftime("%Y-%m-%d")
    ws[f'O{row}'] = f"{CHECK} Appropriate"
    ws[f'P{row}'] = datetime.now().strftime("%Y-%m-%d")
...
def create_access_approval_records_sheet(wb, styles, validations):
    ...
    ws[f'L{row}'] = datetime.now().strftime("%Y-%m-%d")
    ws[f'M{row}'] = ""
    ws[f'N{row}'] = f"{MINUS} N/A"
    ws[f'O{row}'] = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
...
def create_access_review_log_sheet(wb, styles, validations):
    ...
    ws[f'P{row}'] = (datetime.now() + timedelta(days=7)).strftime("%Y-%m-%d")
    ws[f'Q{row}'] = "=P4-A4"  # Formula: Days to complete
    ws[f'R{row}'] = (datetime.now() + timedelta(days=90)).strftime("%Y-%m-%d")
...
def create_deprovisioning_log_sheet(wb, styles, validations):
    ...
    ws[f'D{row}'] = (datetime.now() - timedelta(days=2)).strftime("%Y-%m-%d")
    ws[f'E{row}'] = (datetime.now() - timedelta(days=2)).strftime("%Y-%m-%d")
    ws[f'F{row}'] = "[Example: customer-portal]"
    ws[f'G{row}'] = "Write/Contribute"
    ws[f'H{row}'] = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    ws[f'M{row}'] = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
...
def create_compliance_scoring_sheet(wb, styles):
    ...
    metrics = [
        ...
        ("Access Control Compliance", 
         "=COUNTIFS(Repository_Inventory!Q:Q,f\"{CHECK} Yes\")/COUNTA(Repository_Inventory!Q:Q)*100", 
         "100%", "", "20%"),
        ("Appropriate Access Rate", 
         "=COUNTIFS(User_Access_Matrix!O:O,f\"{CHECK} Appropriate\")/COUNTA(User_Access_Matrix!O:O)*100", 
         "≥95%", "", "25%"),
        ("Orphaned Account Rate", 
         "=COUNTIFS(User_Access_Matrix!O:O,f\"{XMARK} Orphaned\")/COUNTA(User_Access_Matrix!O:O)*100", 
         "0%", "", "15%"),
        ("Access Review Completion", 
         "=COUNTIFS(Access_Review_Log!S:S,f\"{CHECK} Compliant\")/COUNTA(Access_Review_Log!S:S)*100", 
         "100%", "", "15%"),
        ("Deprovisioning SLA Compliance", 
         "=COUNTIFS(Deprovisioning_Log!N:N,f\"{CHECK} Yes\")/COUNTA(Deprovisioning_Log!N:N)*100", 
         "≥95%", "", "10%"),
    ]
...
def create_gap_analysis_sheet(wb, styles, validations):
    ...
    ws[f'M{row}'] = (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d")
    ws[f'N{row}'] = "1 week"
...
def create_evidence_register_sheet(wb, styles):
    ...
    ws[f'J{row}'] = datetime.now().strftime("%Y-%m-%d")
    ws[f'K{row}'] = "CSV"
    ws[f'L{row}'] = "3 years"
    ws[f'M{row}'] = (datetime.now() + timedelta(days=1095)).strftime("%Y-%m-%d")
...
def main():
    ...
    print("Starting ISMS-A84-1 Repository Access Assessment Workbook Generation...")
    ...
    print(f"{CHART} Sheets created: {len(wb.sheetnames)}")
