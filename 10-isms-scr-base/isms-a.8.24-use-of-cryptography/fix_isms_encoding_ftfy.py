...
Usage:
    ...
    python3 fix_isms_encoding_ftfy.py --recursive ./10_generator_utf    # Scan current directory recursively
...
Use --no-backup to skip (NOT RECOMMENDED for production files).
        ...
        # Banner
        logger.info("=" * 80)
        logger.info("ISMS Encoding Fix Utility (ftfy-powered)")
        logger.info("Automatically detects and repairs UTF-8 mojibake")
        logger.info("=" * 80)
            ...
            if args.verbose or (not args.recursive and len(all_files) < 20):
                if result['success']:
                    if result['changed']:
                        ...
                        logger.info(f"{status}{backup_info}")
        ...
        logger.info(f"Already correct: {successful - fixed_count}")
        ...
        if args.dry_run and fixed_count > 0:
            logger.info(f"Run without --dry-run to fix {fixed_count} file(s)")
        elif fixed_count > 0 and not args.dry_run:
            logger.info(f"Successfully fixed {fixed_count} file(s)!")
            if backed_up > 0:
                logger.info("To restore originals: for f in *.bak; do cp \"$f\" \"${f%.bak}\"; done")
        elif fixed_count == 0:
            logger.info("All files have correct encoding!")
