...
def add_data_transmission_samples(filepath):
    ...
    sheet_data = {
        "1.1 External HTTPS-TLS": [
            ...
            ["mail.company.com", "TLS 1.2", "GoDaddy", "Expires 2026-02-28", "⚠️ Partial", "Cert renew scheduled", "Upgrade TLS 1.3", "Yes"],
            ...
            ["vpn.company.com", "TLS 1.2", "Internal CA", "Valid 2025-12-31", "⚠️ Partial", "Internal CA", "Plan upgrade", "Yes"],
            ["test.company.com", "TLS 1.2", "Self-signed", "Valid 2026-03-01", "⚠️ Partial", "Test env", "Use valid cert", "Yes"],
        ...
        "2.1 Email Encryption": [
            ["Microsoft 365", "TLS 1.3", "S/MIME + TLS", "85% adoption", "⚠️ Partial", "M365 admin", "Increase adoption", "Yes"],
            ["Gmail Business", "TLS 1.3", "TLS only", "100%", "✅ Compliant", "Google Workspace", "", "No"],
            ["Internal Mail", "TLS 1.2", "PGP + TLS", "45% adoption", "⚠️ Partial", "Postfix logs", "Promote PGP", "Yes"],
        ...
        "4.1 VPN": [
            ...
            ["Site-to-Site VPN", "IPSec", "IPSec IKEv2", "AES-256-CBC", "⚠️ Partial", "Firewall logs", "Add MFA", "Yes"],
...
def add_data_storage_samples(filepath):
    ...
    sheet_data = {
        "1. Mobile Devices": [
            ...
            ["Tablets", "Internal", "Device encryption", "Software", "AES-256", "MDM managed", "Yes", "⚠️ Partial", "Compliance reports", "Enforce 100%", "Yes"],
            ["BYOD iPhones", "Internal", "Containerization", "Software", "AES-256", "Work data only", "Yes", "⚠️ Partial", "BYOD policy", "Increase enrollment", "Yes"],
            ["BYOD Android", "Internal", "Containerization", "Software", "AES-256", "Work data only", "Yes", "⚠️ Partial", "BYOD policy", "Training needed", "Yes"],
        ...
        "2. Laptops & Workstations": [
            ...
            ["Linux Workstations", "Internal", "LUKS", "Full disk", "AES-256", "Manual backup", "Annual", "⚠️ Partial", "Asset inventory", "Enforce 100%", "Yes"],
        ...
        "4. Databases": [
            ...
            ["MongoDB Analytics", "Internal", "At Rest", "Encrypted storage", "AES-256-GCM", "Self-managed", "No", "⚠️ Partial", "MongoDB config", "Use KMS", "Yes"],
            ["Redis Cache", "Internal", "None", "None", "None", "N/A", "No", "❌ Non-Compliant", "Redis config", "Add encryption", "Yes"],
            ["Oracle Legacy", "Confidential", "TDE", "Transparent", "AES-192", "Manual", "No", "⚠️ Partial", "Oracle audit", "Upgrade AES-256", "Yes"],
        ],
        "5. Cloud Storage": [
...
def add_authentication_samples(filepath):
    ...
    sheet_data = {
        "1. Password Security": [
            ...
            ["CMS Backend", "PBKDF2-SHA256", "Admin", "Confidential", "PBKDF2-SHA256", "310k iterations", "Medium", "⚠️ Partial", "CMS config", "Upgrade to 600k", "Yes"],
        ...
        "2. Multi-Factor Authentication": [
            ...
            ["Contractors", "SMS + Authenticator", "Contractor", "N/A", "SMS/TOTP", "88%", "105/120", "⚠️ Partial", "Contractor reports", "Mandate for all", "Yes"],
...
def add_key_management_samples(filepath):
    ...
    sheet_data = {
        ...
        "2. Key Storage": [
            ...
            ["SSH Keys", "SSH key", "Ed25519", "256", "~/.ssh/", "Internal", "Filesystem", "⚠️ Partial", "Endpoint scan", "Add passphrases", "Yes"],
        ...
        "3. Key Rotation": [
            ...
            ["DB Encryption", "DEK", "AES", "256", "Manual process", "Restricted", "365 days", "⚠️ Partial", "Rotation schedule", "Automate", "Yes"],
        ...
        "4. Key Destruction": [
            ...
            ["Legacy SSH Keys", "SSH key", "RSA", "2048", "Manual deletion", "Internal", "Should destroy", "⚠️ Partial", "Key inventory", "Verify destruction", "Yes"],
...
def main():
    print("=" * 80)
    print("POPULATING A.8.24 WORKBOOKS - MERGED & ENHANCED VERSION")
    print("=" * 80)
    ...
    for name, (filepath, func) in files.items():
        print(f"\n[{list(files.keys()).index(name) + 1}/4] Processing {name}...")
        try:
            count = func(filepath)
            total_entries += count
            results[name] = count
            print(f"  ✅ Added {count} sample entries total")
        except Exception as e:
            print(f"  ⚠️  Error: {e}")
            import traceback
            traceback.print_exc()
    ...
    print("\n" + "=" * 80)
    print("✅ MERGED DATA POPULATION COMPLETE!")
    print("=" * 80)
    ...
    print("\nData includes:")
    print("  • Mix of ✅ Compliant (~70%), ⚠️ Partial (~20%), ❌ Non-Compliant (~10%)")
    print("  • Realistic system names and configurations")
    print("  • Evidence locations for compliance tracking")
    print("  • Gap descriptions and remediation plans")
    print("  • Multiple assessment categories per workbook")
    
    print("\n✅ ALL STATUS VALUES IN CORRECT COLUMNS!")
    print("✅ Summary Dashboard formulas will now calculate correctly!")
    
    print("\n📁 Workbooks Updated:")
    print("  ✓ ISMS-IMP-A.8.24.1_Data_Transmission_20260113.xlsx")
    print("  ✓ ISMS-IMP-A.8.24.2_Data_Storage_20260113.xlsx")
    print("  ✓ ISMS-IMP-A.8.24.3_Authentication_20260113.xlsx")
    print("  ✓ ISMS-IMP-A.8.24.4_Key_Management_20260113.xlsx")
    
    print("\nNext Steps:")
    print("  1. Open workbooks in Excel to verify data")
    print("  2. Check Summary Dashboard calculations")
    print("  3. Generate consolidated dashboard workbook")
    print("  4. Test external link updates")
    print("  5. Validate compliance percentage calculations")
    
    print("\n🚀 Workbooks ready for dashboard consolidation!")
    print("=" * 80 + "\n")
