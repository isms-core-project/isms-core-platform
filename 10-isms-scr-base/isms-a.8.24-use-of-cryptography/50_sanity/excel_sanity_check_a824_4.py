...
"""
================================================================================
Excel Sanity Checker - A.8.24.4 Key Management Assessment Workbook
================================================================================

SPECIALIZED diagnostic for Key Management assessment workbooks.

**Purpose:**
Domain-specific validation tailored to the unique structure and requirements
of A.8.24.4 assessment workbooks.
...
- Key management-specific validation logic

**When to Use:**
- After generating A.8.24.4 assessment workbook
- Before stakeholder distribution
- When Excel shows repair warnings for this specific domain

**Usage:**
    python3 excel_sanity_check_a824_4.py ISMS-IMP-A.8.24.4_Assessment_YYYYMMDD.xlsx
...
Assessment Domain: 4 of 4 (Key Management)
...
# ============================================================================
# A.8.24.4 SPECIFIC VALIDATION DATA
# ============================================================================
...
# Base columns A-Q (17 columns) - same across all key management sheets
...
# Extended columns R-X (varies by sheet) - 5-7 additional columns
EXTENDED_COLUMNS = {
    "1. Key Generation": 5,  # R-V: 5 generation-specific columns
    "2. Key Storage": 6,  # R-W: 6 storage-specific columns
    "3. Key Rotation": 6,  # R-W: 6 rotation-specific columns
    "4. Key Backup & Recovery": 7,  # R-X: 7 backup-specific columns
    "5. Certificate Management": 7,  # R-X: 7 certificate-specific columns
}
...
def check_workbook_health(filename):
    ...
    # ========================================================================
    # CHECK 0: A.8.24.4 SPECIFIC STRUCTURE VALIDATION
    # ========================================================================
    logger.info("")
    logger.info("=" * 80)
    logger.info("CHECK 0: A.8.24.4 STRUCTURE VALIDATION")
    logger.info("=" * 80)
    ...
    for sheet_name in KEY_MGMT_ASSESSMENT_SHEETS:
                ...
                for cell in row:
                    if cell.value and isinstance(cell.value, str):
                        if any(keyword in cell.value.upper() for keyword in
                               ["APPROVED", "ALGORITHM", "STORAGE", "ROTATION", "BACKUP", "CERTIFICATE", "VALIDITY"]):
    ...
    if "Summary Dashboard" in wb.sheetnames:
        ...
        # Check for expected sections (11 sections)
        expected_sections = [
            "COMPLIANCE SUMMARY",
            "MATURITY",
            "ALGORITHM",
        ...
        if missing_sections:
            ...
            logger.info(f"  Summary Dashboard: All 11 analysis sections found")

    if structure_issues == 0:
        logger.info("  Workbook structure matches A.8.24.4 specification")
    ...
    if not issues_found and not warnings_found:
        ...
        logger.info("The workbook appears structurally sound and matches A.8.24.4 specification.")
        ...
        if structure_issues > 0:
            ...
            logger.info("   - Verify reference tables exist in assessment sheets")
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("A.8.24.4 SPECIFIC NOTES:")
    logger.info("=" * 80)
    logger.info("Expected structure:")
    logger.info("  - 9 sheets total")
    logger.info("  - 5 key management assessment sheets:")
    logger.info("    - Key Generation (17 base + 5 extended = 22 columns, 14 checklist items)")
    logger.info("    - Key Storage (17 base + 6 extended = 23 columns, 17 checklist items)")
    logger.info("    - Key Rotation (17 base + 6 extended = 23 columns, 16 checklist items)")
    logger.info("    - Key Backup & Recovery (17 base + 7 extended = 24 columns, 18 checklist items)")
    logger.info("    - Certificate Management (17 base + 7 extended = 24 columns, 20 checklist items)")
    logger.info("  - Each assessment sheet: 13 data rows, compliance checklist, 1-2 reference tables")
    logger.info("  - Summary Dashboard with 11 comprehensive analysis sections:")
    logger.info("    1. Compliance summary by key management area")
    logger.info("    2. Key management maturity by area")
    logger.info("    3. Algorithm distribution analysis")
    logger.info("    4. Storage method distribution with security ratings")
    logger.info("    5. Key rotation coverage")
    logger.info("    6. Certificate management health")
    logger.info("    7. FIPS 140-2 validation status")
    logger.info("    8. Critical gaps requiring attention (8 rows)")
    logger.info("    9. Risk summary by data classification")
    logger.info("    10. Key backup coverage metrics")
    logger.info("    11. Overall assessment summary with maturity rating")
    logger.info("  - Evidence Register with 100 entry rows")
    logger.info("  - Approval Sign-Off with 3-level workflow")
    logger.info("=" * 80)
...
def main():
    ...
    try:
        if len(sys.argv) < 2:
            logger.error("Usage: python3 excel_sanity_check_a824_4.py <filename.xlsx>")
            logger.info("Example:")
            logger.info("  python3 excel_sanity_check_a824_4.py ISMS-IMP-A.8.24.4_Key_Management_20251231.xlsx")
            sys.exit(1)
