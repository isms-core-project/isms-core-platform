...
Common Unicode -> ASCII conversions:
  ...
  "smart quotes" -> "regular quotes"
        ...
        # Banner
        logger.info("=" * 80)
        logger.info("Unicode to ASCII Conversion Utility (unidecode-powered)")
        logger.info("Automatically transliterates Unicode characters to ASCII equivalents")
        logger.info("=" * 80)

        if args.dry_run:
            logger.info("DRY RUN MODE - No files will be modified")
        elif args.no_backup:
            logger.info("WARNING: Backup disabled - original files will be overwritten!")
        else:
            logger.info("Backup enabled - .bak files will be created")
        ...
        if args.recursive:
            logger.info("Recursively scanned for Python files")

        logger.info(f"Found {len(all_files)} Python file(s) to process")
            ...
            if args.verbose or (not args.recursive and len(all_files) < 20):
                if result['success']:
                    if result['changed']:
                        ...
                        backup_info = f" (backup: {result['backup_path'].name})" if result.get('backup_path') else ""
                        logger.info(f"{status} ({char_count} chars){backup_info}")

                        # Show samples in verbose mode
                        if args.verbose and result['samples']:
                            for sample in result['samples'][:3]:  # Show first 3
                                logger.info(f"    Line {sample['line']}: {sample['before'][:60]}...")
                    else:
                        logger.info("ASCII-ONLY")
                else:
        ...
        logger.info(f"Already ASCII-only: {successful - converted_count}")
        logger.info(f"Total Unicode characters converted: {total_unicode}")

        if backed_up > 0:
            logger.info(f"Backups created: {backed_up} (.bak files)")
        ...
        if args.dry_run and converted_count > 0:
            logger.info(f"Run without --dry-run to convert {converted_count} file(s)")
        elif converted_count > 0 and not args.dry_run:
            logger.info(f"Successfully converted {converted_count} file(s)!")
            if backed_up > 0:
                logger.info("To restore originals: for f in *.bak; do cp \"$f\" \"${f%.bak}\"; done")
        elif converted_count == 0:
            logger.info("All files are already ASCII-only!")
        ...
        logger.info("Anti-cargo-cult ISMS: Evidence-based compliance automation")
        logger.info("=" * 80)
