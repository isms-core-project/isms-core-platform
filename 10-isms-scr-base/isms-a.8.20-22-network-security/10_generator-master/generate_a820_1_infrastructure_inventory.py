...
def create_device_criticality_matrix_sheet(ws, styles):
    ...
    row += 1
    ws.merge_cells(f"A{row}:G{row}")
    ws[f"A{row}"] = "⚠️ IMPORTANCriticality assessment must consider YOUR organization's specific context, not just device type. Always assess based on business impact."
    apply_style(ws[f"A{row}"], styles["info_box"])
    ws.row_dimensions[row].height = 30
