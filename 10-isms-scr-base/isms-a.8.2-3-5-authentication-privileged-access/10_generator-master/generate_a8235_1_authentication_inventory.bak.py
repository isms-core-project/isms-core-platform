...
"""
================================================================================
ISMS-IMP-A.8.2-3-5-1 - Authentication Inventory Assessment Excel Generator
================================================================================
...
Output:
    ISMS-IMP-A.8.2-3-5.1_Authentication_Inventory_Assessment_YYYYMMDD.xlsx
...
DOCUMENT_ID = "ISMS-IMP-A.8.2-3-5.1"
VERSION = "1.0"
CONTROL_REF = "ISO/IEC 27001:2022 A.8.5 (Secure Authentication)"
...
def populate_instructions(wb, styles):
    ...
    # Title
    ws['A1'] = "Authentication Inventory & Methods Assessment"
    ...
    legend = [
        ...
        (f"{WARNING} Partial Compliance", "Authentication mechanism meets some but not all requirements"),
        (f"{XMARK} Non-Compliant", "Authentication mechanism does not meet security requirements"),
...
def populate_sso_integration(wb, styles):
    ...
    ws[f'B{row}'] = f'=COUNTIF(D5:D{4 + SSO_APP_ROW_COUNT},f"{CHECK} Integrated")'
...
def populate_password_policy(wb, styles):
    ...
    dv_compliance = DataValidation(type="list", formula1=f'"{",".join(COMPLIANCE_STATUS)}"', allow_blank=True)
    ws.add_data_validation(dv_compliance)
    dv_compliance.add(f'J5:J{4 + PASSWORD_POLICY_COUNT}')
...
def populate_remaining_sheets(wb, styles):
    ...
    for col_idx, header in enumerate(legacy_headers, start=1):
        cell = ws_legacy.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
...
def generate_workbook():
    ...
    print("║  Workbook 1: Authentication Inventory & Methods Assessment     ║")
    print("║                                                                ║")
    print("║  'Evidence > Theater' - Real authentication security           ║")
    print("╚════════════════════════════════════════════════════════════════╝")
    print()
    ...
    print("Populating System Authentication Inventory (150 systems)...")
    populate_system_inventory(wb, styles)
    
    print("Populating Authentication Protocol Analysis...")
    populate_protocol_analysis(wb, styles)
    
    print("Populating SSO Integration Status...")
    populate_sso_integration(wb, styles)
    
    print("Populating Password Policy Compliance...")
    ...
    # Save workbook
    filename = f"ISMS-IMP-A.8.2-3-5.1_Authentication_Inventory_{GENERATED_TIMESTAMP}.xlsx"
    wb.save(filename)
    ...
    print("  2. Complete System_Auth_Inventory (main assessment)")
    print("  3. Review SSO_Integration_Status (track SSO adoption)")
    print("  4. Verify Password_Policy_Compliance")
    print("  5. Document gaps in Gap_Analysis")
    print("  6. Collect evidence in Evidence_Register")
    print("  7. Obtain approvals in Approval_Sign_Of")
    print()
    print("Target Metrics:")
    print("  • MFA Coverage: 100% privileged users, 90%+ all users")
    print("  • SSO Adoption: 90%+ of applications")
    print("  • Modern Protocols: >95% (SAML, OAuth, OIDC, Kerberos)")
    print("  • Legacy Protocols: <5% (plan deprecation)")
    print()
...
def validate_workbook(filename):
    ...
    try:
        ...
        if ws.max_row >= 154:  # 4 header rows + 150 data rows
            print(f"  ✅ System_Auth_Inventory: {SYSTEM_ROW_COUNT} pre-formatted rows")
        else:
            print(f"  ⚠️ System_Auth_Inventory: Fewer than expected rows")
