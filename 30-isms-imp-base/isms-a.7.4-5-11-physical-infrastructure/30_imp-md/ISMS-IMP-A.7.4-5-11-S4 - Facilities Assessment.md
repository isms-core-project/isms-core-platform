...
## Consolidated Physical Infrastructure Assessment Framework
...
**Sheets:**
1. **Instructions & Legend** - How to use dashboard
2. **Executive Dashboard** - 1-page summary (overall score, key metrics, status)
3. **Metrics Detail** - Detailed breakdowns (access monitoring, environmental, utility)
4. **Incident Trends** - 12-month incident charts
5. **Recommendations** - Gap remediation tracking
...
3. Navigate to "Metrics Detail" sheet, update all detailed metrics
4. Navigate to "Incident Trends" sheet, update quarterly incident counts
5. Navigate to "Recommendations" sheet, update remediation status
6. Dashboard auto-calculates overall compliance score
7. Save and attach to quarterly report
...
**File Name:** ISMS_Dashboard_Physical_Infrastructure_YYYY-MM.xlsx (monthly) or _QYYYY-QX.xlsx (quarterly)

**Generation:** Automated via Python script (generate_a74_4_compliance_dashboard.py)

**Sheet Count:** 5 worksheets
...
| 2 | Executive Dashboard | 1-page summary view | Manual input + Formulas | Monthly/Quarterly |
| 3 | Metrics Detail | Detailed metric breakdowns | Manual input + Formulas | Quarterly |
| 4 | Incident Trends | 12-month incident charts | Manual input + Charts | Monthly |
| 5 | Recommendations | Gap remediation tracking | Manual input | Quarterly |
...
**Purpose:** 1-page summary for executives

**Layout:**

**Section 1: Header (Rows 1-3)**
- Row 1: "Physical Infrastructure Compliance Dashboard"
- Row 2: Assessment Month/Quarter
- Row 3: Overall Compliance Score (large font, color-coded)

**Section 2: Key Metrics (Rows 5-15)**

| Metric | Input/Formula | Target | Status Formula |
|--------|---------------|--------|----------------|
| Failed Access Attempts | Manual input | <5 | IF(value<5,"✅","❌") |
| Temperature Excursions | Manual input | <5 | IF(value<5,"✅",IF(value<10,"⚠️","❌")) |
| Power Uptime (%) | Manual input | ≥99.99% (Tier 1), ≥99.9% (Tier 2) | IF(value≥target,"✅","❌") |
| HVAC Uptime (%) | Manual input | ≥99.9% (Tier 1), ≥99% (Tier 2) | IF(value≥target,"✅","❌") |
| ISP Uptime (%) | Manual input | ≥99.9% | IF(value≥99.9,"✅","❌") |
| UPS Battery Health | Manual input | All >80% | Pass/Fail dropdown |

**Section 3: Compliance Scores (Rows 17-25)**

| Control | Score Source | Weighting |
|---------|--------------|-----------|
| A.7.4 (Physical Monitoring) | Imported from S1 Summary Dashboard | 35% |
| A.7.5 (Environmental Protection) | Imported from S2 Summary Dashboard | 35% |
| A.7.11 (Utility Resilience) | Imported from S3 Summary Dashboard | 30% |

**Overall Compliance Score Formula:**
```excel
=(A74_Score*0.35 + A75_Score*0.35 + A711_Score*0.30)
```

**Section 4: Trend Indicator (Rows 27-30)**
- Compare current month/quarter to previous month/quarter
- Display: ↑ Improving, → Stable, ↓ Declining

**Conditional Formatting:**
- Overall Score: Green ≥90%, Amber 75-89%, Red <75%
- Metrics: Green if meets target, Red if fails target

### Sheet 3: Metrics Detail

**Purpose:** Detailed breakdowns for operational review

**Layout:**

**Section 1: Access Monitoring (Rows 3-15)**
- Access Control Coverage (% of entry/exit points with badge readers)
- CCTV Coverage (% of required areas with cameras)
- Intrusion Detection Coverage (% of protected areas with sensors)
- Access Log Analysis (failed attempts, after-hours access details)

**Section 2: Environmental Protection (Rows 17-29)**
- Fire Detection Coverage (% of facility area with smoke/heat detectors)
- Fire Suppression Coverage (% of critical areas with suppression)
- Water Detection Coverage (% of high-risk areas with water sensors)
- Temperature Monitoring Coverage (% of server rooms with temp sensors)
- Excursion Details (which facilities, how many excursions, root causes)

**Section 3: Utility Resilience (Rows 31-43)**
- UPS Specifications (per facility: capacity, runtime, battery health)
- Generator Specifications (per facility: capacity, runtime, fuel type)
- HVAC Specifications (per facility: capacity, margin, redundancy)
- ISP Specifications (per facility: primary ISP, secondary ISP, diverse paths)
- Uptime Details (which systems had downtime, duration, causes)

### Sheet 4: Incident Trends

**Purpose:** 12-month incident tracking and trend analysis

**Layout:**

**Table (Rows 3-15):**

| Month | Physical Security Incidents | Environmental Incidents | Utility Incidents | Total |
|-------|----------------------------|------------------------|-------------------|-------|
| Jan 2025 | [Count] | [Count] | [Count] | [Formula: Sum] |
| Feb 2025 | [Count] | [Count] | [Count] | [Formula: Sum] |
| ... | ... | ... | ... | ... |
| Dec 2025 | [Count] | [Count] | [Count] | [Formula: Sum] |

**Charts (Below Table):**
- Chart 1: Line chart showing incident trend over 12 months (3 lines: Physical, Environmental, Utility)
- Chart 2: Pie chart showing incident distribution by type (current year total)
...
**Table (Rows 3-103):** 100 data rows for tracking gaps

| Gap ID | Control | Finding | Severity | Remediation Action | Owner | Due Date | Status | Notes |
|--------|---------|---------|----------|-------------------|-------|----------|--------|-------|
| GAP-001 | A.7.4 | Building B lacks CCTV coverage | High | Install 5 cameras | Facilities Mgr | 31.03.2026 | In Progress | Quote received |
| GAP-002 | A.7.5 | Datacenter 1 water sensors missing | Critical | Install 10 sensors | Facilities Mgr | 15.02.2026 | Complete | Sensors installed 10.02.2026 |
...
**Conditional Formatting:**
- Status: Green if "Complete", Amber if "In Progress", Red if "Not Started" and past due date
- Severity: Red background if "Critical", Orange if "High"
...
**END OF ISMS-IMP-A.7.4-5-11-S4: Compliance Dashboard & Assessment Process**

---

*"Physical infrastructure compliance is not just quarterly paperwork. It's continuous monitoring—monthly automated metrics, quarterly comprehensive assessments, annual audit-readiness verification—all consolidated in a unified dashboard that provides executives, auditors, and operational teams with real-time visibility into Controls A.7.4, A.7.5, and A.7.11 compliance, demonstrating systematic and rigorous adherence to ISO 27001:2022 physical infrastructure security requirements."*