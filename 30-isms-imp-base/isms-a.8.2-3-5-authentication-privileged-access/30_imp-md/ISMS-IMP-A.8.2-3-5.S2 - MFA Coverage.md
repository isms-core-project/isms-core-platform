...
**User Inventory:**
- [ ] Complete user list from identity provider (Azure AD, Okta, Active Directory)
...
- [ ] Read access to identity provider (Azure AD, Okta admin console)
...
- [ ] Azure AD: "MFA Registration Details" report
...
Get-AzureADUser -All $true | Select UserPrincipalName, DisplayName, 
  @{N='MFAStatus';E={
    (Get-MsolUser -UserPrincipalName $_.UserPrincipalName).StrongAuthenticationRequirements.State
  }} | Export-Csv -Path "AzureAD_MFA_Status.csv" -NoTypeInformation
```
...
```bash
python3 generate_a8235_2_mfa_coverage.py --input AzureAD_MFA_Status.csv
```
...
**Step 6: Review Calculated Metrics**
...
2. **MFA Enforcement Policies**:
   - Azure AD Conditional Access policies (if Azure)
...
- [ ] Date/time stamp visible
- [ ] Source clearly identified (Azure AD, Okta, etc.)
...
**Solution**:
- Azure AD: "Capable" means user CAN register, not that they HAVE registered
...
### 7.1 Understanding MFA Coverage Scores
...
| 2 | User MFA Enrollment | Individual user MFA status | 1003 | User completes (or auto-import) |
| 3 | Coverage by User Type | MFA coverage metrics by user type | ~30 | Auto-calculated |
| 4 | MFA Gap Analysis | Users without MFA with remediation | 503 | Auto-populated from Sheet 2 |
| 5 | MFA Method Distribution | Breakdown by MFA method | ~20 | Auto-calculated |
| 6 | Enrollment Trend | Month-over-month progress | ~15 | User completes (historical) |
| 7 | Evidence Register | Evidence tracking | 53 | User completes |
| 8 | Approval & Sign-Off | Three-level approval | ~25 | User completes |

**Total Workbook:** 8 sheets, ~1,700 rows of structured data collection
...
## 2. Sheet 2: User MFA Enrollment (Primary Data Collection)
...
### 2.4 MFA Method Tier Calculation (Column L)
...
### 2.5 MFA Requirement Calculation (Column Q)
...
### 2.6 Compliance Status Calculation (Column R)
...
### 4.4 Gap Priority Calculation (Column S)
...
- Imports user data from CSV (Azure AD, Okta, Google export)
