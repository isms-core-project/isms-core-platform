#!/usr/bin/env python3
"""
fix_isms_encoding_md.py

Universal Unicode encoding fixer for Markdown files using the ftfy library.
Automatically detects and repairs mojibake (UTF-8 double-encoding) in Markdown documentation.
...
Usage:
    # Fix single file (auto-backup to file.md.bak)
    python3 fix_isms_encoding_md.py --dry-run *.md
    python3 fix_isms_encoding_md.py policy.md
    python3 fix_isms_encoding_md.py *.md
    
    python3 fix_isms_encoding_md.py ./policies --dry-run
    python3 fix_isms_encoding_md.py ./ --dry-run
    
    # Fix all .md files in current directory
    python3 fix_isms_encoding_md.py *.md
    
    # Recursively scan directory
    python3 fix_isms_encoding_md.py --recursive /path/to/docs
    
    # Scan current directory recursively
    python3 fix_isms_encoding_md.py --recursive .
    
    # Preview with detailed report (no backup needed)
    python3 fix_isms_encoding_md.py --dry-run --report corruption_report.txt --recursive .
    
    # Fix with report
    python3 fix_isms_encoding_md.py --report fixed.txt --recursive .
    
    # Skip backup (NOT RECOMMENDED)
    python3 fix_isms_encoding_md.py --no-backup policy.md

Author: Greg & Claude AI
Date: 2026-01-27
Purpose: Fix encoding issues in ISMS Markdown documentation
"""
...
def find_markdown_files(path: Path, recursive: bool = False) -> List[Path]:
    ...
    if recursive:
        files = list(path.rglob('*.md')) + list(path.rglob('*.markdown'))
    else:
...
def generate_report(results: List[Dict], report_file: Path, dry_run: bool, backup_enabled: bool):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        # Header
        f.write("=" * 80 + "\n")
        f.write("ISMS MARKDOWN ENCODING FIX REPORT\n")
        f.write("=" * 80 + "\n")
        ...
        if backed_up > 0 and not dry_run:
            ...
            f.write("  for f in *.md.bak; do cp \"$f\" \"${f%.bak}\"; done\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Fix Unicode encoding corruption in Markdown files using ftfy',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s policy.md                    # Fix single file (auto-backup)
  %(prog)s *.md                         # Fix all .md in current dir
  %(prog)s -r /path/to/docs             # Recursively scan directory
...
Use --no-backup to skip (NOT RECOMMENDED for production files).
    ...
    # Banner
    print("=" * 80)
    print("ISMS Markdown Encoding Fix Utility (ftfy-powered)")
    print("Automatically detects and repairs UTF-8 mojibake in documentation")
    print("=" * 80)
    ...
    print(f"Found {len(all_files)} Markdown file(s) to process\n")
    ...
    if args.dry_run and fixed_count > 0:
        ...
        if backed_up > 0:
            print(f"\nTo restore originals: for f in *.md.bak; do cp \"$f\" \"${{f%.bak}}\"; done")
    elif fixed_count == 0:
