...
"""
================================================================================
A.5.3 Workbook Normalizer
================================================================================

Normalizes all A.5.3 workbooks to ensure consistent formatting, styling,
and structure across all generated Excel files.

Usage:
    python3 normalize_a53.py
================================================================================
"""
...
def normalize_workbook(filepath: Path) -> bool:
    ...
    try:
        ...
        for ws in wb.worksheets:
            ...
            if ws["A1"].value:
                # Check if it's a title row (Instructions sheet)
                if ws.title == "Instructions" or "Dashboard" in ws.title:
            ...
            if ws.freeze_panes is None:
                if ws.title == "Instructions":
                    pass  # Don't freeze Instructions sheet
                elif ws.max_row > 3:
                    ws.freeze_panes = "A2"

            # Ensure all cells in row 1 have header styling (except Instructions)
            if ws.title != "Instructions":
...
def main() -> int:
    ...
    workbooks = list(WORKBOOK_DIR.glob("ISMS-IMP-A.5.3*.xlsx"))

    if not workbooks:
        logger.info("No A.5.3 workbooks found to normalize")
        return 0
