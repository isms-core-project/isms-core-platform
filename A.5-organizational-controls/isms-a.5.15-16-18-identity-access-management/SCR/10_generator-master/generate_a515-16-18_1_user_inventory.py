...
def create_user_inventory_sheet(wb, styles):
    ...
    for user in users:
        ...
        ws.cell(row=row, column=9).value = user["hire_date"].strftime("%d.%m.%Y") if user["hire_date"] else ""
        ws.cell(row=row, column=10).value = user["termination_date"].strftime("%d.%m.%Y") if user["termination_date"] else ""
        ws.cell(row=row, column=11).value = user["status"]
        ws.cell(row=row, column=12).value = user["last_login"].strftime("%d.%m.%Y") if user["last_login"] else "Never"
        ws.cell(row=row, column=13).value = user["created_date"].strftime("%d.%m.%Y") if user["created_date"] else ""
...
def create_employee_lifecycle_sheet(wb, styles, users):
    ...
    for event in joiner_events:
        ...
        ws.cell(row=row, column=5).value = event["hire_date"].strftime("%d.%m.%Y")
        ws.cell(row=row, column=6).value = event["provision_date"].strftime("%d.%m.%Y")
    ...
    for event in leaver_events:
        ...
        ws.cell(row=row, column=3).value = event["full_name"]
        ws.cell(row=row, column=4).value = event["department"]
        ws.cell(row=row, column=5).value = event["termination_date"].strftime("%d.%m.%Y")
        ws.cell(row=row, column=6).value = event["disable_date"].strftime("%d.%m.%Y %H:%M")
...
def create_contractor_lifecycle_sheet(wb, styles, users):
    ...
    for contractor in contractors:
        ...
        ws.cell(row=row, column=6).value = contractor["hire_date"].strftime("%d.%m.%Y")
        ws.cell(row=row, column=7).value = contract_end.strftime("%d.%m.%Y")
        ws.cell(row=row, column=8).value = contract_end.strftime("%d.%m.%Y")  # Assume same as contract end
...
def create_service_accounts_sheet(wb, styles, users):
    ...
    for account in service_accounts:
        ...
        ws.cell(row=row, column=6).value = account["created_date"].strftime("%d.%m.%Y")
        ws.cell(row=row, column=7).value = account["last_login"].strftime("%d.%m.%Y") if account["last_login"] else "Unknown"
...
def create_orphaned_accounts_sheet(wb, styles, users):
    ...
    for account in orphaned:
        ...
        ws.cell(row=row, column=5).value = account["department"]
        ws.cell(row=row, column=6).value = account["last_login"].strftime("%d.%m.%Y") if account["last_login"] else "Never"
...
def create_gap_analysis_sheet(wb, styles):
    ...
    gaps = [
        ("GAP-001", "Provisioning", "3 users provisioned after hire date", "Medium", "3", "Manual provisioning delays",
         "Automate provisioning via HR system", "IAM Manager", (datetime.now() + timedelta(days=30)).strftime("%d.%m.%Y"), "Open"),
        ("GAP-002", "Deprovisioning", "2 terminated users retained access >24h", "High", "2", "No termination notification from HR",
         "Implement automated HR termination feed", "IAM Manager", (datetime.now() + timedelta(days=14)).strftime("%d.%m.%Y"), "In Progress"),
        ("GAP-003", "Orphaned Accounts", "15 orphaned accounts detected", "High", "15", "No monthly orphaned account scan",
         "Schedule monthly automated scan", "Security Team", (datetime.now() + timedelta(days=7)).strftime("%d.%m.%Y"), "Open"),
        ("GAP-004", "Contractor Management", "3 contractors past contract end date", "Medium", "3", "No automated contract expiry alerts",
         "Implement contract expiry monitoring", "IAM Manager", (datetime.now() + timedelta(days=21)).strftime("%d.%m.%Y"), "Open"),
        ("GAP-005", "Service Accounts", "1 service account missing owner", "Medium", "1", "Incomplete service account documentation",
         "Conduct service account inventory review", "IT Manager", (datetime.now() + timedelta(days=14)).strftime("%d.%m.%Y"), "Open"),
    ]
...
def main():
    ...
    # Save workbook
    filename = f"ISMS-IMP-A.5.15-16-18.S1_User_Inventory_Assessment_{GENERATED_TIMESTAMP}.xlsx"
