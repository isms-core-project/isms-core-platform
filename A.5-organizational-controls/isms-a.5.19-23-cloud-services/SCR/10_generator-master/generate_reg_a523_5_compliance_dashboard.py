...
"""
================================================================================
ISMS-IMP-A.5.23.S5 - Compliance Monitoring Dashboard Excel Generator
================================================================================
...
1. Source workbook paths (update to your actual file locations)
...
4. Regulatory mappings (DORA, NIS2 per your jurisdictions)
...
**Purpose:**
Consolidates data from four cloud service assessment workbooks into unified
executive dashboard for ISO 27001:2022 Control A.5.23 compliance oversight
...
3. Service Overview - Cloud service compliance summary
4. Vendor Compliance - Due diligence status
5. Configuration Status - Security baseline compliance
6. Governance Metrics - Risk and compliance KPIs
7. Regulatory Alignment - DORA, NIS2, AI Act status
...
Prerequisites:
    - Complete source workbooks (S1-S4) must exist in current directory
...
# DOCUMENT METADATA
# =============================================================================
DOCUMENT_ID = "ISMS-IMP-A.5.23.S5"
...
def validate_all_inputs() -> dict:
    ...
    """
    logger.info("\n" + "=" * 78)
    logger.info("WORKBOOK VALIDATION")
    logger.info("=" * 78)
    ...
    for wb_type, schema in schemas.items():
        ...
        try:
            ...
            if is_valid:
                ...
                logger.error(f"   [ERROR] Schema validation failed:")
                for error in errors:
    ...
    logger.info("\n" + "=" * 78)
    logger.info("[OK] All workbooks validated successfully")
    logger.info("=" * 78)
...
def extract_exit_strategy_data(inventory_filepath: str) -> dict:
    ...
    logger.info(f"   [OK] Extracted data from {data['total_services']} services")
    logger.info(f"        Cloud-to-Cloud: {data['cloud_to_cloud']}")
    logger.info(f"        Hybrid: {data['hybrid']}")
    logger.info(f"        On-Premises: {data['on_premises']}")
    logger.info(f"        Not Determined: {data['not_determined']}")
...
def extract_poc_testing_data(governance_filepath: str) -> dict:
    ...
    if GOVERNANCE_SCHEMA['exit_review_sheet'] not in wb.sheetnames:
        logger.info("   [WARNING] No Exit Strategy Review sheet found in governance workbook")
        wb.close()
        return {
    ...
    logger.info(f"   [OK] Extracted PoC testing data from {data['total_requiring_poc']} Critical services")
    logger.info(f"        Completed (Pass): {data['poc_completed']}")
    logger.info(f"        Not Tested: {data['poc_not_tested']}")
    logger.info(f"        Overdue: {data['poc_overdue']}")
...
def create_exit_strategy_dashboard(ws, styles, exit_data):
    ...
    for col in ['A', 'B', 'C', 'D', 'E']:
        ...
        ws[f'{col}4'].border = header_border
    ...
    logger.info("   [OK] Exit Strategy dashboard section created")
...
def create_poc_testing_dashboard(ws, styles, poc_data, start_row):
    ...
    for col in ['A', 'B', 'C', 'D']:
        ...
        ws[f'{col}{start_row}'].border = header_border
    ...
    for metric, value, status, requirement in metrics:
        ...
        # Add borders to data cells
        for col in ['A', 'B', 'C', 'D']:
            ws[f'{col}{start_row}'].border = cell_border
    ...
    logger.info("   [OK] PoC Testing dashboard section created")
...
def create_dashboard_workbook(workbooks: dict) -> Workbook:
    ...
    logger.info("\n" + "=" * 78)
    logger.info("DASHBOARD CREATION")
    logger.info("=" * 78)
    ...
    for col in ['A', 'B', 'C', 'D', 'E', 'F']:
        ...
        ws2[f'{col}4'].border = header_border
    ...
    for service in exit_data['services']:
        ...
        if is_high_risk:
            ...
            cell_border = Border(left=thin, right=thin, top=thin, bottom=thin)
            for col in ['A', 'B', 'C', 'D', 'E', 'F']:
                ws2[f'{col}{row}'].border = cell_border
    ...
    logger.info("   [OK] Risk Overview sheet created")
    ...
    for col in ['A', 'B', 'C', 'D']:
        ...
        ws3[f'{col}3'].border = header_border
    ...
    for icon, severity, finding, recommendation in recommendations:
        ...
        # Add borders to data cells
        for col in ['A', 'B', 'C', 'D']:
            ws3[f'{col}{row}'].border = cell_border
    ...
    logger.info("   [OK] Recommendations sheet created")
...
def main():
    ...
    logger.info("=" * 78)
    logger.info("ISMS-IMP-A.5.23.S5 - Compliance Monitoring Dashboard Generator (REGULATORY)")
    logger.info("ISO/IEC 27001:2022 Control A.5.23 + Exit Strategy Framework")
    logger.info("=" * 78)
    logger.info("")
    logger.info(f"{SHIELD} Consolidates data from 4 assessment workbooks:")
    logger.info(f"   1. Inventory (Exit Strategy framework)")
    logger.info(f"   2. Vendor Due Diligence")
    logger.info(f"   3. Secure Configuration")
    logger.info(f"   4. Governance (Exit Strategy review + PoC testing)")
    ...
    logger.info("=" * 78)
    logger.info("")
    logger.info(f"{CHART} DASHBOARD STRUCTURE (3 sheets):")
    logger.info("   1. Exit Strategy Dashboard (Exit Strategy metrics + PoC Testing compliance)")
    logger.info("   2. Risk Overview (High-risk services requiring attention)")
    logger.info("   3. Recommendations (Actionable findings)")
    logger.info("")
    logger.info(f"{SHIELD} COMPLIANCE COVERAGE:")
    logger.info("   ✓ ISO 27001:2022 A.5.23 (Exit planning requirements)")
    logger.info("   ✓ POL-A.5.19-23-S5 Section 8 (Exit Strategy Framework)")
    logger.info("   ✓ DORA Article 28.6 (Annual PoC testing)")
    logger.info("")
    logger.info(f"{TARGET} NEXT STEPS:")
    logger.info("   1. Review Exit Strategy Dashboard for compliance status")
    logger.info("   2. Address Critical/High priority items in Recommendations")
    logger.error("   3. Escalate PoC testing failures to CISO immediately")
    logger.info("   4. Update risk register with findings")
    logger.info("   5. Quarterly reporting to Risk Committee")
    logger.info("")
    logger.info("=" * 78)
