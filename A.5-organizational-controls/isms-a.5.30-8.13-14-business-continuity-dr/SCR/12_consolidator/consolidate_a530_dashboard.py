...
# =============================================================================
"""
ISMS-A.5.30-8.13-14 Business Continuity/DR - Dashboard Consolidation Utility
...
SOURCE_WORKBOOKS = [
    ('A.5.30.1', 'Backup_Inventory', 'Backup Inventory'),
    ('A.5.30.2', 'Redundancy_Analysis', 'Redundancy Analysis'),
    ('A.5.30.3', 'RPO_RTO', 'RPO/RTO Compliance'),
    ('A.5.30.4', 'Testing_Results', 'Testing Results'),
]

DASHBOARD_PATTERNS = ['A.5.30.5', 'Compliance_Dashboard', 'BC_DR_Dashboard']
...
def extract_gaps_from_workbook(filepath, assessment_area, gap_counter_start=1):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            if any(x in sheet_name.lower() for x in ['gap', 'finding', 'issue', 'fail']):
                ws = wb[sheet_name]
                for row in range(8, min(ws.max_row + 1, 100)):
                    ...
                    for val in row_values:
                        if val:
                            val_str = str(val)
                            if any(x in val_str for x in ['⚠️', '❌', 'Non-Compliant', 'Partial', 'Failed']):
                                status = val_str
                            elif len(val_str) > 30:
                                gap_desc = val_str
                    if status or gap_desc:
                        severity = 'High' if status and ('❌' in str(status) or 'Failed' in str(status)) else 'Medium'
                        gaps.append([f'GAP-BCR-{gap_counter:03d}', assessment_area,
                            os.path.basename(filepath), row_values[0] or 'System',
...
def generate_risks_and_remediation(gaps):
    ...
    for i, gap in enumerate([g for g in gaps if g[7] == 'High'], 1):
        risks.append([f'RISK-{i:03d}', gap[0], 'BC/DR', f'Risk: {gap[4][:80]}',
...
def consolidate_dashboard():
    print("=" * 80)
    print("ISMS-A.5.30-8.13-14 Business Continuity/DR - Dashboard Consolidation")
    print("=" * 80)
