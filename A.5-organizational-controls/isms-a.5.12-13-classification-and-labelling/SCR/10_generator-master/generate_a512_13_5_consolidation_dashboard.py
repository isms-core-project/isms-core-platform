...
four A.5.12-13 assessment workbooks into a unified executive view.

**Source Assessment Domains:**
1. Domain 1: Classification Scheme Definition (A.5.12-13.1)
2. Domain 2: Labelling Procedures and Standards (A.5.12-13.2)
3. Domain 3: Asset Classification Inventory (A.5.12-13.3)
4. Domain 4: Compliance Dashboard (A.5.12-13.4)
...
4. Classification_Compliance - Scheme implementation status
5. Labelling_Compliance - Labelling standards adherence
6. Inventory_Compliance - Asset classification coverage
7. Cross_Domain_Gaps - Gaps across all domains
8. Remediation_Tracker - Consolidated action items
9. KPI_Summary - Key metrics from all domains
10. Evidence_Index - Cross-reference to source evidence
...
CONTROL_REF = f"ISO/IEC 27001:2022 - Controls A.5.12 & A.5.13: {CONTROL_NAME}"
...
def create_instructions_sheet(ws):
    ...
    instructions = [
        ...
        [""],
        ["SOURCE WORKBOOKS"],
        ["  • A.5.12-13.1: Classification Scheme Definition"],
        ["  • A.5.12-13.2: Labelling Procedures and Standards"],
        ["  • A.5.12-13.3: Asset Classification Inventory"],
        ["  • A.5.12-13.4: Compliance Dashboard"],
        [""],
        ["COMPLIANCE SCORING"],
...
def create_executive_summary_sheet(ws):
    ...
    title_cell = ws.cell(row=1, column=1, value="A.5.12-13 Classification & Labelling - Executive Summary")
    ...
    domains = [
        ("A.5.12", "Classification Scheme Definition", "", "", "", ""),
        ("A.5.13", "Labelling Procedures & Standards", "", "", "", ""),
        ("A.5.12-13", "Asset Classification Inventory", "", "", "", ""),
        ("A.5.12-13", "Compliance Dashboard", "", "", "", ""),
    ...
    metrics = [
        ("Classification scheme documented", "100%", "", ""),
        ("Assets classified in inventory", "100%", "", ""),
        ("Labelling standards defined", "100%", "", ""),
        ("Staff trained on classification", "100%", "", ""),
    ]
...
def create_domain_overview_sheet(ws):
    ...
    domains = [
        (3, "DOMAIN 1: CLASSIFICATION SCHEME (A.5.12-13.1)", [
            "Classification levels defined", "Handling requirements documented",
            "CIA matrix completed", "Regulatory mapping done"]),
        (10, "DOMAIN 2: LABELLING STANDARDS (A.5.12-13.2)", [
            "Digital labelling standards", "Physical labelling standards",
            "Automation tools assessed", "Labelling procedures approved"]),
        (17, "DOMAIN 3: ASSET INVENTORY (A.5.12-13.3)", [
            "Asset inventory complete", "Classification assignments current",
            "Reclassification process defined", "Gap analysis completed"]),
    ]
...
def create_classification_compliance_sheet(ws):
    ws.title = "Classification_Compliance"
    ws.merge_cells('A1:H1')
    title_cell = ws.cell(row=1, column=1, value="Classification Scheme Compliance - Consolidated View")
    title_cell.font = TITLE_FONT
    title_cell.fill = TITLE_FILL
    title_cell.alignment = Alignment(horizontal="center")

    headers = ["Classification_Level", "Definition_Status", "Handling_Rules", "Examples_Documented",
               "Training_Available", "Compliance_Status", "Gap_Notes", "Owner"]
    for col, header in enumerate(headers, 1):
        ws.cell(row=3, column=col, value=header)
    apply_header_style(ws, 3, 1, len(headers))

    levels = ["Public", "Internal", "Confidential", "Restricted", "Top Secret"]
    for row_num, level in enumerate(levels, 4):
        ws.cell(row=row_num, column=1, value=level).border = THIN_BORDER
        for col in range(2, 9):
            cell = ws.cell(row=row_num, column=col)
            cell.fill = INPUT_FILL
            cell.border = THIN_BORDER

    for col, width in enumerate([20, 18, 18, 20, 18, 18, 30, 20], 1):
        ws.column_dimensions[get_column_letter(col)].width = width


def create_labelling_compliance_sheet(ws):
    ws.title = "Labelling_Compliance"
    ws.merge_cells('A1:H1')
    title_cell = ws.cell(row=1, column=1, value="Labelling Standards Compliance - Consolidated View")
    title_cell.font = TITLE_FONT
    title_cell.fill = TITLE_FILL
    title_cell.alignment = Alignment(horizontal="center")

    headers = ["Asset_Type", "Digital_Label_Standard", "Physical_Label_Standard",
               "Automation_Status", "Compliance_Status", "Gap_Notes", "Remediation", "Owner"]
    for col, header in enumerate(headers, 1):
        ws.cell(row=3, column=col, value=header)
    apply_header_style(ws, 3, 1, len(headers))

    asset_types = ["Documents", "Emails", "Databases", "Files/Folders", "Physical Media",
                   "Hardware", "Cloud Storage", "Source Code"]
    for row_num, asset_type in enumerate(asset_types, 4):
        ws.cell(row=row_num, column=1, value=asset_type).border = THIN_BORDER
        for col in range(2, 9):
            cell = ws.cell(row=row_num, column=col)
            cell.fill = INPUT_FILL
            cell.border = THIN_BORDER

    for col, width in enumerate([18, 22, 22, 18, 18, 30, 25, 18], 1):
        ws.column_dimensions[get_column_letter(col)].width = width


def create_inventory_compliance_sheet(ws):
    ws.title = "Inventory_Compliance"
    ws.merge_cells('A1:H1')
    title_cell = ws.cell(row=1, column=1, value="Asset Inventory Classification - Consolidated View")
    title_cell.font = TITLE_FONT
    title_cell.fill = TITLE_FILL
    title_cell.alignment = Alignment(horizontal="center")

    headers = ["Asset_Category", "Total_Assets", "Classified", "Labelled", "% Complete",
               "Compliance_Status", "Gap_Notes", "Owner"]
    for col, header in enumerate(headers, 1):
        ws.cell(row=3, column=col, value=header)
    apply_header_style(ws, 3, 1, len(headers))

    categories = ["Information Assets", "Software Assets", "Physical Assets", "Services",
                  "People", "Intangibles", "Cloud Resources"]
    for row_num, category in enumerate(categories, 4):
    ...
    for col, width in enumerate([22, 14, 14, 14, 12, 18, 30, 18], 1):
        ws.column_dimensions[get_column_letter(col)].width = width
...
def create_trend_dashboard_sheet(ws):
    ...
    headers = ["Period", "Classification %", "Labelling %", "Inventory %",
    ...
    for col, width in enumerate([12, 16, 14, 14, 12, 15, 18, 35], 1):
        ws.column_dimensions[get_column_letter(col)].width = width
...
def generate_workbook():
    ...
    sheets = [
        ...
        ("Classification_Compliance", create_classification_compliance_sheet),
        ("Labelling_Compliance", create_labelling_compliance_sheet),
        ("Inventory_Compliance", create_inventory_compliance_sheet),
