...
Part 1: Foundation - Workbook creation, styles, columns, validations, checklists
...
def create_workbook() -> Workbook:
    """Create workbook with all required sheets (v2.0 - 11 sheets)."""
    wb = Workbook()
    ...
    sheets = [
        ...
        "8. Jurisdictional Risk",           # Renumbered from 7 in v2.0
...
def get_extended_columns(sheet_type: str) -> list:
    ...
    extensions = {
        ...
        "incident_mgmt": [
            ...
            # NEW v2.0 - Regulatory columns for incident mgmt
...
# ============================================================================
# SECTION 5: CHECKLIST DEFINITIONS (v2.0 - Regulatory Additions)
# ============================================================================
...
def get_extended_validations(sheet_type: str) -> dict:
    ...
    validations = {
        ...
        "incident_mgmt": {
            ...
            # NEW v2.0 - Regulatory columns (Y-AB)
...
print("Part 2 loaded: Checklists + Extended Validations (Regulatory Enhanced)")
...
# ============================================================================
# SECTION 8: INSTRUCTIONS SHEET CREATOR (v2.0 - Regulatory Updates)
# ============================================================================
...
def create_instructions_sheet(ws, styles):
    ...
    ws["A1"] = "ISMS-IMP-A.5.23.S4 - Ongoing Governance & Risk Management v2.0"
    ...
    legend = [
        (f"{CHECK} Compliant", "C6EFCE", "Fully meets requirements"),
        (f"{WARNING} Partial", "FFEB9C", "Partially implemented, gaps exist"),
        (f"{XMARK} Non-Compliant", "FFC7CE", "Not implemented or major gaps"),
    ...
    # NEW v2.0 - Regulatory Compliance Section
    row += 2
...
def create_7_exit_strategy_review(ws, styles):
    ...
    print("   [OK] Sheet 7: Exit Strategy Annual Review & PoC Testing created")
    print(f"        Columns: A-N (14 columns)")
    print(f"        Conditional formatting: Overdue reviews, Overdue PoC, On-Prem warning")
    print(f"        Compliance: POL-S5 Section 8, DORA Article 28.6")


# ============================================================================
# SECTION 10: JURISDICTIONAL RISK SHEET (NEW v2.0)
# ============================================================================

def create_7_jurisdictional_risk(ws, styles):
    """NEW v2.0: Sheet 8 - Jurisdictional Risk Assessment (CLOUD Act)."""
...
print("Part 3 loaded: Sheet Creators (Instructions + Assessment Sheets 2-8)")
...
# ============================================================================
# SECTION 11: SUMMARY DASHBOARD (v2.0 - Jurisdictional + Regulatory Metrics)
# ============================================================================
...
def create_summary_dashboard(ws, styles):
    ...
    ws["A1"] = "ONGOING GOVERNANCE & RISK - COMPLIANCE SUMMARY DASHBOARD v2.0"
    ws["A1"].font = styles["header_font"]()
    ws["A1"].fill = styles["header_fill"]()
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 40
    ...
    for area_name, sheet_ref in assessment_sheets:
        ...
        ws.cell(row=row, column=3,
                value=f'=COUNTIF({sheet_ref}!H6:H24,f"{CHECK} Compliant")').border = styles["border"]()
        # Partial
        ws.cell(row=row, column=4,
                value=f'=COUNTIF({sheet_ref}!H6:H24,f"{WARNING} Partial")').border = styles["border"]()
        # Non-Compliant
        ws.cell(row=row, column=5,
                value=f'=COUNTIF({sheet_ref}!H6:H24,f"{XMARK} Non-Compliant")').border = styles["border"]()
        # N/A
    ...
    jurisdictional_metrics = [
        ("US-HQ Providers (CLOUD Act Scope)", "=COUNTIF('8. Jurisdictional Risk'!E6:E30,\"United States\")", "=IF(B{row}>0,f\"{WARNING} Review\",f\"{CHECK} OK\")"),
        ("Providers with US Parent Company", "=COUNTIF('8. Jurisdictional Risk'!F6:F30,\"Yes\")", "=IF(B{row}>0,f\"{WARNING} Review\",f\"{CHECK} OK\")"),
        ("CLOUD Act Potential Exposure (Unmitigated)", "=COUNTIF('8. Jurisdictional Risk'!G6:G30,\"Potential*\")", "=IF(B{row}>0,f\"{WARNING} Review\",f\"{CHECK} OK\")"),
        ("CLOUD Act Mitigated", "=COUNTIFS('8. Jurisdictional Risk'!G6:G30,\"Mitigated*\")", "Info"),
        ("High/Critical Jurisdictional Risk", "=COUNTIF('8. Jurisdictional Risk'!N6:N30,\"High\")+COUNTIF('8. Jurisdictional Risk'!N6:N30,\"Critical\")", "=IF(B{row}>0,f\"{XMARK} Action\",f\"{CHECK} OK\")"),
        ("Providers Without EU Data Boundary", "=COUNTIF('8. Jurisdictional Risk'!I6:I30,\"No\")", "=IF(B{row}>0,f\"{WARNING} Review\",f\"{CHECK} OK\")"),
        ("Providers Without Customer-Managed Keys", "=COUNTIF('8. Jurisdictional Risk'!J6:J30,\"No\")", "=IF(B{row}>0,f\"{WARNING} Review\",f\"{CHECK} OK\")"),
    ]
    ...
    # === TABLE 3: REGULATORY RISK MONITORING STATUS (NEW v2.0) ===
    row += 2
    ...
    regulatory_metrics = [
        ("DORA Risks Not Monitored", "=COUNTIF('4. Incident Management'!Y6:Y30,\"Not Monitored\")", "=IF(B{row}>0,f\"{XMARK} Action\",f\"{CHECK} OK\")"),
        ("NIS2 Significant Incidents (Require ≤24h notification)", "=COUNTIF('4. Incident Management'!Z6:Z30,\"Significant*\")", "=IF(B{row}>0,f\"{WARNING} Review\",f\"{CHECK} OK\")"),
        ("AI Systems Requiring Active Monitoring", "=COUNTIF('4. Incident Management'!AA6:AA30,\"Active Monitoring\")", "Info"),
        ("Regulatory Risks Without Assigned Owner", "=COUNTIF('4. Incident Management'!AB6:AB30,\"\")", "=IF(B{row}>0,f\"{XMARK} Action\",f\"{CHECK} OK\")"),
    ]
...
# ============================================================================
# SECTION 12: EVIDENCE REGISTER (v2.0 - Regulatory Evidence Types)
# ============================================================================
...
def create_evidence_register(ws, styles):
    ...
    ws["A1"] = "EVIDENCE REGISTER - ONGOING GOVERNANCE & RISK v2.0"
    ws["A1"].font = styles["header_font"]()
    ws["A1"].fill = styles["header_fill"]()
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 35
...
# ============================================================================
# SECTION 13: APPROVAL SIGN-OFF (v2.0 - DPO + CRO Sections)
# ============================================================================
...
def create_approval_signoff(ws, styles):
    ...
    ws["A1"] = "ASSESSMENT APPROVAL AND SIGN-OFF v2.0"
    ws["A1"].font = styles["header_font"]()
    ws["A1"].fill = styles["header_fill"]()
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 40
    ...
    summary_fields = [
        ("Assessment Document:", "ISMS-IMP-A.5.23.S4 - Ongoing Governance & Risk Management v2.0"),
    ...
    approvers = [
        ...
        ("REVIEWED BY (DATA PROTECTION OFFICER)", "4472C4"),  # NEW v2.0
        ("REVIEWED BY (CHIEF RISK OFFICER)", "4472C4"),       # NEW v2.0
...
def main():
    ...
    print("=" * 80)
    print("ISMS-IMP-A.5.23.S4 - Ongoing Governance & Risk Management v2.0")
    ...
    print("[2/11] Creating Instructions & Legend (with regulatory guidance)...")
    ...
    for idx, (sheet_name, sheet_type) in enumerate(assessment_sheets, start=3):
        print(f"[{idx}/11] Creating {sheet_name}...")
    ...
    print("[7/11] Creating Exit Strategy Annual Review & PoC Testing (NEW - DORA Art. 28.6)...")
    create_7_exit_strategy_review(wb["7. Exit Strategy Review"], styles)
    
    # Create Jurisdictional Risk
    print("[8/11] Creating Jurisdictional Risk (CLOUD Act)...")
    create_7_jurisdictional_risk(wb["8. Jurisdictional Risk"], styles)
    
    # Create Dashboard
    print("[9/11] Creating Summary Dashboard (with regulatory metrics)...")
    create_summary_dashboard(wb["9. Summary Dashboard"], styles)
    
    # Create Evidence Register
    print("[10/11] Creating Evidence Register (expanded for regulatory)...")
    create_evidence_register(wb["10. Evidence Register"], styles)
    
    # Create Approval Sign-Off
    print("[11/11] Creating Approval Sign-Off (with DPO + CRO sections)...")
    ...
    print("  2. Access Review & Recertification (15 checklist items)")
    print("  3. Change Management (15 checklist items)")
    print("  4. Incident Management (30 checklist items: 15 base + 8 DORA + 5 NIS2 + 6 AI Act)")
    print("  5. Business Continuity (15 checklist items)")
    print("  6. Vendor Risk Monitoring (15 checklist items)")
    print("  7. Exit Strategy Annual Review (14 cols: Review + PoC Testing) [NEW v2.1]")
    print("  8. Jurisdictional Risk (20 cols: CLOUD Act assessment) [NEW]")
    print("  9. Summary Dashboard (3 tables: compliance + jurisdictional + regulatory)")
    print("  10. Evidence Register (120 rows, EV-OGR-###, regulatory evidence types)")
    print("  11. Approval Sign-Off (IT Ops → Compliance → DPO → CRO → CISO) [DPO+CRO NEW]")
    
    print("\n🆕 VERSION 2.0 ENHANCEMENTS:")
    print("  • NEW Sheet 7: Exit Strategy Annual Review + PoC Testing (DORA Art. 28.6) [v2.1]")
    print("  • NEW Sheet 8: Jurisdictional Risk Assessment (CLOUD Act)")
    print("  • Incident Management: +4 columns (DORA/NIS2/AI Act/Owner)")
    print("  • Dashboard: +2 tables (jurisdictional + regulatory monitoring)")
    print("  • Approval: +DPO & CRO review sections")
    print("  • Checklists: +19 regulatory items (8 DORA, 5 NIS2, 6 AI Act)")
    print("  • Dropdowns: +10 regulatory validation types")
    
    print("\n📊 REGULATORY COMPLIANCE COVERAGE:")
    print("  ✓ DORA (Digital Operational Resilience Act) - ICT risk management")
    print("  ✓ NIS2 (Network and Information Security Directive 2) - Incident management")
    print("  ✓ EU AI Act (High-Risk AI Systems) - AI risk monitoring")
    print("  ✓ US CLOUD Act (Jurisdictional Risk Assessment)")
    
    print("\n🔧 COLUMN STRUCTURE:")
    print("  • Base columns (A-Q): 17 standard columns")
    print("  • Extended columns (R-X): 7 domain-specific per sheet")
    print("  • Incident Mgmt (R-AB): 11 columns (7 base + 4 regulatory)")
    print("  • Jurisdictional Risk (A-T): 20 dedicated columns")
    
    print("\n📝 NEXT STEPS:")
    print(f"  1. Run validation: python3 excel_sanity_check.py {filename}")
    print("  2. Open in Excel and verify all 11 sheets present")
    ...
    print("     - IT Operations (access review, change mgmt, BC)")
    print("     - Risk Management (vendor risk, incident mgmt)")
    print("     - Compliance (all regulatory fields)")
    ...
    print('"Risk management is what you do when you realize \'YOLO\' isn\'t a valid')
    print('governance strategy." - Every CRO, eventually')
    print("=" * 80)
