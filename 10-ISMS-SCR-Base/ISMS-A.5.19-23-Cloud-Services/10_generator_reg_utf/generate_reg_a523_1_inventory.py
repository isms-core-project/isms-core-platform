...
def create_exit_strategy_sheet(ws, styles):
    ...
    val_status = DataValidation(
        type="list", formula1=f'"{CHECK} Compliant,âš ï¸ Partial,⌠Non-Compliant"', allow_blank=False)
    ...
    print("   [OK] Sheet 4: Exit Feasibility & Strategy Assessment created")
    print(f"        Columns: A-AC (29 total)")
    print(f"        Exit Strategy framework: Columns R-AC (12 columns)")
    print(f"        Conditional formatting: On-Prem warning, Lock-In risk, Untested exports")
...
def create_summary_dashboard(ws, styles):
    ...
    for label, sheet in categories:
        ...
        ws.cell(row=row, column=3, value=f'=COUNTIF(\'{sheet}\'!H8:H50,f"{CHECK}*")')
        ws.cell(row=row, column=4, value=f'=COUNTIF(\'{sheet}\'!H8:H50,f"{WARNING}*")')
        ws.cell(row=row, column=5, value=f'=COUNTIF(\'{sheet}\'!H8:H50,f"{XMARK}*")')
    ...
    for metric, formula in reg_metrics:
        ...
        ws.cell(row=row, column=3, value=f'=IF(B{row}>0,f"{WARNING} Review",f"{CHECK} OK")')
        row += 1
...
def main():
    ...
    print("=" * 70)
    print("ISMS-IMP-A.5.23.S1 - Cloud Service Inventory Generator")
    print("WITH REGULATORY COLUMNS (DORA / NIS2 / AI Act / CLOUD Act)")
    print("=" * 70)
    ...
    print("[2/10] Creating 2. SaaS Services...")
    ...
    print("[3/10] Creating 3. IaaS PaaS Services...")
    create_assessment_sheet(
    ...
    print("[4/10] Creating 4. Exit Feasibility & Strategy Assessment...")
    create_exit_strategy_sheet(wb["4. Cloud Security Services"], styles)
    
    print("[5/10] Creating 5. Cloud Storage Services...")
    create_assessment_sheet(
    ...
    print("[6/10] Creating 6. Data Classification... (placeholder)")
    print("[7/10] Creating 7. Service Criticality... (placeholder)")
    
    print("[8/10] Creating 8. Summary Dashboard...")
    create_summary_dashboard(wb["8. Summary Dashboard"], styles)
    
    print("[9/10] Creating 9. Evidence Register...")
    create_evidence_register(wb["9. Evidence Register"], styles)
    
    print("[10/10] Creating 10. Approval Sign-Off...")
    ...
    print(f"{CHART} Sheets: {len(wb.sheetnames)}")
    print(f"Exit Strategy: Sheet 4 (A-AC, 29 cols) | Regulatory: DORA/NIS2/AI/CLOUD")
    print("=" * 70)
