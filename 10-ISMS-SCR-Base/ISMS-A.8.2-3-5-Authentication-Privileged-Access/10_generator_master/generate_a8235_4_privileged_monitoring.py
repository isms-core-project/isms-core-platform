...
"""
================================================================================
ISMS-IMP-A.8.2-3-5-4 - Privileged Access Monitoring Assessment Excel Generator
================================================================================
...
Output:
    ISMS-IMP-A.8.2-3-5.4_Privileged_Monitoring_Assessment_YYYYMMDD.xlsx
...
DOCUMENT_ID = "ISMS-IMP-A.8.2-3-5.4"
VERSION = "1.0"
CONTROL_REF = "ISO/IEC 27001:2022 A.8.2 (Privileged Access Rights - Monitoring)"
...
def populate_instructions(wb, styles):
    ...
    # Title
    ws['A1'] = "Privileged Access Monitoring & Anomaly Detection Assessment"
...
def populate_session_recording(wb, styles):
    ...
    ws[f'B{row}'] = f'=COUNTIFS(B5:B{4+SESSION_RECORDING_COUNT},"Tier 0*",D5:D{4+SESSION_RECORDING_COUNT},f"{CHECK} Enabled*")/COUNTIF(B5:B{4+SESSION_RECORDING_COUNT},"Tier 0*")'
    ws[f'B{row}'].number_format = '0.0%'
    ws[f'C{row}'] = "Target: 100%"
    
    row += 1
    ws[f'A{row}'] = "Tier 1 Prod Recording Coverage:"
    ws[f'B{row}'] = f'=COUNTIFS(B5:B{4+SESSION_RECORDING_COUNT},"Tier 1*",C5:C{4+SESSION_RECORDING_COUNT},"Production",D5:D{4+SESSION_RECORDING_COUNT},f"{CHECK} Enabled*")/COUNTIFS(B5:B{4+SESSION_RECORDING_COUNT},"Tier 1*",C5:C{4+SESSION_RECORDING_COUNT},"Production")'
    ws[f'B{row}'].number_format = '0.0%'
...
def populate_remaining_sheets(wb, styles):
    ...
    for col_idx, header in enumerate(logging_headers, start=1):
        cell = ws_logging.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(anomaly_headers, start=1):
        cell = ws_anomaly.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(activity_headers, start=1):
        cell = ws_activity.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(alert_headers, start=1):
        cell = ws_alerts.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(offhours_headers, start=1):
        cell = ws_offhours.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(failed_headers, start=1):
        cell = ws_failed.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
...
def generate_workbook():
    ...
    print("║  Workbook 4: Privileged Access Monitoring Assessment           ║")
    print("║                                                                ║")
    print("║  'Without monitoring, abuse is invisible'                      ║")
    print("╚════════════════════════════════════════════════════════════════╝")
    print()
    ...
    print("Populating Instructions & Legend...")
    populate_instructions(wb, styles)
    
    print("Populating Session Recording Coverage...")
    ...
    # Save workbook
    filename = f"ISMS-IMP-A.8.2-3-5.4_Privileged_Monitoring_{GENERATED_TIMESTAMP}.xlsx"
    wb.save(filename)
    ...
    print()
    print("Next Steps:")
    print("  1. Verify session recording coverage (Tier 0: 100% target)")
    print("  2. Confirm command logging enabled (PowerShell, sudo, database, cloud)")
    print("  3. Configure anomaly detection rules in SIEM")
    print("  4. Track alert response times (<30 min for critical)")
    print("  5. Review off-hours access log weekly")
    print("  6. Investigate failed login patterns")
    print("  7. Monitor tier violation incidents (target: ZERO)")
    print("  8. Complete quarterly privileged access reviews")
    print("  9. Collect evidence in Evidence_Register")
    print(" 10. Obtain approvals in Approval_Sign_Of")
    print()
    print("Monitoring Targets:")
    print("  • Session Recording: Tier 0 100%, Tier 1 Prod 90%+")
    print("  • Critical Alert Response: <30 minutes average")
    print("  • Tier Violations: ZERO (immediate investigation)")
    print("  • Quarterly Reviews: 100% completion on time")
    print()
...
def validate_workbook(filename):
    ...
    try:
        ...
        if len(wb.sheetnames) != 12:
            print(f"  ❌ Expected 12 sheets, found {len(wb.sheetnames)}")
            return False
