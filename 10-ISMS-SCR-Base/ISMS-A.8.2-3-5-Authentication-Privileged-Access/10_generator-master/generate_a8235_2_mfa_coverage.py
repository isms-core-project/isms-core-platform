...
"""
================================================================================
ISMS-IMP-A.8.2-3-5-2 - Multi-Factor Authentication Coverage Assessment Excel Generator
================================================================================
...
Output:
    ISMS-IMP-A.8.2-3-5.2_MFA_Coverage_YYYYMMDD.xlsx
...
DOCUMENT_ID = "ISMS-IMP-A.8.2-3-5.2"
VERSION = "1.0"
CONTROL_REF = "ISO/IEC 27001:2022 A.8.5 (Secure Authentication - MFA)"
...
def populate_instructions(wb, styles):
    ...
    # Title
    ws['A1'] = "Multi-Factor Authentication (MFA) Coverage Assessment"
...
def populate_user_enrollment(wb, styles):
    ...
    for row_num in range(5, 5 + USER_ROW_COUNT):
        ...
        k_cell.value = f'=IF(AND(J{row_num}<>"",E{row_num}=f"{XMARK} Not Enrolled - Overdue"),TODAY()-J{row_num},"")'
...
def populate_remaining_sheets(wb, styles):
    ...
    for col_idx, header in enumerate(method_headers, start=1):
        cell = ws_method.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(timeline_headers, start=1):
        cell = ws_timeline.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(gap_headers, start=1):
        cell = ws_gaps.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(campaign_headers, start=1):
        cell = ws_campaign.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
    ...
    for col_idx, header in enumerate(backup_headers, start=1):
        cell = ws_backup.cell(row=row, column=col_idx)
        cell.value = header
        apply_style(cell, styles['header'])
...
def generate_workbook():
    ...
    print("║  Workbook 2: MFA Coverage Assessment                           ║")
    print("║                                                                ║")
    print("║  NIS2 Article 21(2)(e): MFA is MANDATORY                       ║")
    print("╚════════════════════════════════════════════════════════════════╝")
    print()
    ...
    print("Populating User MFA Enrollment (200 users)...")
    populate_user_enrollment(wb, styles)
    
    print("Populating MFA Coverage by Type (metrics dashboard)...")
    ...
    # Save workbook
    filename = f"ISMS-IMP-A.8.2-3-5.2_MFA_Coverage_{GENERATED_TIMESTAMP}.xlsx"
    wb.save(filename)
    ...
    print()
    print("Next Steps:")
    print("  1. Export user list from identity provider (Azure AD, Okta, etc.)")
    print("  2. Populate User_MFA_Enrollment with current MFA status")
    print("  3. Review MFA_Coverage_By_Type metrics dashboard")
    print("  4. Identify gaps in MFA_Gaps_Priority worksheet")
    print("  5. Plan enrollment campaigns in Enrollment_Campaign")
    print("  6. Track progress monthly in Enrollment_Timeline")
    print("  7. Collect evidence in Evidence_Register")
    print("  8. Obtain approvals in Approval_Sign_Of")
    print()
    print("Critical Targets:")
    print("  • Privileged Users: 100% MFA (MANDATORY)")
    print("  • Remote Access: 100% MFA (MANDATORY)")
    print("  • Standard Users: 90%+ MFA (RECOMMENDED)")
    print("  • Overall: 85%+ MFA coverage")
    print()
    print(f"{WARNING}  NIS2 REMINDER: MFA is explicitly required by NIS2 Article 21(2)(e)")
    print()
...
def validate_workbook(filename):
    ...
    try:
        ...
        if ws.max_row >= 204:  # 4 header rows + 200 data rows
            print(f"  ✅ User_MFA_Enrollment: {USER_ROW_COUNT} pre-formatted rows")
        else:
            print(f"  ⚠️ User_MFA_Enrollment: Fewer than expected rows")
