...
def create_review_schedule_sheet(wb, styles):
    ...
    for review in schedule:
        ...
        ws.cell(row=row, column=8).value = review["due_date"].strftime("%Y-%m-%d")
...
def create_review_completion_sheet(wb, styles, schedule):
    ...
    for review in schedule[:60]:  # First 60 reviews
        ...
        ws.cell(row=row, column=5).value = review["due_date"].strftime("%Y-%m-%d")
        ...
        if review["due_date"] < today:
            ...
            if random.random() > 0.15:  # 85% completion rate
                ...
                ws.cell(row=row, column=7).value = completion_date.strftime("%Y-%m-%d")
...
def create_review_findings_sheet(wb, styles):
    ...
    for finding in findings:
        ...
        if finding["completion_date"]:
            ws.cell(row=row, column=9).value = finding["completion_date"].strftime("%Y-%m-%d")
...
def create_overdue_reviews_sheet(wb, styles, schedule):
    ...
    for review in overdue:
        ...
        ws.cell(row=row, column=8).value = (today - timedelta(days=random.randint(0, days_overdue))).strftime("%Y-%m-%d")
...
def create_gap_analysis_sheet(wb, styles):
    ...
    gaps = [
        ...
        ("GAP-002", "Finding Remediation", "18 review findings pending action", "Medium", "18 findings",
         ...
         (datetime.now() + timedelta(days=21)).strftime("%Y-%m-%d"), "In Progress"),
...
def main():
    ...
    # Save workbook
    filename = f"Access_Review_Results_{GENERATED_TIMESTAMP}.xlsx"
    ...
    print(f"📊 Sheets: 10")
    print(f"📋 Scheduled reviews: {SCHEDULED_REVIEWS}")
    print("="*80)
    print("\nWorkbook Contents:")
    print("  • Review Schedule (80 annual reviews)")
    print("  • Review Completion Tracking (60 reviews)")
    print("  • Review Findings (60 findings)")
    print("  • Overdue Reviews (15 escalations)")
    print("  • Reviewer Performance (20 reviewers)")
    print("  • Review Metrics & KPIs")
