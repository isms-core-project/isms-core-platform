...
def create_access_matrix_sheet(wb, styles):
    ...
    for mapping in access_mappings:
        ...
        ws.cell(row=row, column=8).value = mapping["granted_date"].strftime("%Y-%m-%d")
        ws.cell(row=row, column=9).value = mapping["granted_by"]
        ws.cell(row=row, column=10).value = mapping["last_used"].strftime("%Y-%m-%d") if mapping["last_used"] else "Never"
...
def create_role_assignments_sheet(wb, styles, users):
    ...
    for i in range(min(ROLE_ASSIGNMENT_COUNT, len(users))):
        ...
        ws.cell(row=row, column=6).value = (datetime.now() - timedelta(days=random.randint(30, 365))).strftime("%Y-%m-%d")
...
def create_group_memberships_sheet(wb, styles, users):
    ...
    for i in range(GROUP_COUNT):
        ...
        ws.cell(row=row, column=6).value = (datetime.now() - timedelta(days=random.randint(180, 1095))).strftime("%Y-%m-%d")
        ws.cell(row=row, column=7).value = (datetime.now() - timedelta(days=random.randint(1, 90))).strftime("%Y-%m-%d")
...
def create_privileged_access_sheet(wb, styles, users):
    ...
    for i in range(min(PRIVILEGED_ACCESS_COUNT, len(users))):
        ...
        ws.cell(row=row, column=7).value = (datetime.now() - timedelta(days=random.randint(90, 730))).strftime("%Y-%m-%d")
        ...
        ws.cell(row=row, column=10).value = (datetime.now() - timedelta(days=random.randint(0, 180))).strftime("%Y-%m-%d")
...
def create_access_documentation_sheet(wb, styles):
    ...
    for i in range(DOCUMENTATION_COUNT):
        ...
        ws.cell(row=row, column=5).value = (datetime.now() - timedelta(days=random.randint(30, 730))).strftime("%Y-%m-%d")
        ...
        ws.cell(row=row, column=8).value = (datetime.now() - timedelta(days=random.randint(30, 730))).strftime("%Y-%m-%d") if justification else ""
...
def create_gap_analysis_sheet(wb, styles):
    ...
    gaps = [
        ("GAP-001", "Documentation", "15 access grants missing business justification", "Medium", "15 users", 
         ...
         (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d"), "Open"),
        ("GAP-002", "RBAC Adoption", "20 users with direct access (not via roles)", "Medium", "20 users",
         ...
         (datetime.now() + timedelta(days=45)).strftime("%Y-%m-%d"), "In Progress"),
...
def main():
    ...
    # Save workbook
    filename = f"Access_Rights_Matrix_{GENERATED_TIMESTAMP}.xlsx"
    ...
    print(f"📊 Sheets: 10")
    print(f"📋 Access mappings: {ACCESS_MAPPING_COUNT}")
    print("="*80)
    print("\nWorkbook Contents:")
    print("  • Access Rights Matrix (150 user-to-system mappings)")
    print("  • Role Assignments (80 RBAC users)")
    print("  • Group Memberships (100 groups)")
    print("  • Privileged Access Tracking (25 admin users)")
    print("  • Access Documentation Completeness")
    print("  • Coverage Analysis (system-level stats)")
    print("  • Gap Analysis & Remediation")
    print("  • Evidence Register for A.5.18")
