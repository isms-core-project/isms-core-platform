    ...
    conflicts = [
        {
            ...
            "compensating_controls": "Dual approval for payments >$10,000",
        },
        {
            "conflict_id": "SOD-002",
...
def create_role_catalog_sheet(wb, styles):
    ...
    for role in roles:
        ...
        ws.cell(row=row, column=8).value = (datetime.now() - timedelta(days=random.randint(30, 365))).strftime("%Y-%m-%d")
...
def create_role_assignments_sheet(wb, styles, roles):
    ...
    for user in users:
        ...
        ws.cell(row=row, column=6).value = (datetime.now() - timedelta(days=random.randint(30, 730))).strftime("%Y-%m-%d")
...
def create_direct_access_sheet(wb, styles):
    ...
    for user in users:
        ...
        ws.cell(row=row, column=7).value = (datetime.now() - timedelta(days=random.randint(180, 1095))).strftime("%Y-%m-%d")
...
def create_sod_violations_sheet(wb, styles, conflicts):
    ...
    for i, user in enumerate(users):
        ...
        ws.cell(row=row, column=8).value = (datetime.now() - timedelta(days=random.randint(1, 30))).strftime("%Y-%m-%d")
        ...
        ws.cell(row=row, column=10).value = (datetime.now() + timedelta(days=random.randint(7, 30))).strftime("%Y-%m-%d")
...
def create_gap_analysis_sheet(wb, styles):
    ...
    gaps = [
        ("GAP-001", "RBAC Adoption", "20 users with direct access (non-RBAC)", "Medium", "20 users",
         ...
         (datetime.now() + timedelta(days=90)).strftime("%Y-%m-%d"), "In Progress"),
        ...
        ("GAP-004", "Role Documentation", "3 roles missing complete documentation", "Low", "3 roles",
         ...
         (datetime.now() + timedelta(days=45)).strftime("%Y-%m-%d"), "Open"),
    ]
...
def main():
    ...
    print("  2/10 Creating Role Catalog...")
    ...
    print("  4/10 Creating Direct Access Users...")
    create_direct_access_sheet(wb, styles)
    
    print("  5/10 Creating SoD Matrix...")
    _, conflicts = create_sod_matrix_sheet(wb, styles)
    
    print("  6/10 Creating SoD Violations...")
    create_sod_violations_sheet(wb, styles, conflicts)
    
    print("  7/10 Creating RBAC Metrics...")
    ...
    # Save workbook
    filename = f"Role_Compliance_SoD_{GENERATED_TIMESTAMP}.xlsx"
    ...
    print(f"📊 Sheets: 10")
    print(f"📋 Roles defined: {ROLE_COUNT}")
    print("="*80)
    print("\nWorkbook Contents:")
    print("  • Role Catalog (30 RBAC roles)")
    print("  • Role Assignments (80 users)")
    print("  • Direct Access Users (20 non-RBAC users)")
    print("  • SoD Matrix (6 conflict definitions)")
    print("  • SoD Violations (5 detected violations)")
    print("  • RBAC Adoption Metrics & KPIs")
    print("  • Gap Analysis & Remediation")
    print("  • Evidence Register for A.5.15/A.5.18")
