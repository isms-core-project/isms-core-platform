...
def populate_executive_dashboard(wb):
    ...
    print(f"{CHART} Creating Executive Dashboard...")
    ...
    today = datetime.now()
    ws["A2"] = f"Last Updated: {today.strftime('%Y-%m-%d %H:%M')}"
    ...
    for idx in range(0, len(kpis), 2):
        ...
        ws[f"C{row}"].font = Font(size=16, bold=True, color="203864")
        ...
        if idx + 1 < len(kpis):
            ...
            ws[f"G{row}"].font = Font(size=16, bold=True, color="203864")
...
def populate_regulatory_status(wb):
    ...
    print("📋 Creating Regulatory Status...")
...
def populate_requirements_progress(wb):
    ...
    print("📈 Creating Requirements Progress...")
...
def populate_control_coverage(wb):
    ...
    print(f"{TARGET} Creating Control Coverage...")
    ...
    for sec, total, impl, prog, not_started, pct in sections:
        ...
        if bold:
            for col in range(1, 7):
                ws.cell(row, col).fill = PatternFill(start_color="D9E1F2", fill_type="solid")
...
def populate_evidence_health(wb):
    ...
    print(f"{CHART} Creating Evidence Health...")
    ...
    metrics = [
        ...
        (f"{CHECK} Verified & Current", "132 (85%)"),
        ("⏳ Pending Verification", "12 (8%)"),
        (f"{XMARK} Expired / Outdated", "8 (5%)"),
        ("❓ Missing / Not Collected", "4 (3%)"),
        ("", ""),
        ("Audit Ready Status", ""),
        (f"{CHECK} Audit Ready", "142 (91%)"),
        (f"{WARNING} Needs Update", "10 (6%)"),
        (f"{XMARK} Not Audit Ready", "4 (3%)"),
    ]
...
def populate_action_items(wb):
    ...
    ws = wb["Action_Items"]
    print(f"{CHECK} Creating Action Items...")
...
def main():
    ...
    print("Control A.5.31 - Workbook 6: Compliance Dashboard")
    print("=" * 70)
    print()
    ...
    print("   File: ISMS_Assessment_531_6_Compliance_Dashboard.xlsx")
    print("   Sheets: 7 (Executive_Dashboard, Regulatory_Status, Requirements_Progress,")
    print("           Control_Coverage, Evidence_Health, Action_Items, Instructions)")
    print()
    print(f"{TARGET} Dashboard Features:")
    print("   ✓ Executive KPI summary")
    print("   ✓ Critical alerts (expired, missing)")
    print("   ✓ Regulatory compliance status by regulation")
    print("   ✓ Requirements progress tracking")
    print("   ✓ ISO 27001 control coverage analysis")
    print("   ✓ Evidence health metrics")
