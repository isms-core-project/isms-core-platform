...
def populate_evidence_register(wb):
    ...
    print("📝 Populating Evidence Register...")
    ...
    ws.conditional_formatting.add("K2:K1000",
        CellIsRule(operator="containsText", formula=['f"{CHECK}"'], fill=verified_fill))
    ws.conditional_formatting.add("K2:K1000",
        CellIsRule(operator="containsText", formula=['"⏳"'], fill=pending_fill))
    ws.conditional_formatting.add("K2:K1000",
        CellIsRule(operator="containsText", formula=['f"{XMARK}"'], fill=expired_fill))
    ws.conditional_formatting.add("K2:K1000",
    ...
    ws.conditional_formatting.add("O2:O1000",
        CellIsRule(operator="containsText", formula=['f"{CHECK}"'], fill=verified_fill))
    ws.conditional_formatting.add("O2:O1000",
        CellIsRule(operator="containsText", formula=['f"{WARNING}"'], fill=pending_fill))
    ws.conditional_formatting.add("O2:O1000",
        CellIsRule(operator="containsText", formula=['f"{XMARK}"'], fill=expired_fill))
...
def populate_instructions(wb):
    ...
    ws["A1"] = "EVIDENCE REGISTER - INSTRUCTIONS"
    ws["A1"].font = Font(size=16, bold=True, color="FFFFFF")
    ws["A1"].fill = PatternFill(start_color="203864", fill_type="solid")
...
def populate_summary_sheets(wb):
    ...
    print(f"{CHART} Populating Summary Sheets...")
    
    # Evidence by Regulation
    w5.31.3 = wb["Evidence_by_Regulation"]
    w5.31.3.merge_cells("A1:G1")
    w5.31.3["A1"] = "EVIDENCE SUMMARY - BY REGULATION"
    w5.31.3["A1"].font = Font(size=14, bold=True, color="FFFFFF")
    w5.31.3["A1"].fill = PatternFill(start_color="203864", fill_type="solid")
    w5.31.3["A1"].alignment = Alignment(horizontal="center")
    
    headers = ["Regulation ID", "Regulation", "Total Evidence", "Verified", "Pending", "Expired", "Missing"]
    for idx, h in enumerate(headers, 1):
        cell = w5.31.3.cell(2, idx, h)
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill(start_color="003366", fill_type="solid")
    
    w5.31.3.column_dimensions["A"].width = 15
    w5.31.3.column_dimensions["B"].width = 30
    for col in ["C", "D", "E", "F", "G"]:
        w5.31.3.column_dimensions[col].width = 12
    
    regs = [("REG-EU-001", "GDPR"), ("REG-INT-001", "ISO 27001"), ("REG-CH-001", "Swiss FADP")]
    for idx, (rid, rname) in enumerate(regs, 3):
        w5.31.3.cell(idx, 1, rid).font = Font(size=9, bold=True)
        w5.31.3.cell(idx, 2, rname).font = Font(size=9)
        w5.31.3.cell(idx, 3, f'=COUNTIF(Evidence_Register!D:D,"{rid}")').font = Font(size=9, bold=True)
        w5.31.3.cell(idx, 4, f'=COUNTIFS(Evidence_Register!D:D,"{rid}",Evidence_Register!K:K,"*✅*")').font = Font(size=9)
        w5.31.3.cell(idx, 5, f'=COUNTIFS(Evidence_Register!D:D,"{rid}",Evidence_Register!K:K,"*⏳*")').font = Font(size=9)
        w5.31.3.cell(idx, 6, f'=COUNTIFS(Evidence_Register!D:D,"{rid}",Evidence_Register!K:K,"*❌*")').font = Font(size=9)
        w5.31.3.cell(idx, 7, f'=COUNTIFS(Evidence_Register!D:D,"{rid}",Evidence_Register!K:K,"*❓*")').font = Font(size=9)
    
    # Evidence by Control
    w5.31.4 = wb["Evidence_by_Control"]
    w5.31.4.merge_cells("A1:F1")
    w5.31.4["A1"] = "EVIDENCE SUMMARY - BY ISO 27001 CONTROL"
    w5.31.4["A1"].font = Font(size=14, bold=True, color="FFFFFF")
    w5.31.4["A1"].fill = PatternFill(start_color="203864", fill_type="solid")
    
    headers2 = ["Control ID", "Control", "Total Evidence", "Verified", "Pending", "Missing"]
    for idx, h in enumerate(headers2, 1):
        cell = w5.31.4.cell(2, idx, h)
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill(start_color="003366", fill_type="solid")
    
    w5.31.4.column_dimensions["A"].width = 12
    w5.31.4.column_dimensions["B"].width = 50
    for col in ["C", "D", "E", "F"]:
        w5.31.4.column_dimensions[col].width = 12
    ...
    for idx, (cid, cname) in enumerate(controls, 3):
        w5.31.4.cell(idx, 1, cid).font = Font(size=9, bold=True)
        w5.31.4.cell(idx, 2, cname).font = Font(size=9)
        w5.31.4.cell(idx, 3, f'=COUNTIF(Evidence_Register!C:C,"{cid}")').font = Font(size=9, bold=True)
        w5.31.4.cell(idx, 4, f'=COUNTIFS(Evidence_Register!C:C,"{cid}",Evidence_Register!K:K,"*✅*")').font = Font(size=9)
        w5.31.4.cell(idx, 5, f'=COUNTIFS(Evidence_Register!C:C,"{cid}",Evidence_Register!K:K,"*⏳*")').font = Font(size=9)
        w5.31.4.cell(idx, 6, f'=COUNTIFS(Evidence_Register!C:C,"{cid}",Evidence_Register!K:K,"*❓*")').font = Font(size=9)


def populate_alerts(wb):
    """Populate Alerts & Actions sheet."""
    ws = wb["Alerts_Actions"]
    print(f"{WARNING} Populating Alerts & Actions...")
...
def main():
    ...
    print("Control A.5.31 - Workbook 5: Evidence Register")
    print("=" * 70)
    print()
    ...
    print("   File: ISMS_Assessment_531_5_Evidence_Register.xlsx")
    print("   Sheets: 5 (Evidence_Register, Instructions, Evidence_by_Regulation,")
    print("           Evidence_by_Control, Alerts_Actions)")
    print("   Sample Data: 11 evidence items across multiple regulations")
    print()
    print("📋 Features:")
    print("   ✓ 17-column evidence tracking")
    print("   ✓ Data validation (Type, Status, Frequency, Audit Ready)")
    print("   ✓ Conditional formatting (emoji-based status colors)")
    print("   ✓ Auto-filter enabled")
    print("   ✓ Summary by regulation and control")
    print("   ✓ Alerts dashboard (expired, upcoming, missing)")
    print("   ✓ UTF-8 encoding with emoji support")
    print()
    print("=" * 70)
