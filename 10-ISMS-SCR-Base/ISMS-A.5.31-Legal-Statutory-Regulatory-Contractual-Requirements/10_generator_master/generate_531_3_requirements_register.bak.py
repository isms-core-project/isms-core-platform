...
def apply_conditional_formatting(ws):
    ...
    ws.conditional_formatting.add(
        "J2:J1000",
        CellIsRule(operator="containsText", formula=['f"{CHECK}"'], fill=implemented_fill)
    )
    ws.conditional_formatting.add(
        "J2:J1000",
    ...
    ws.conditional_formatting.add(
        "L2:L1000",
        CellIsRule(operator="containsText", formula=['f"{XMARK}"'], fill=complete_gap_fill)
    )
    ws.conditional_formatting.add(
        "L2:L1000",
        CellIsRule(operator="containsText", formula=['f"{WARNING}"'], fill=partial_gap_fill)
    )
    ws.conditional_formatting.add(
        "L2:L1000",
        CellIsRule(operator="containsText", formula=['f"{CHECK}"'], fill=no_gap_fill)
    )
    ws.conditional_formatting.add(
        "L2:L1000",
...
def populate_instructions_sheet(wb):
    ...
    title_cell = ws["A1"]
    title_cell.value = "REQUIREMENTS REGISTER - INSTRUCTIONS"
...
def populate_summary_sheet(wb):
    ...
    ws.cell(row=row, column=2).font = Font(bold=True, size=12, color="203864")
    ...
    statuses = [
        (f"{CHECK} Implemented:", 'f"{CHECK}"', "00B050"),
        ("⏳ In Progress:", '"⏳"', "FF9900"),
        ("➖ Not Started:", '"➖"', "C00000"),
        ("❓ N/A:", '"❓"', "808080"),
    ]
    ...
    ws.cell(row=row, column=2).font = Font(bold=True, size=11, color="203864")
    ...
    gaps = [
        (f"{XMARK} Complete Gap:", 'f"{XMARK}"', "C00000"),
        (f"{WARNING} Partial Gap:", 'f"{WARNING}"', "FF9900"),
        (f"{CHECK} No Gap:", 'f"{CHECK}"', "00B050"),
        ("🔍 TBD:", '"🔍"', "003366"),
    ]
...
def main():
    ...
    print("Control A.5.31 - Workbook 3: Requirements Register")
    print("=" * 70)
    print()
    ...
    # Populate sheets
    print("📝 Populating Requirements Register sheet...")
    populate_register_sheet(wb, styles)
    
    print("📖 Populating Instructions sheet...")
    populate_instructions_sheet(wb)
    
    print(f"{CHART} Populating Summary Metrics sheet...")
    populate_summary_sheet(wb)
    
    print("📋 Populating Extraction Worksheet...")
    ...
    print(f"   File: ISMS_Assessment_531_3_Requirements_Register.xlsx")
    print(f"   Location: ../90_workbooks/")
    print(f"   Sheets: 4 (Requirements_Register, Instructions, Summary_Metrics, Extraction_Worksheet)")
    print(f"   Sample Data: 13 example requirements from 5 regulations")
    print()
    print("📋 Features:")
    print("   ✓ 18-column requirements register")
    print("   ✓ Data validation (Category, Priority, Status, Gap dropdowns)")
    print("   ✓ Conditional formatting (emoji-based status colors)")
    print("   ✓ Auto-filter enabled")
    print("   ✓ Freeze panes (header row)")
    print("   ✓ Summary metrics with formulas")
    print("   ✓ Extraction worksheet for article-by-article review")
    print("   ✓ Comprehensive instructions")
    print("   ✓ UTF-8 encoding with emoji support (✅❌⚠️⏳🔍🔴🟡🟢)")
    print()
    print(f"{TARGET} Next Steps:")
    print("   1. Use Extraction Worksheet to review regulations systematically")
    print("   2. Extract requirements into Requirements Register")
    print("   3. Assign categories, priorities, and deadlines")
    print("   4. Map requirements to controls (Workbook 4)")
    print("   5. Identify and remediate gaps")
    print("   6. Collect evidence (Workbook 5)")
    print()
    print("=" * 70)
