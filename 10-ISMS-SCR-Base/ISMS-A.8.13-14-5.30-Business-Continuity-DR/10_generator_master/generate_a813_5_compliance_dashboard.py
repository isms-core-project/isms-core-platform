...
Controls A.8.13, A.8.14, and A.5.30, enabling evidence-based decision-making
...
Basic Usage:
    python3 generate_a813_5_compliance_dashboard.py

Advanced Usage:
    # Generate with custom output directory
    python3 generate_a813_5_compliance_dashboard.py --output /path/to/dir
    
    # Generate with specific date suffix
    python3 generate_a813_5_compliance_dashboard.py --date 20250125
...
Post-Generation Steps:
    ...
    2. Run normalize_a813_assessments.py to normalize source file names
...
Version 1.0 - [Date to be set]
    - Initial release
    - Implements full dashboard framework per ISMS-IMP-A.8.13-14-5.30 specification
...
def create_detailed_metrics_sheet(wb, data):
    ...
    ws[f'A{row}'] = 'BACKUP COVERAGE & COMPLIANCE (A.8.13)'
    apply_style(ws[f'A{row}'], font=SUBHEADER_FONT, fill=SUBHEADER_FILL,
                alignment=Alignment(horizontal='center'))
    ...
    row += 1
    ws.merge_cells(f'A{row}:D{row}')
    ws[f'A{row}'] = 'RPO/RTO COMPLIANCE (A.8.13, A.8.14, A.5.30)'
    apply_style(ws[f'A{row}'], font=SUBHEADER_FONT, fill=SUBHEADER_FILL,
                alignment=Alignment(horizontal='center'))
    ...
    row += 1
    ws.merge_cells(f'A{row}:D{row}')
    ws[f'A{row}'] = 'TESTING COMPLIANCE (A.8.13, A.8.14, A.5.30)'
    apply_style(ws[f'A{row}'], font=SUBHEADER_FONT, fill=SUBHEADER_FILL,
                alignment=Alignment(horizontal='center'))
...
def create_evidence_checklist_sheet(wb, data):
    ...
    evidence_data = [
        ('A.8.13', 'BCDR_1_Backup_Inventory.xlsx', 
        ...
        ('A.8.13, A.8.14, A.5.30', 'BCDR_3_RPO_RTO_Compliance.xlsx', 
         data['rporto'].get('evidence_count', 0) if data['rporto'] else 0, 5),
        ('A.8.13, A.8.14, A.5.30', 'BCDR_4_Testing_Results.xlsx', 
