...
ISO/IEC 27001:2022 Control A.8.4 (Access to Source Code - Branch Protection)
...
def create_instructions_sheet(wb, styles):
    ...
    row += 1
    legend_data = [
        (f"{CHECK}", "Compliant", "Protection correctly configured"),
        (f"{WARNING}", "Partial", "Some protection rules missing"),
        (f"{XMARK}", "Non-Compliant", "Protection not configured"),
...
def create_protection_details_sheet(wb, styles, validations):
    ...
    ws[f'L{row}'] = "=((COUNTIF(C4:K4,f\"{CHECK} Yes\")/9)*100)"
...
def create_exception_management_sheet(wb, styles, validations):
    ...
    ws[f'F{row}'] = (datetime.now() + timedelta(days=90)).strftime("%Y-%m-%d")
    ws[f'G{row}'] = (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d")
    ws[f'H{row}'] = "🟢 Active"
...
def create_compliance_scoring_sheet(wb, styles):
    ...
    metrics = [
        ("Branch Protection Configuration Rate",
         "=COUNTIFS(Branch_Protection_Details!M:M,f\"{CHECK} Compliant\")/COUNTA(Branch_Protection_Details!M:M)*100",
         "100%", "", "40%"),
        ("Pull Request Enforcement Rate",
         "=COUNTIFS(Pull_Request_Configuration!H:H,f\"{CHECK} Compliant\")/COUNTA(Pull_Request_Configuration!H:H)*100",
         "≥95%", "", "30%"),
        ("Status Check Compliance Rate",
         "=COUNTIFS(Status_Check_Verification!I:I,f\"{CHECK} Compliant\")/COUNTA(Status_Check_Verification!I:I)*100",
         "100%", "", "20%"),
...
def main():
    ...
    print("Starting ISMS-A84-2 Branch Protection Assessment Workbook Generation...")
    ...
    print("Creating Repository_Branch_Inventory sheet...")
    create_branch_inventory_sheet(wb, styles, validations)
    
    print("Creating Branch_Protection_Details sheet...")
    create_protection_details_sheet(wb, styles, validations)
    
    print("Creating Pull_Request_Configuration sheet...")
    create_pr_configuration_sheet(wb, styles, validations)
    
    print("Creating Status_Check_Verification sheet...")
    create_status_check_sheet(wb, styles, validations)
    
    print("Creating Signed_Commits_Audit sheet...")
    create_signed_commits_sheet(wb, styles, validations)
    
    print("Creating Exception_Management sheet...")
