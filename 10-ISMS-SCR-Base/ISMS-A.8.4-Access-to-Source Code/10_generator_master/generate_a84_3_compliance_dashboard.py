...
# -*- coding: utf-8 -*-
"""
ISMS-IMP-A.8.4.3 - Source Code Security Compliance Dashboard Generator
ISO/IEC 27001:2022 Control A.8.4 (Access to Source Code - Master Dashboard)
...
def create_gap_priority_matrix_sheet(wb, styles):
    ...
    row += 1
    gaps = [
        ("🔴 Critical", "2 critical secrets exposed", "High", "2 days", (datetime.now() + timedelta(days=2)).strftime("%Y-%m-%d"), "🟡 In Progress"),
        ("🟠 High", "5 repos without branch protection", "Medium", "2 weeks", (datetime.now() + timedelta(days=14)).strftime("%Y-%m-%d"), "🟡 In Progress"),
        ("🟡 Medium", "Pre-commit hooks adoption <90%", "Low", "1 month", (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d"), "🔴 Not Started"),
    ]
...
def create_action_items_sheet(wb, styles):
    ...
    row += 1
    actions = [
        ("ACT-001", "Remediate secret exposure in api-gateway", "Security Team", "🔴 Critical", (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d"), "🟡 In Progress", "0"),
        ("ACT-002", "Enable branch protection on 5 repos", "Platform Team", "🟠 High", (datetime.now() + timedelta(days=14)).strftime("%Y-%m-%d"), "🟡 In Progress", "0"),
        ("ACT-003", "Complete overdue access reviews", "Repo Owners", "🟡 Medium", (datetime.now() + timedelta(days=7)).strftime("%Y-%m-%d"), "🔴 Not Started", "0"),
    ]
...
def main():
    ...
    print("Starting ISMS-A84-3 Source Code Security Dashboard Generation...")
    ...
    print("Creating Repository_Overview sheet...")
    create_repository_overview_sheet(wb, styles)
    
    print("Creating Access_Control_Metrics sheet...")
    create_access_control_metrics_sheet(wb, styles)
    
    print("Creating Branch_Protection_Metrics sheet...")
    create_branch_protection_metrics_sheet(wb, styles)
    
    print("Creating Secret_Management_Metrics sheet...")
    create_secret_management_metrics_sheet(wb, styles)
    
    print("Creating Third_Party_Access sheet...")
    create_third_party_access_sheet(wb, styles)
    
    print("Creating Trend_Analysis sheet...")
    create_trend_analysis_sheet(wb, styles)
    
    print("Creating Gap_Priority_Matrix sheet...")
    create_gap_priority_matrix_sheet(wb, styles)
    
    print("Creating Action_Items sheet...")
    create_action_items_sheet(wb, styles)
    
    print("Creating Evidence_Summary sheet...")
