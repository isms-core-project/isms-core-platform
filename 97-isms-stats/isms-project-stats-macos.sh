...
# ISMS Project Statistics Generator (macOS Compatible)
...
# Count by file type
TOTAL_MD_FILES=$(find . -name "*.md" -type f 2>/dev/null | wc -l | tr -d ' ')
TOTAL_PY_FILES=$(find . -name "*.py" -type f 2>/dev/null | wc -l | tr -d ' ')
TOTAL_WORKBOOKS=$(find . -name "*.xlsx" -type f 2>/dev/null | wc -l | tr -d ' ')

# Count POL/IMP/SCR files
TOTAL_POL_FILES=$(find . -type f \( -name "*POL*.md" -o -name "*Policy*.md" \) 2>/dev/null | wc -l | tr -d ' ')
TOTAL_IMP_FILES=$(find . -type f \( -name "*IMP*.md" -o -name "*Implementation*.md" \) 2>/dev/null | wc -l | tr -d ' ')
TOTAL_SCR_FILES=$(find . -type f \( -name "*SCR*.py" -o -name "*Script*.py" \) -o -path "*/scripts/*.py" 2>/dev/null | wc -l | tr -d ' ')
...
if [ "$TOTAL_PY_FILES" -gt 0 ]; then
    ...
    TOTAL_LINES_CODE=$(find . -name "*.py" -type f -exec cat {} \; 2>/dev/null | wc -l | tr -d ' ')
    echo "🐍 Total Python Lines:         $TOTAL_LINES_CODE"
fi

if [ "$TOTAL_MD_FILES" -gt 0 ]; then
    echo "Counting Markdown documentation lines..."
    TOTAL_LINES_DOCS=$(find . -name "*.md" -type f -exec cat {} \; 2>/dev/null | wc -l | tr -d ' ')
...
echo "Scanning for ISO 27001:2022 Annex A controls..."
...
for section in 5 6 7 8; do
    for control in {1..34}; do
        ...
        # Search for control references in files and directories
        if find . -type f -name "*${control_id}*" 2>/dev/null | grep -q .; then
            echo "$control_id" >> "$CONTROLS_FILE"
        elif find . -type d -name "*${control_id}*" 2>/dev/null | grep -q .; then
            echo "$control_id" >> "$CONTROLS_FILE"
            # Store folder name
            folder=$(find . -type d -name "*${control_id}*" 2>/dev/null | head -1)
            echo "$control_id|$folder" >> "$CONTROL_FOLDERS_FILE"
        elif grep -r -l "${control_id}" . 2>/dev/null | head -1 | grep -q .; then
            echo "$control_id" >> "$CONTROLS_FILE"
        fi
    done
done
...
CODE_SIZE=$(find . -name "*.py" -type f -exec du -ch {} + 2>/dev/null | tail -1 | cut -f1 || echo "0")
DOCS_SIZE=$(find . -name "*.md" -type f -exec du -ch {} + 2>/dev/null | tail -1 | cut -f1 || echo "0")
